...
$vbfiles=array("C1Sub","C2Sub","D1Sub","D2Sub","C1","C2","D1","D2","B","A");

$vbfields=array("classdist","location","grossreceipts","offtotal","insdeduct","hostgiven","hostallow","nsaaallow","balance","milespaid","milespaidtot","prorate","milespaidpro","milespaidprotot","bonus","hostperc","hostbonus","nsaabonus","visperc","visitorbonus","grossreceipts","offtotal","hosttotal","hostexp","nsaapartial","insdeduct","nsaatotal");

$vbtitles=array("District","Site","Total Receipts","Officials","Insurance-10%","Host Allowance","Host School","NSAA","Balance","Expenses","Total Expenses","Prorate","Expenses","Expenses Paid","Balance Bonus","% Host","Host School","NSAA","% Teams","Teams","Total Receipts","Officials","Host","Teams","NSAA","Insurance","Check Amount");
...
for($file=0;$file<count($vbfiles);$file++)
{
    ...
    $sql="SELECT * FROM finance_vb WHERE class='$class' AND round='$round' ORDER BY district";
    $result=mysql_query($sql);
    $tid=0;	//tournament id
    $tourn=array();
    while($row=mysql_fetch_array($result))
    {
       // GET (SUB)DISTRICT INFO FROM finance_vb: 
       ...
       $sql2="SELECT * FROM finance_vb_exp WHERE class='$class' AND district='".$tourn[district][$tid]."' AND round='$round' ORDER BY school";
   ...
   for($field=0;$field<count($vbfields);$field++)
   {
	 ...
	 for($t=0;$t<$tid;$t++)
	 {
	    ...
	    else if($vbfields[$field]=="hostexp")
	       $totalvis+=$tourn[$vbfields[$field]][$t];
	    else if($vbfields[$field]=="nsaapartial")
	       $totalnsaa+=$tourn[$vbfields[$field]][$t];
	    else if($vbfields[$field]=="insdeduct")
