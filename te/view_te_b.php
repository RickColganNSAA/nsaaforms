...
/*******************************************
view_te_b.php
VIEW District Entry, Boys Tennis
Created 6/27/09
Author: Ann Gaffigan
********************************************/
...
else if($sid)
{
   ...
   $hostid=$row[0];
   $sql="SELECT * FROM $db_name2.tebdistricts WHERE hostid='$hostid'";
   $result=mysql_query($sql);
...
echo "<br><a class=small href=\"host_teb.php?school_ch=$school_ch&session=$session\">".$sportname." District Host Main Menu</a><br><br>";
if($gender=="M") $hisher="his";
else $hisher="her";
echo "<table cellspacing=4 cellpadding=4><caption><b><u>$school</u> ".$sportname." District Entry:</b><br><br>";
//echo "<a href=\"edit_te_b.php?session=$session&school_ch=$sid\">Edit this Entry Form (use Substitutes)</a>";
echo "</b></caption>";
