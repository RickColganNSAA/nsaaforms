...
if($level==1) 
   echo "<br><a href=\"te_gmain.php?session=$session\" class=small>$sportname STATE SEEDING & BRACKETS Main Menu</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href=\"te_gdistricts.php?session=$session\" class=small>$sportname DISTRICT RESULTS Main Menu</a><br>";
...
if($class=="Z")	//No State Entry, only District Entry - NO DISTRICTS FOR ANY CLASS AS OF 10/7/11
{
   if($level==1)
...
else
{
   ...
   if(PastDue(GetDueDate('te_gstate'),0) || $secret)
      echo "<tr align=left><td><a href=\"stentries.php?session=$session&sport=$sport\">See ALL State Entries</a></td></tr>";
   else
   {
      $date=split("-",GetDueDate('te_gstate'));
