<?php
/************************************
meetresults_te_g.php
Enter Meet Results for Girls TE Meet 
Created 7/18/08
Author: Ann Gaffigan
************************************/
...
if($hiddensave)
{
   ...
   header("Location:meetresults_te_g.php?session=$session&school_ch=$school_ch&added=$added&meetid=$meetid");
   exit();
}
...
?>
<body onload="Team2.initialize('<?php echo $session; ?>','te_g','oppid','oppplayer','1');TEMeetResults.initialize('showresults','<?php echo $sport; ?>','<?php echo $meetid; ?>','<?php echo $sid; ?>','<?php echo $session; ?>','<?php echo $school_ch2; ?>','<?php echo $curresultid; ?>');">
<?php
echo $header;

echo "<br><form method=post name=resultsform action=\"meetresults_te_g.php\">";
...
//RESULTS
      ...
      $sql="SELECT * FROM ".$sport."meetresults WHERE (oppid1='$sid' OR oppid2='$sid') AND meetid='$meetid' ORDER BY division";
      $result=mysql_query($sql);
      $string="";
      while($row=mysql_fetch_array($result))
      {
         ...
         if($row[winnerid]==$sid) $winloss="WIN";
         else $winloss="LOSS";
