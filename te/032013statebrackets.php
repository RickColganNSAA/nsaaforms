...
if($saveround1)	//save opponents for round 1
{
   foreach($round as $index => $rnd) 
   {
      if($rnd==1)
      {
         $curplayers=$players[1][$line[$index]];
	 ...
	 else
	 {
	    $player1=$curplayers; $player2=0;
         ...
         $sql="SELECT * FROM ".$sport."brackets WHERE class='$class' AND division='$division' AND round='1' AND line='$line[$index]'";
         $result=mysql_query($sql);
	 if(mysql_num_rows($result)==0)	//INSERT
	    $sql2="INSERT INTO ".$sport."brackets (class,division,round,line,player1,player2) VALUES ('$class','$division','1','$line[$index]','$player1','$player2')";
	 else
	    $sql2="UPDATE ".$sport."brackets SET player1='$player1',player2='$player2' WHERE class='$class' AND division='$division' AND round='1' AND line='$line[$index]'";
...
for($i=0;$i<count($entries[name]);$i++)
{
   $sql="SELECT * FROM ".$sport."brackets WHERE class='$class' AND division='$division' AND round='1' AND player1='".$entries[player1][$i]."'";
   if(ereg("doubles",$division))
      $sql.=" AND player2='".$entries[player2][$i]."'";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0)	//IF PLAYER(S) NOT ON BRACKET YET, GIVE THEM A RANDOM SPOT IN ROUND 1
   {
      //get random line
      if($entryct==33)
         $randline=rand(1,32);	//IF THEY GET A 14, then randomly determined whether they get Line 1 or 2 for Round "0"
      else 
         $randline=rand(1,$entryct);

      $sql2="SELECT * FROM ".$sport."brackets WHERE class='$class' AND division='$division' AND round='1' AND line='$randline' AND (player1>0 OR bye='x')";
      $result2=mysql_query($sql2);
      $through=0;
      while($row2=mysql_fetch_array($result2))	//IF PLAYER/BYE ALREADY ON THIS LINE, GET DIFFERENT LINE
      {
         $randline++; 
         if($class=="Z" && $randline>16) { $randline=1; $through++; }//ALL BOYS CLASSES & GIRLS - NO DISTRICTS (AS OF 10/7/11)
         else if($class!='Z' && $randline>32) { $randline=1; $through++; }
         $sql2="SELECT * FROM ".$sport."brackets WHERE class='$class' AND division='$division' AND round='1' AND line='$randline' AND (player1>0 OR bye='x')";
         $result2=mysql_query($sql2);
         //if($randline==17) echo "$sql2<br>";
         if($through==3 && mysql_num_rows($result2)>0)
         {
	     echo "$sql2<br>Can't find an open slot for ".GetStudentInfo($entries[player1][$i])." #".$entries[player1][$i]."<br>";
	     exit();
	 //THIS BECOMES AN INFINITE LOOP IF THERE ARE TOO MANY BYES CHECKED.
	 }
      }
      ...
      $sql2="INSERT INTO ".$sport."brackets (class,division,round,line,player1,player2) VALUES ('$class','$division','1','$randline','".$entries[player1][$i]."','$player2')";
      $result2=mysql_query($sql2);
   }
}
...
if($class=="Z") $rounds=4; //ALL BOYS CLASSES & GIRLS - NO DISTRICTS (AS OF 10/7/11)
else $rounds=5; 
$height=60;
if(ereg("doubles",$division))
{
...
$pdfcells=array();
if($class=="Z") $lines=35; //ALL BOYS CLASSES & GIRLS CLASS A - NO DISTRICTS (AS OF 10/7/11)
else $lines=64;
for($i=0;$i<$lines;$i++)
{
   for($j=0;$j<=$rounds;$j++)
...
for($r=1;$r<=$rounds;$r++)
{
   if($r==1) { $width=$width1; $widthpdf=$width1pdf; }	//ROUND 1 IS WIDER TO ACCOMODATE <select> BOXES
   else { $width=$width2; $widthpdf=$width2pdf; }

   $games=pow(2,($rounds-($r-1)))/2;	//# OF GAMES IN EACH ROUND = 2 to the power of (TOTAL ROUNDS - (CURRENT RND - 1)), DIVIDED BY 2

   if($r==1) { $curleft=$left; $curleftpdf=$left; }	//START ALL THE WAY TO THE LEFT FOR ROUND 1, ADD PADDING FOR OTHER ROUNDS
   else { $curleft=$width1+(($r-2)*$width2)+$left; $curleftpdf=$width1pdf+(($r-2)*$width2pdf)+$left; }
   ...
   {	
      ...
      $opp1line=(($g+1)*2)-1; $opp2line=($g+1)*2; $oppleft=$curleft+5;
	 ...
	 if(mysql_num_rows($result2)==0)
	 {
	    if($pdf!=1) echo ", School W-L"; $pdf1.=", School W-L";
	 }
	 else
	 {
...
//echo $sql."<br>";
	 ...
	 echo "&nbsp;<input type=radio name=\"winner[$r][$g]\" id=\"winner".$r.$opp1line."\" value=\"$winnerid\" onClick=\"TEBrackets.UpdateGame($r,$opp1line);\"";
         ...
         if($pdf!=1) echo "><font style=\"font-size:8pt;\">WINNER</font></div>";
      }
      ...
      //NOW SHOW OPPONENT 2
      if($pdf!=1) echo "</td></tr><tr align=center valign=top><td><div id=\"querystatus".$r.$g."\" style=\"display:none;\"></div></td></tr></table></div>";
	 ...
	 if(mysql_num_rows($result2)==0) { if($pdf!=1) echo "First Last"; $pdf2.="First Last"; }
      ...
      else if($r==1)      //SELECT BYE OR PLAYERS
      {
	 ...
	 {            
      ...
      $winnerid=$row[player1];         
      if(ereg("doubles",$division)) $winnerid.=";".$row[player2];         
      if($pdf!=1) echo "&nbsp;<input type=radio name=\"winner[$r][$g]\" id=\"winner".$r.$opp2line."\" value=\"$winnerid\" onClick=\"TEBrackets.UpdateGame($r,$opp2line);\"";
      if($row[winner]=='x') if($pdf!=1) echo " checked";
      if($row[bye]=='x') if($pdf!=1) echo " disabled";
      if($pdf!=1) echo "><font style=\"font-size:8pt;\">WINNER</font>";      
      $ix++; 
      if($pdf!=1) echo "</div>"; 
	 ...
	 $pdf1="";
	    ...
	    $pdf1.=$row[abbreviation];
         ...
         $winnerid=$row[player1];      
         if(ereg("doubles",$division)) $winnerid.=";".$row[player2];      
	 if($pdf!=1) echo "&nbsp;<input type=radio name=\"winner[$r][1]\" id=\"winner".$r."3\" value=\"$winnerid\" onClick=\"TEBrackets.UpdateGame($r,3);\"";
         if($row[winner]=='x') if($pdf!=1) echo " checked";      
         if($pdf!=1) echo "><font style=\"font-size:8pt;\">WINNER</font>";      
	 if($pdf!=1) echo "</div>";  
         $ix++;      
	 ...
	 $pdfcells[$pdfline2][$r2]=$pdf2;
         ...
         $winnerid=$row[player1];      
         if(ereg("doubles",$division)) $winnerid.=";".$row[player2];      
         if($pdf!=1) echo "&nbsp;<input type=radio name=\"winner[$r][1]\" id=\"winner".$r."4\" value=\"$winnerid\" onClick=\"TEBrackets.UpdateGame($r,4);\"";
         if($row[winner]=='x') if($pdf!=1) echo " checked";      
         if($pdf!=1) echo "><font style=\"font-size:8pt;\">WINNER</font>";           
         $ix++;         
         if($pdf!=1) echo "</div>"; 
...
for($i=0;$i<$lines;$i++)	//FOR EACH LINE
{
   ...
   for($j=0;$j<=$rounds;$j++)	//FOR EACH ROUND
   {
      $x=$pdf->getX();
      $y=$pdf->getY();
      if($j==0 && ereg("doubles",$division)) $colwidth=70;	//1st Round, Doubles (widest column)
      else if($j==0) $colwidth=50;				//1st Round, Singles (2nd widest)
