...
echo "<br><br><table width='600px' cellspacing=0 cellpadding=4><caption><b>Boys Tennis District Results</b><br><i>Please choose from the following.</i></caption>";
echo "<tr align=left><td><ul>";
echo "<li><a target=\"_blank\" href=\"https://nsaahome.org/teb.php\">Preview Boys Tennis Page</a></li><br>";
...
$sql="SELECT * FROM $db_name2.tebdistricts WHERE type='District' ORDER BY class,district";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result))
{
   echo "<li><b>District $row[class]-$row[district]:</b> [<a class=small href=\"te_bdistresults.php?session=$session&distid=$row[id]\">Edit</a>]&nbsp;[<a class=small href=\"../../tebdistresults.php?distid=$row[id]\" target=\"_blank\">Preview</a>]";
