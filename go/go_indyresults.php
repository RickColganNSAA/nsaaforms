...
if ($save && $sid &&isset($tournid))    //SAVE SCORES
{
    var_dump($_REQUEST);
    $sql = "DELETE FROM $indytbl WHERE tournid='$tournid' AND sid='$sid'";
    $result = mysql_query($sql);
...
//    var_dump($_REQUEST);
    ...
    echo $sql="SELECT * FROM $sport"."sched"." WHERE tid=$tournid AND sid=$sid AND oppid=$oppid AND homeid=$homeid AND scoreid=$scoreid";
    $result=mysql_query($sql);

    if (mysql_num_rows($result)==1){
        echo  $sql="UPDATE $sport"."sched"." SET sidscore=$teamscore WHERE tid=$tournid AND sid=$sid AND oppid=$oppid AND homeid=$homeid AND scoreid=$scoreid";
        mysql_query($sql);
    }else{
        echo $sql="UPDATE $sport"."sched"." SET oppscore=$teamscore WHERE tid=$tournid AND sid=$oppid AND oppid=$sid AND homeid=$homeid AND scoreid=$scoreid";
        mysql_query($sql);
    }
    ...
    <script language='javascript'>
        ...
        if ($sport == "gob") $sport = "go_b";
        if ($sport == "gog") $sport = "go_g";
        $sql = "SELECT school FROM $sport"."school WHERE sid=$reportsid";
        $result = mysql_query($sql);
        $row = mysql_fetch_array($result);
        $school = $row['school'];
        ?>
        window.opener.location.href = "results_main.php?session=<?php echo $session;?>&school_ch=<?php echo $school;?>&sport=<?php echo $sport;?>";
        window.close();
    </script>
    <?php
}
...
echo "<form method=post action=\"go_indyresults.php\">";
echo "<input type=hidden name=\"tournid\" value=\"$tournid\">";
echo "<input type=hidden name=\"reportsid\" value=\"$reportsid\">";
echo "<input type=hidden name=\"sid\" value=\"$sid\">";
echo "<input type=hidden name=\"oppid\" value=\"$oppid\">";
echo "<input type=hidden name=\"homeid\" value=\"$homeid\">";
...
echo $sql = "SELECT * FROM $schooltbl  ORDER BY outofstate,school";
$result = mysql_query($sql);
