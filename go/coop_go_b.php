<?php
//coop_go_b.php: pop-up window to add coop-students to district entry
...
for($i=0;$i<8;$i++)
{
   if($check[$i]=='y' && $coop_school[$i]!="Choose School" && $coop_student[$i]!="Choose Student")
   {
      ...
      $sql="SELECT * FROM go_b WHERE student_id='$coop_student[$i]' AND co_op='$school2'";
      $result=mysql_query($sql);
      $coop_school[$i]=ereg_replace("\'","\'",$coop_school[$i]);
      if(mysql_num_rows($result)==0)
      {
         $sql="INSERT INTO go_b (student_id, school, checked, class_dist, co_op) VALUES ('$coop_student[$i]','$coop_school[$i]','y','$class_dist','$school2')";
         $result=mysql_query($sql);
      }
   }
}
...
<body>
<center>
<form name=form1 method="post" action="coop_go_b.php">
...
for($i=0;$i<8;$i++)
{
   ...
   if($coop_school[$i] && $coop_school[$i]!="Choose School")
   {
      ...
      $sql="SELECT * FROM eligibility WHERE go='x' AND school='$coop_school[$i]'";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
