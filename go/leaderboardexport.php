...
array_multisort($schs,SORT_STRING,SORT_ASC,$studs);

$csv.="\"Name\",\"Grade\",\"School\"\r\n";
for($i=0;$i<count($studs);$i++)
{
   ...
   $grade=GetYear($row[0]);
   $csv.="\"".GetStudentInfo($studs[$i],FALSE)."\",\"$grade\",\"$schs[$i]\"\r\n";
}

$filename=strtoupper($sport2)."Class".$class."Leaderboard.csv";
