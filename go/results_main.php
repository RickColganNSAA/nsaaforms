...
if ($level == 1) $school = $school_ch;
if ($level == 1) $sid = GetSID2($school, $sport_abb);
...
if ($removetournid && ($sid || $level == 1)) {
    ...
    $result = mysql_query($sql);
    echo "<div class=\"alert\"><p>" . GetSchoolName($sid, $sport) . " has been removed from $row[tournname].</p></div>";
}
...
if ($level == 1) {
    ...
    echo "<p ><a href=\"indyranings.php?sport=$sport\" target=\"_blank\">" . GetActivityName($sport) . " Individual Differential</a></p>";
...
$sql = "SELECT DISTINCT t1.sid FROM $teamtbl AS t1,$tourntbl AS t2 WHERE t1.tournid=t2.id AND t2.datesub>0";
$result = mysql_query($sql);
$sids = array();
$diffavgs = array();
$recentupdates = array();
...
while ($row = mysql_fetch_array($result)) {
    ...
    $difftotal = 0;
    $ct = 0;
    $sql2 = "SELECT DISTINCT t1.*,t2.sid,t2.score FROM $tourntbl AS t1,$teamtbl AS t2 WHERE t1.id=t2.tournid AND t1.datesub>0 AND t2.sid='$sid' AND t1.noscores!='x' AND t1.norating!='x'";
...
if ($level == 10) {
    ...
    */
    ...
    if (mysql_num_rows($result) == 0 || $row[lastupdate] == 0)
        ...
        echo "<p>The team rankings were last updated on the public website on <b><u>" . date("F j, Y", $row[lastupdate]) . " at " . date("g:ia T", $row[lastupdate]) . "</u></b>.</p><ul><li><a href=\"teamrankings.php?sport=$sport\" target=\"_blank\">View Team Rankings on Public Website</a></li></ul>";
    echo "<p><i>Click the button below to publish the latest team rankings to the NSAA website.</i></p><input type=\"submit\" name=\"publish\" value=\"Publish Updated Team Rankings to the Website\"></div>";
    echo "</form></caption>";
...
?>
<script>
    function indyScore(tournid, sid, oppid, homeid, scoreid) {
        // alert(sid+" "+oppid);
        window.open('go_indyresults.php?reportsid=<?php echo $sid;?>' + '&sid=' + sid + '&homeid=' + homeid + '&scoreid=' + scoreid + '&session=<?php echo $session?>&tournid=' + tournid + '&oppid=' + oppid + '&sport=<?php echo $_REQUEST['sport']?>', 'Individual_Results', 'width=550,height=500,location=no');
    }
</script>
