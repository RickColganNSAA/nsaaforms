...
//submit_go_b.php: submits form info for boys golf.
//  Form is submitted from edit_go_b.php
...
$asst=ereg_replace("\"","\'",$asst);
$sql="UPDATE logins SET asst_coaches='$asst' WHERE level=3 AND sport='Boys Golf' AND school='$school2'";
...
for($i=0;$i<count($id);$i++)
{
     ...
     $sql="SELECT student_id FROM go_b WHERE (school='$school2' OR co_op='$school2')";
     $result=mysql_query($sql);
     $update=0;
     while($row=mysql_fetch_array($result))
     {
	if($row[0]==$id[$i]) $update=1;
     }
     
     //if student has been submitted before, use UPDATE or DELETE:
     if($update==1)
     {
	$sql="UPDATE go_b SET checked='$check[$i]', avg_round='$avg_round[$i]', class_dist='$class_dist' WHERE student_id='$id[$i]'";
     }

     //if student has not been submitted yet, use INSERT:
     if($update==0)
     {
        $sql="INSERT INTO go_b (student_id, school, checked, avg_round, class_dist";
...
{
   $sql="UPDATE go_b SET checked='$coop_check[$i]', avg_round='$coop_avg_round[$i]', class_dist='$class_dist' WHERE student_id='$coop_student[$i]'";
   $result=mysql_query($sql);
}
if($submit=="Save and View Form")
{
   header("Location:view_go_b.php?session=$session&school_ch=$school_ch");
   exit();
}
