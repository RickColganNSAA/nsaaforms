<?php
//view_go_b.php: Show submitted district
//   entry info.  If none have been submitted,
//   redirect to edit_go_b.php
...
else if($director==1)
{
   ...
   $hostid=$row[0];
   $sql="SELECT * FROM $db_name2.go_bdistricts WHERE hostid='$hostid'";
   $result=mysql_query($sql);
...
//get due date for boys golf form:
...
$sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM go_b AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND ((t1.school='$school2' AND t2.school='$school2') OR (t1.co_op='$school2' AND t2.school=t1.school)) AND t1.checked='y' ORDER BY t2.last";
$result=mysql_query($sql); 
  //if it hasn't been submitted, redirect to Edit page:
if(mysql_num_rows($result)==0)
{
   if($director!=1)
      header("Location:edit_go_b.php?session=$session&school_ch=$school_ch");
   else
...
$sql2="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Boys Golf'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
...
if($print!=1)
{
   echo "<br><a class=small href=\"view_go_b.php?session=$session&print=1&school_ch=$school_ch\" target=_\"blank\">Printer-Friendly Version</a>";
   echo "&nbsp;&nbsp;&nbsp;<a class=small href=\"edit_go_b.php?school_ch=$school_ch&session=$session\">Edit this Form</a>";
}
$info.="<form method=post action=\"edit_go_b.php\">";
...
$info.="<th>BOYS GOLF DISTRICT ENTRY</th>";
$info.="</tr>";
$info.="<tr align=center>";
...
$sql2="SELECT * FROM go_bschool WHERE sid='$sid'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
if($row2[mascot]!='') $mascot=$row2[mascot];
if($row2[colors]!='') $colors=$row2[colors];
if($row2[coach]!='') $coach=$row2[coach];
$info.="<td>".GetSchoolName($sid,'go_b')." $mascot</td></tr>";
$csv.="School/Mascot:,".GetSchoolName($sid,'go_b')." $mascot\r\n";
...
$info.="<td>$class_dist</td></tr></table>";
...
<td><br>
<?php
echo "<a class=small target=new href=\"view_go_b.php?session=$session&school_ch=$school_ch&print=1\">Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
echo "<a class=small href=\"edit_go_b.php?session=$session&school_ch=$school_ch\">Edit this Form</a>&nbsp;&nbsp;&nbsp;";
