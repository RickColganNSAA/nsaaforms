<?php
//hostapp_te_b.php: site survey for boys tennis
...
if($submitapp=="Submit Application")
{
   ...
   */
   ...
   $sql="SELECT * FROM hostapp_te_b WHERE school='$school2'";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0)	//INSERT
   {
      $sql2="INSERT INTO hostapp_te_b (school,interested,indoorcourts,outdoorcourts,indoor,outdoor,facility,location,director,comments) VALUES ('$school2','$interested','$indoorcourts','$outdoorcourts','$indoor','$outdoor','$facility','$location','$director','$comments')";
   }
   else					//UPDATE
   {
      $sql2="UPDATE hostapp_te_b SET interested='$interested', indoorcourts='$indoorcourts', outdoorcourts='$outdoorcourts', indoor='$indoor', outdoor='$outdoor', facility='$facility', location='$location', director='$director',comments='$comments' WHERE school='$school2'";
   }
...
$sql="SELECT duedate FROM app_duedates WHERE sport='te_b'";
$result=mysql_query($sql);
...
{
   $sql2="INSERT INTO hostapp_te_b (school) VALUES ('$school2')";
   $result2=mysql_query($sql2);
   $result=mysql_query($sql);
}
while($row=mysql_fetch_array($result))
{
   ...
   echo "<h3>Application to Host a $fallyear BOYS TENNIS District/Sub-District Event</h3>";

   if($print!=1) echo "<p>Due $duedate2</p>";
echo "<form method=post action=\"hostapp_te_b.php\">";
...
{
   echo "<font style=\"color:red\" size=2><b><i>Your application to host Boys Tennis District/Sub-District Events has been submitted.  Below is what you have submitted.  You may make changes to this application until the above due date.</i></b></font>";
}
...
if(PastDue($duedate) && $level!=1)
{
   ...
   {
      echo "<tr align=center><td><table><tr align=left><td><b>You did NOT submit an Application to Host Boys Tennis District/Subdistrict Event.  The due date for this application is past.</b></td></tr></table></td></tr>";
      echo "</table>";
      echo $end_html;
      exit();
   }  
...
echo "<tr align=left><th align=left>1) Are you interested in hosting any NSAA district contests for Boys Tennis on $te_bhostdates[0]?</th></tr>";
