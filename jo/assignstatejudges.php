...
if ($_POST['save']) {
    for ($i = 0; $i < count($judgeid); $i++) {
        $sql = "SELECT * FROM jostateassignments WHERE catid='$catid[$i]' AND class='$class[$i]'";
        $result = mysql_query($sql);
        if (mysql_num_rows($result) == 0)
            $sql = "INSERT INTO jostateassignments (judgeid,catid,class) VALUES ('$judgeid[$i]','$catid[$i]','$class[$i]')";
        else
            $sql = "UPDATE jostateassignments SET judgeid='$judgeid[$i]' WHERE catid='$catid[$i]' AND class='$class[$i]'";
        $result = mysql_query($sql);
    }

    header("Location:assignstatejudges.php?session=$session&saved=1");
}

echo $init_html;
echo $header;

echo "<form method='post' action='assignstatejudges.php'>";
echo "<input type=hidden name=\"session\" value=\"$session\">";
echo "<br><a href=\"statejudges.php?session=$session\">Return to Journalism Judges</a>";
echo "<br><h2>Assign State Journalism Judges:</h2>";
...
while ($row = mysql_fetch_array($result)) {
    echo "<tr align=left>";
    for ($c = 0; $c < count($classes); $c++) {
        $sql2 = "SELECT * FROM jostateassignments WHERE catid='$row[id]' AND class='$classes[$c]'";
        ...
        //echo "<td>$classes[$c]<input type=hidden name=\"class[$ix]\" value=\"$classes[$c]\"><input type=hidden name=\"catid[$ix]\" value=\"$row[id]\"></td><td><select name=\"judgeid[$ix]\"><option value=\"0\">Select Judge</option>";
