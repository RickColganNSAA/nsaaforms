...
if ($_POST['save'] && $catid)    //Saved the screen with entries for an event
{
    $usedstudids = array();
    $s = 0;
    $saveerrors = "";
    for ($i = 1; $i <= count($entryid); $i++) {
        ...
        if (!in_array($curstudlist, $usedstudids) || $curstudlist == "") {
            ...
            if (!empty($_FILES["docfile"]["name"][$i])) {
                ...
                $sql = "UPDATE joentries SET filename='$image' WHERE id='$entryid[$i]' AND sid='$sid' AND catid='$catid'";
                $result = mysql_query($sql);
            }
            if (!empty($_FILES["docfile2"]["name"][$i])) {
                ...
                $image = mysql_real_escape_string($image);
                $sql = "UPDATE joentries SET filename2='$image' WHERE id='$entryid[$i]' AND sid='$sid' AND catid='$catid'";
                $result = mysql_query($sql);
            }
            //code by robin
            if (!empty($_FILES["docfile3"]["name"][$i])) {
                $image = rand(0, 10000) . $_FILES["docfile3"]["name"][$i];
                $target_dir = $_SERVER['DOCUMENT_ROOT'] . "/nsaaforms/downloads/";
                $target_file = $target_dir . basename($image);
                $refFileType = pathinfo($target_file, PATHINFO_EXTENSION);
                citgf_moveuploadedfile($_FILES["docfile3"]["tmp_name"][$i], $target_file);
                $image = mysql_real_escape_string($image);
                $sql = "UPDATE joentries SET filename3='$image' WHERE id='$entryid[$i]' AND sid='$sid' AND catid='$catid'";
                $result = mysql_query($sql);
            }
...
echo "<br>";
if ($level == 1)
    echo "<p><a href=\"stateentries.php?session=$session\">Return to JO Contest Entry Submissions</a></p>";

echo "<form method='post' action='stateentry.php' enctype='multipart/form-data'>";
echo "<input type=hidden name=\"session\" value=\"$session\">";
echo "<input type=hidden name=\"sid\" value=\"$sid\">";
echo "<h2>State Journalism Contest Entry Submission for <u>" . GetSchoolName($sid, 'jo') . "</u>:</h2>";
//echo "<br/><button name='reset' type='submit' id='clear' >Clear</button>";
...
$sql = "SELECT * FROM joentries WHERE sid='$sid'";
$result = mysql_query($sql);
...
if ($catid) {
    ...
    if ($editable) {
        ...
        echo "</tr>";
        $ix = 1;
        $sql = "SELECT * FROM joentries WHERE sid='$sid' AND catid='$catid'";
        $result = mysql_query($sql);
        while ($row = mysql_fetch_array($result)) {
            echo "<tr align=center";
            if ($ix == 2) echo " bgcolor='#f0f0f0'";
            echo "><td rowspan=\"$maxfiles\"><b>$ix</b></td><td ><input type=hidden name=\"entryid[$ix]\" value=\"$row[id]\">";
            for ($j = 1; $j <= $max; $j++) {
                $studvar = "studentid";
                if ($j > 1) $studvar .= $j;
            ...
            if ($maxfiles > 1) {
                ...
                if ($ix == 2) echo " bgcolor='#f0f0f0'";
                echo "><td></td><td><input type=text size=40 name=\"label2[$ix]\" value=\"$row[label2]\"></td>";
                echo "<td>";
                if (preg_match('/http/', $row[filename2])) {
                    ...
                    if ($row[filename2] != '')   //FILE UPLOADED ALREADY
                    {
                ...
                if ($maxfiles==3){
                    echo "<tr align=center";
                    if ($ix == 2) echo " bgcolor='#f0f0f0'";
                    echo "><td></td><td><input type=text size=40 name=\"label3[$ix]\" value=\"$row[label3]\"></td>";
                    echo "<td>";
                    if (preg_match('/http/', $row[filename3])) {
                        echo "<p style=\"text-align:left;\">You've uploaded <a href=\"$row[filename]\" target=\"_blank\">THIS FILE</a>.<br>";
                        echo "To <b>replace</b> this file, upload a new one below:</p>";
                    } else {
                        //if(citgf_file_exists($_SERVER['DOCUMENT_ROOT']."/nsaaforms/downloads/".$row[filename2]) && $row[filename2]!='')   //FILE UPLOADED ALREADY
                        if ($row[filename3] != '')   //FILE UPLOADED ALREADY
                        {
                            echo "<p style=\"text-align:left;\">You've uploaded <a href=\"/nsaaforms/downloads/$row[filename3]\" target=\"_blank\">THIS FILE</a>.<br>";
                            echo "To <b>replace</b> this file, upload a new one below:</p>";
                        } else
                            echo "<p style=\"text-align:left;\">No file uploaded yet. Click \"Choose File\" to find your file and then click \"Save Entries\" below:</p>";
                    }
                    ...
                    echo "<table><tr align=left><th align=left>Find Your File:</th>";
                    echo "<td align=left><input type=file name=docfile3[$ix]></td></tr>";
                    echo "<tr valign=top align=left><th align=left>OR Type in the Link URL:</th>";
                    echo "<td><input type=text class=tiny size=70 name=linkurl3[$ix] ";
                    if (preg_match('/http/', $row[filename3]))
                        echo "value=\"$row[filename3]\"><br>";
                    else
                        echo "><br>";
                    echo "(Example:https://nsaa-static.s3.amazonaws.com/../bb_3.pdf)</td></tr>";
                    echo "<!--<tr align=left><td></td><td><input type=submit name=submit value=\"Upload\"></td></tr>--></table>";
        ...
        while ($ix <= $maxentries) {
            $sql = "INSERT INTO joentries (sid,catid) VALUES ('$sid','$catid')";
            $result = mysql_query($sql);
    ...
    else    //SHOW WHAT WAS SUBMITTED, INCLUDING JUDGE'S COMMENTS
    {
        ...
        $sql = "SELECT t1.* FROM joentries AS t1, joassignments AS t2 WHERE t1.catid=t2.catid AND t1.sid='$sid' AND t1.catid='$catid' AND t2.datesub>0";
        $result = mysql_query($sql);
        $ix = 1;
        while ($row = mysql_fetch_array($result)) {
            ...
            $sql2 = "SELECT * FROM joqualifiers WHERE showtopublic='x'";
...
{
    $sql = "SELECT DISTINCT t1.catid,t2.category FROM joentries AS t1,jocategories AS t2 WHERE t1.catid=t2.id AND t1.sid='$sid' AND t1.studentid>0";
    $sql = "SELECT * FROM jocategories order by category";
    $result = mysql_query($sql);
    ...
    } else        //SOME ENTRIES - SHOW THEM
    {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            if (!PastDue($duedate, 0) || $level == 1)
                echo "<a class=small href=\"stateentry.php?session=$session&sid=$sid&catid=$row[id]\">Edit Submissions</a></h3>";
            $curct = 1;
            $sql2 = "SELECT * FROM joentries WHERE catid='$row[id]' AND sid='$sid' AND studentid>0";
            $result2 = mysql_query($sql2);
...
//code by robin to reset
if (isset($_POST['reset'])) {
    if (isset($catid)) {
        $sql = "SELECT filename,filename2 FROM  joentries WHERE catid=$catid";
    } else {
        $sql = "SELECT filename,filename2 FROM  joentries";
    }
    $result = mysql_query($sql);
