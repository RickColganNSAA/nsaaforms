...
if($savedates)
{
   ...
   $sql="UPDATE jostatesettings SET judgeduedate='$judge'";
   $result=mysql_query($sql);
}
...
echo "<br><h2>Manage Due Date</h2>";
echo "<form method=post action=\"duedate.php#settings\">";
echo "<input type=hidden name=\"session\" value=\"$session\">";
echo "<a href=\"statejudges.php?session=$session\">Return State Journalism JUDGES</a></br>";
...
echo "<li><a href=\"previewresults.php?session=$session\">Review & Approve Results for NSAA WEBSITE</a><p>Review information that will appear on the public NSAA website; approve for publishing.</p></li>";
...
if($ct>0)
   echo "<p><a class=\"small\" href=\"stateadmin.php?session=$session&deletetable=joqualifiers\" onClick=\"return confirm('Are you sure you wish to remove the state qualifiers from the database?');\">Remove all state qualifiers</a> <i>(this does not remove the entries or their rankings, just their designation as a state qualifier.)</i></p>";
	...
	//DATES:
echo "<br><p><b>DATE:</b></p>";
if($savedates)
   echo "<div class='alert'>The date has been saved.</div>";
...
//echo "<p>Due Date for Schools to Submit Preliminary Entries: <select name=\"prelimm\">".GetDateSelectOptions("MM",$date[1])."</select>/<select name=\"prelimd\">".GetDateSelectOptions("DD",$date[2])."</select>/<select name=\"prelimy\">".GetDateSelectOptions("YYYY",$date[0])."</select></p>";
	//Judges Due Date:
$date=explode('-',GetJOStateSetting('judgeduedate'));
