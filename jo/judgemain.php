...
if($judgeid) $level=2;
$assignids=explode(",",GetJOJudgeAssignment($judgeid));
if(!$assignid && count($assignids)==1) $assignid=$assignids[0];
$catids=array(); $classes=array();
for($i=0;$i<count($assignids);$i++)
{
   $sql="SELECT catid,class FROM joassignments WHERE id='$assignids[$i]'";
...
{
   $sql="UPDATE joassignments SET datesub='".time()."' WHERE judgeid='$judgeid' AND id='$assignid'";
   $result=mysql_query($sql);
}
if($assignid)   //CHECK IF SUBMITTED
{
   $sql3="SELECT * FROM joassignments WHERE judgeid='$judgeid' AND id='$assignid'";
...
echo "<form method=post action=\"judgemain.php#rankingstop\">";
echo "<input type=hidden name=\"session\" value=\"$session\">";

$duedate=GetJOSetting('judgeduedate');
...
if(count($assignids)>1)	//SELECT FROM CATEGORIES
{
   ...
   for($i=0;$i<count($assignids);$i++)
   {
      $assigns.="<u>".GetJOCategory($catids[$i])." (Class $classes[$i])</u> and ";
   }
   ...
   for($i=0;$i<count($assignids);$i++)
   {
      ...
      echo ">".GetJOCategory($catids[$i])." - Class $classes[$i]</option>";
   }
   echo "</select></p></div><br>";
   if($assignid)
      echo "<h3>You are judging <u>".GetJOCategory($catid)." (Class $class)</u>.</h3>";
}
else if(count($assignids)==1)
{
   echo "<h3>You have been assigned to judge <u>".GetJOCategory($catid)." - Class $class</u>. Judging must be completed no later than <u>$duedate2</u>.</h3>";
}
else //NO ASSIGNMENTS
{
...
if($assignid)
{
   ...
   echo "<h3 style='text-align:center;'>$row[header]</h3><p><b>Criteria for judging:</b> $row[description]</p></div>";
	...
	//RANKED YET?
   $sql2="SELECT t1.* FROM joentries AS t1,eligibility AS t2,joschool AS t3 WHERE t1.sid=t3.sid AND t1.studentid=t2.id AND t1.catid='$catid' AND t3.class='$class' AND classrank>0";
   $result2=mysql_query($sql2);
   if(mysql_num_rows($result2)>0) $ranked=1;
   else $ranked=0;
	//ANY ENTRIES YET?
   $sql="SELECT t1.* FROM joentries AS t1,eligibility AS t2,joschool AS t3 WHERE t1.sid=t3.sid AND t1.studentid=t2.id AND t1.catid='$catid' AND t3.class='$class' ORDER BY classrank ASC";
   $result=mysql_query($sql);
   ...
   if($ranked==0)
   {
      echo str_replace('CLASS',"CLASS $class ".GetJOCategory($catid),$row3['un_submitted_instruction']);
      ...
      //echo "<li>Please arrange the submissions so that the <b>TOP <span style=\"background-color:yellow;font-size:125%;\"><u>15</u></span> ENTRIES</b> are positioned at the top, with <b>1st Place</b> in the upper left corner, <b>2nd Place</b> immediately to the right, followed by <b>3rd and 4th place</b>. Then move <b>5th Place</b> to the first position in the second row, <b>6th place</b> to the right of 5th, and so on until you have ranked the top 15 (or less if there are not 15 entries in your event). Please note that your 13th, 14th and 15th-ranked entries will be used in the event that ALTERNATES are needed ONLY.</li>";
   }
   else
   {
      echo str_replace('CLASS',"CLASS $class ".GetJOCategory($catid),$row3['submitted_instruction']);
      //echo "<ul><li>Below are the entry submissions for <b>CLASS $class ".GetJOCategory($catid)."</b>, with the <b>Top 15</b> showing first, as you have ranked them.</li>";
    ...
    echo str_replace('CLASS',"CLASS $class ".GetJOCategory($catid),$row3['general_instruction']);
   //echo "<li>You can move a submission by clicking and dragging it to its desired location.</li><li>Click \"Save Order\" to save your rankings.</li>
	...
	//<li><b>Once you are satisfied with your rankings,</b> you will need to check the box indicating you are ready to submit final rankings to the NSAA.</li></ul>";
      //echo "<p style='text-align:center;'>Arrange your Top 15 entries as shown below, with the rest of the entries following in an order that does not matter:<br>";
	...
	//IF $ranked, CAN SUBMIT TO NSAA:
      if($ranked==1)    //THEY CAN SUBMIT WHEN THEY ARE READY
      {
            ...
            if($submitted==0)
            {
               echo "<p>Once you are satisfied with your Top 15 rankings in this event, you may check the box below to submit them to the NSAA.</p><h3>";
