...
if($_POST['login'])
{
   $session=LoginJOJudge($email,$password);
   if(!$session)
      $error=1;
   else
   {
      header("Location:judgemain.php?session=$session");
      exit();
   }
} 
...
echo GetMainHeader();
?>
<form method="post" action="http://nsaahome.criticalitgroup.com/nsaaforms/jo/index.php">
<br>
