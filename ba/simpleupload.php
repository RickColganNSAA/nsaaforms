...
{	
	...
	if ( !empty($_FILES) )
	{
		...
		$uniqueFileName = sprintf("%s/ba_teamphoto_%s_1.%s",$pathinfo['dirname'],$sid,$pathinfo['extension']); //$fileName;
		for($k = 1; $k < 100; $k++)
		{
			...
			else
			{
				...
				$uniqueFileName = sprintf("%s/ba_teamphoto_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
                                $uniqueThumbName = sprintf("%s/ba_teamphoto_resized_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
			}
...
/*
	...
	*/
			...
			$sql="UPDATE baschool SET filename='$useFileName' WHERE sid='$sid'";
