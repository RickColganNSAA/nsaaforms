<?php
//coop_ba.php: pop-up window to add coop-students to district entry
...
<body>
<form name=form1 method="post" action="coop_ba.php">
...
for($i=0;$i<8;$i++)
{
   ...
   if($coop_school[$i] && $coop_school[$i]!="Choose School")
   {
      ...
      $sql="SELECT * FROM eligibility WHERE ba='x' AND school='$coop_school[$i]'";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
