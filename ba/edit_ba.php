...
$max_roster_size = 22;  //constant for maximum roster size
$mycount = 0;  //counts the players that are checked on the roster
...
<form method=post action="submit_ba.php" enctype="multipart/form-data">
            ...
            <td>
                <table cellspacing=0 cellpadding=2 style="width:600px">
                    ...
                    <tr align=left>
                        ...
                        <td>
                            <?php if ($filename != ''): ?>
                                <p><a href="/nsaaforms/downloads/<?php echo $filename; ?>" target="_blank">Preview Team
                                        Photo</a></p>
                            <?php endif; ?>
                    ...
                    ?>
                    <tr align=left>
                        <td colspan=2><p>Check the box next to the name of each student who will be participating in
                                the <?php echo $form_type; ?> competition. Required fields for the district tournament
                                are marked with a *. <b>All fields are required for the state entry form.</b> When your
                                team qualifies for the state tournament, you must update this form with complete
                                statistics on each player by 10 a.m. the morning after your team has qualified. Players
                                may be substituted on this team roster prior to district and between district and state
                                at a school's discretion, provided no more than <?php echo $max_roster_size; ?> players are in uniform.</p></td>
                    </tr>
                    <?php
                        if ($mycount > $max_roster_size) {
                            echo "<tr align=center style=color: red;> <strong>ERROR! More than .$max_roster_size. players on this roster</strong>";
                        }
                    ?>
                </table>
            </td>
   ...
   <tr bgcolor='#f0f0f0' align=center>
   	...
   	<th class=smaller>Dark</th>";
                    ...
                    <tr align=left>
                        <td><br><font size=2><i>(Substitutions may be made the day of the district meet and, if team
                                    qualifies for state, for the start of the state meet.)</i><br>
                                <font style="color:red">Students listed in red are currently <b>ineligible</b>. Please
                                    make sure they will be eligible for the <?php echo $form_type; ?> tournament before
                                    submitting them on this form.</font></p></font></td>
                    </tr>
                    <?php
                    ...
                    <!-- The following are Rick's rows -->
                    <tr align=center>
                        <?php
                        $checked_arr = $_GET['check'];
                        ...
                        <td>
                            ...
                            //$mycount = 0;
                            //for ($i = 0; $i < count($cur_id); $i++) {
                            //    if ($cur_checked[$i] == "y") $mycount++;
                            //}

                            //echo "There are " . count($cur_id) . " players on this roster <br>";
                            echo "There are " . $mycount . " checked players on this roster <br>";
                            if ($mycount > $max_roster_size) {
                                echo "ERROR: Roster exceeds .$max_roster_size. player limit!";
                            }
                            ?>
                        </td>
                    </tr>
