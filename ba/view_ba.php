...
if ($makepdf)    //GET OTHER SCHOOL TO GO ON THIS PAGE
{
    ...
    $row = mysql_fetch_array($result);
    $sid2 = $row[sid];
    $approved2 = $row[approvedforprogram];
...
$info .= "</table></td></tr>";
// flag an illegal roster
if ($mycount > $max_roster_size) {
    echo '<span style="color: red; font-size: large;"><b>Roster exceeds maximum roster size</b></span><br>';
}
...
if ($print != 1 && !$makepdf) {
    ...
    <tr align=center>
        <td><br>
            <a href="view_ba.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>&print=1" target=new
               class=small>Printer/E-mail Friendly Version</a>
            &nbsp;&nbsp;&nbsp;
            <a href="edit_ba.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>" class=small>Edit
                this Form</a>
            &nbsp;&nbsp;&nbsp;
    ...
    <tr align=center>
        <th><br><br>
            <form method=post action="../email_form.php" name=emailform>
                ...
                <table>
                    <tr align=center>
                        <td colspan=2><b>E-MAIL THIS FORM:</b><br>PLEASE NOTE: Your district director will automatically
                            receive these forms once the due date has passed. You do NOT need to email this form to the
                            district director.
                        </td>
                    </tr>
                    <tr align=left>
