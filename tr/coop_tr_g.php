<?php
//coop_tr_g.php: pop-up window: add coop students to student list on dist entry
...
for($i=0;$i<8;$i++)
{
   if($check[$i]=='y' && $coop_school[$i]!="Choose School" && $coop_student[$i]!="Choose Student")
   {
      ...
      $sql="SELECT * FROM tr_g_coop WHERE student_id='$coop_student[$i]' AND co_op='$school2'";
      $result=mysql_query($sql);
      if(mysql_num_rows($result)==0)
      {
         $sql="INSERT INTO tr_g_coop (student_id, co_op) VALUES ('$coop_student[$i]','$school2')";
         $result=mysql_query($sql);
      }
   }
}
...
<body>
<center>
<form name=form1 method="post" action="coop_tr_g.php">
...
for($i=0;$i<8;$i++)
{
   ...
   if($coop_school[$i] && $coop_school[$i]!="Choose School")
   {
      ...
      $sql="SELECT * FROM eligibility WHERE tr='x' AND gender='F' AND school='$coop_school[$i]'";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
