<?php
//view_tr_g.php: Track entry form
...
{
   if($director!=1)
      header("Location:edit_tr_g.php?session=$session&school_ch=$school_ch");
   else
...
{
   echo "<br><a href=\"edit_tr_g.php?session=$session&school_ch=$school_ch\" class=small>Edit this Form</a>&nbsp;&nbsp;&nbsp;";   
   if($level==1 || $level==2)
      echo "<a href=\"view_tr_b.php?session=$session&school_ch=$school_ch\" class=small>Go to BOYS $form_type Entry Form</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"view_tr_g.php?session=$session&school_ch=$school_ch&print=1\" class=small target=new>Printer-Friendly Version</a><br><br>";
}

$info="";
$csv="";
$info.="<table cellspacing=4 cellpadding=4><!--Table of Tables-->";
$info.="<caption><b>GIRLS TRACK & FIELD $form_type ENTRY FORM <br>";
...
if($print!=1)
{
   ...
   echo "<a href=\"teamlist_g.php?school_ch=$school_ch&session=$session&print=1\" target=new>Click Here for your Team's District Roster</a><br>";
...
for($x=0;$x<count($trevents_g);$x++)
{
   ...
   $info.="<caption align=left><b>$treventslong_g[$x]:</b></caption>";
   $info.=$colheaders;
...
if($print!=1)
{
   ...
   echo "<a href=\"edit_tr_g.php?session=$session&school_ch=$school_ch\" class=small>Edit this Form</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"view_tr_g.php?session=$session&school_ch=$school_ch&print=1\" class=small target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
...
if($print==1)
{
   ...
   $info.="<caption><b>$school Girls Track & Field District Roster as of $date:</b><br><br></caption>";
   $csv="$school Girls Track & Field District Roster:\r\n";
   $csv.=",Name,Grade\r\n";
   $info.="<tr align=center><th class=smaller colspan=2>Name</th><th class=smaller>Grade</th></tr>";

   $students=GetRoster('tr',$school,'F');
