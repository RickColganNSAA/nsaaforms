<?php
//submit_tr_g.php: submits form info for girls track.
//  Form is submitted from edit_tr_g.php
...
$asst=ereg_replace("\"","\'",$asst);
$sql="UPDATE logins SET asst_coaches='$asst' WHERE school='$school2' AND sport='Girls Track & Field'";
$result=mysql_query($sql);

$idlist="";
$o=0; $overlimit=array();
for($j=0;$j<count($trevents_g);$j++)
{
   $long=$treventslong_g[$j];
   $short=$trevents_g[$j];
...
$sql="SELECT t1.student_id FROM $table AS t1, tr_g_coop t2 WHERE t1.student_id=t2.student_id AND t2.co_op='$school2'";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result))
{
   $sql2="DELETE FROM $table WHERE student_id='$row[0]'";
...
{
   header("Location:view_tr_g.php?session=$session&school_ch=$school_ch&send=y");
   exit();
}
*/
if($submit=="Save and Keep Editing")
{
   header("Location:edit_tr_g.php?session=$session&school_ch=$school_ch&alert=$alert");
   exit();
}
else if($submit=="Save and View Form")
{
   header("Location:view_tr_g.php?session=$session&school_ch=$school_ch&alert=$alert");
   exit();
}
