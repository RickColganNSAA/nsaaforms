<?php
//tr_state_edit_g.php: Track & Field Dist Results Form (State Qualifiers)
...
$sql="SELECT * FROM $db_name2.trgdistricts WHERE id='$distid'";
...
{
   header("Location:tr_state_view_g.php?session=$session&distid=$distid");
   exit();
}
...
if(($store=="Save & Keep Editing" || $store=="Save & View Form") && $distid)
{
   ...
   for($i=0;$i<count($sch[$event]);$i++) //for each place, one row:
   {
	 ...
	 if(($class=="B" || $class=="A") && preg_match("/Meter/",$treventslong_g[$eventnum]))
	 ...
	 $sql="SELECT * FROM $db1.tr_state_relays_g WHERE district='$distid' AND place='$thisplace' AND relay='$event'";
	 $result=mysql_query($sql);
	 if($class=="A" || $class=="B")
	 {
	 ...
	 {
	    $sql2="INSERT INTO $db1.tr_state_relays_g (district,place,relay,sch,stud,perf) VALUES ('$distid','$thisplace','$event','$relay_sch','$relay_list','$cur_perf')";
	 }
	 else		//UPDATE
	 {
	    $sql2="UPDATE $db1.tr_state_relays_g SET sch='$relay_sch', stud='$relay_list',perf='$cur_perf' WHERE district='$distid' AND place='$thisplace' AND relay='$event'";
	 }
   ...
   if($event!="400r" && $event!="1600r" && $event!="3200r")
   {
      ...
      $sql="SELECT * FROM $db1.tr_state_qual_g WHERE district='$distid'";
      $result=mysql_query($sql);
      if(mysql_num_rows($result)==0)	//INSERT
      {
	 $sql2="INSERT INTO $db1.tr_state_qual_g (district,$eventsch,$eventstud,$eventperf,$eventtie,$eventplace) VALUES ('$distid','$sch_list','$stud_list','$perf_list','$tie_list','$place_list')";
      }
      else				//UPDATE
      {
	 $sql2="UPDATE $db1.tr_state_qual_g SET $eventsch='$sch_list', $eventstud='$stud_list',$eventperf='$perf_list',$eventtie='$tie_list',$eventplace='$place_list' WHERE district='$distid'";
      }
      $result2=mysql_query($sql2);
	 ...
	 $sql="SELECT * FROM $db1.tr_state_place_g WHERE district='$distid'";
	 $result=mysql_query($sql);
	 if(mysql_num_rows($result)==0)	//INSERT
	 {
	    $sql2="INSERT INTO $db1.tr_state_place_g (district,pv,hj,lj,tj,sp,d) VALUES ('$distid','$pv','$hj','$lj','$tj','$sp','$d')";
	 }
	 else
	 {
	    $sql2="UPDATE $db1.tr_state_place_g SET pv='$pv', hj='$hj', lj='$lj', tj='$tj', sp='$sp', d='$d' WHERE district='$distid'";
	 }
	 $result2=mysql_query($sql2); 
      }
   }
   ...
   if($event=="teamscores")
   {
	 ...
	 if($team[$i]!='')
      ...
      $sql0="SELECT * FROM $db1.tr_state_dist_g WHERE dist='$distid'";
      $result0=mysql_query($sql0);
      if(mysql_num_rows($result0)>0)
         $sql="UPDATE $db1.tr_state_dist_g SET teamscores='$teamstr',teams='$teams',indys='$indys' WHERE dist='$distid'";
      else
	 $sql="INSERT INTO $db1.tr_state_dist_g (dist,teamscores,teams,indys) VALUES ('$distid','$teamstr','$teams','$indys')";
      $result=mysql_query($sql);
   }
   if($event=="pv" || $event=="hj" || $event=="lj" || $event=="sp" || $event=="d" || $event=="tj")
   {
      for($i=0;$i<count($extraid);$i++)
      {
	    ...
	    $sql="SELECT * FROM $db1.tr_state_extra_g WHERE id='$extraid[$i]'";
	    $result=mysql_query($sql);
	    if(mysql_num_rows($result)==0 || $extraid=='0')//INSERT
	    {
	       $sql2="INSERT INTO $db1.tr_state_extra_g (district,eventnum,school,student_id,place,perf1,perf2) VALUES ('$distid','$extraevent[$i]','$extrasch2[$i]','$extrastud[$i]','$extraplace[$i]','$extraperf1[$i]','$extraperf2[$i]')";
	    }
	    else //UPDATE
	    {
	       $sql2="UPDATE $db1.tr_state_extra_g SET eventnum='$extraevent[$i]',school='$extrasch2[$i]',student_id='$extrastud[$i]',place='$extraplace[$i]',perf1='$extraperf1[$i]',perf2='$extraperf2[$i]' WHERE id='$extraid[$i]'";
	    }
	    $result2=mysql_query($sql2); 
	 }
	 else if($extraid[$i]!='0')
	 {
	    $sql="DELETE FROM $db1.tr_state_extra_g WHERE id='$extraid[$i]'";
	    $result=mysql_query($sql); 
   ...
   if($store=="Save & View Form" || $final=='y')
   {
      header("Location:tr_state_view_g.php?session=$session&school_ch=$school_ch&distid=$distid&final=$final");
      //exit();
   }
}//end if $store
...
else if($event=="800" || $event=="1600m" || $event=="3200m" || $event=="100h" || $event=="300" || $event=="100" || $event=="200" || $event=="400m")
   $max=$max_track;
else if($event=="3200r")
   $max=$max_relay_lg;
else
...
<body onload="Team2.initialize('<?php echo $session; ?>','tr_g','school','student','<?php echo $max; ?>');">
<?php
echo GetHeader($session);

echo "<form method=post action=\"tr_state_edit_g.php\">";
echo "<input type=hidden name=session value=\"$session\">";
echo "<input type=hidden name=school_ch value=\"$school_ch\">";
?>
<br><h2>DISTRICT RESULTS: Qualifiers for Girls State Track & Field</h2>
<?php
...
echo "<a class=small href=\"tr_state_edit_b.php?session=$session&distid=$distid\">Go to BOYS District Results</a><br>";

echo "<table width=\"700px\"><caption style=\"text-align:left;\">";
$sql="SELECT resultssub_g FROM $db2.trgdistricts WHERE class='$class' AND district='$district'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
if($row[0]!='')    
{
   echo "<br><div class='alert' style=\"width:100%;\"><p>You submitted these results on <b>".date("m/d/y",$row[0])." at ".date("g:ia",$row[0])."</b>.</p><p>To view what you have submitted, <a href=\"tr_state_view_g.php?session=$session&distid=$distid\">Click Here</a>.</p><p>If you need to make a correction, please <b>CONTACT THE NSAA</b></p></div><br>";
   if($level!=1)
   {
      echo $end_html;
      exit();
...
{
   echo "<div class=\"error\" style=\"width:100%;\"><p>You have NOT submitted these results yet.</p>";
...
{
   $tr_sch[$i]=GetSchoolName($tr_sid[$i],'trg');
}
...
echo "</caption><tr align=center><td>";
echo "<input type=hidden name=autosubmit value='1'>";
echo "<input type=hidden name=session value=\"$session\">";
echo "<input type=hidden name=school_ch value=\"$school_ch\">";
if(!$distid || $distid=="Choose District")
   unset($eventnum);
echo "<h3>Select Event or Team Scores: <select name=\"eventnum\" onchange='submit();'><option";
if($trevents_g[$eventnum]=="") echo " selected";
echo ">Choose an Event</option>";
for($i=0;$i<count($treventslong_g);$i++)
{
   echo "<option value=$i";
   if($i==$eventnum && $trevents_g[$eventnum]!="") echo " selected";
   echo ">$treventslong_g[$i]</option>";
}
echo "</select></h3></td></tr><tr align=center>";
$event=$trevents_g[$eventnum];
echo "<a name=$event href=#$event></a>";
if($event!="teamscores")
{
   echo "<td><table cellspacing=0 cellpadding=3 frame=\"all\" rules=\"all\" style=\"min-width:400px;border:#808080 1px solid;\"><caption style=\"text-align:left;\"><b>";
   echo "<h3 style=\"color:red\"><u>".$treventslong_g[$eventnum]."</u></h3>";
   if($treventslong_g[$eventnum]=="")
   {
      echo "<h3 style=\"color:red\">You have not selected an event yet.  Please choose the event you wish to enter results for above.</h3></caption>";
   }
   else
   {
      if($standard=GetTrackStandard($treventslong_g[$eventnum],"Girls",$class))
      {
      ...
      if(preg_match("/Meter/",$treventslong_g[$eventnum]) && ($class=="A" || $class=="B"))   //NOTE ABOUT TOP 8/8 vs TOP 12/9
      {         
      ...
      if($treventslong_g[$eventnum]=="Pole Vault" || $treventslong_g[$eventnum]=="High Jump")
      {
	 echo "<li><b>TIES:</b>
	<ul><li> In <b>".$treventslong_g[$eventnum]."</b>, per the NSAA manual, those tied for the last qualifying spot shall qualify if places cannot be determined by using the tiebreaker rule for field events.</li>
	<li><b>If there is a tie,</b> check \"Yes\" next to \"Did this competitor and the next competitor TIE?\" under the competitor's result.</li>
	<li>If <b>MORE THAN 2 SLOTS</b> are needed because of ties, they will show up as you click the \"Yes\" box.</li>
	...
	<th class=smaller colspan=2>Performance</th></tr>";
      ...
      else if($treventslong_g[$eventnum]=="High Jump" || $treventslong_g[$eventnum]=="Pole Vault")	//SPECIAL CASE: allow for ties
      {
	...
	<th class=smaller>Performance<br><font style=\"font-size:8pt;\">Ex: $trexample[$eventnum]</font></th></tr>";
      ...
      if($event!="400r" && $event!="1600r" && $event!="3200r")
      {
	    ...
	    $tempplace[$event]=split(",",$row[$eventplace]);
	       ...
	       if(!$sch[$event][$i] || $sch[$event][$i]=="Choose School")
            ...
            if(!preg_match("/Meter/",$treventslong_g[$eventnum]) || ($class!="B" && $class!="A"))
            {
      ...
      {
         $sql="SELECT sch,perf FROM $db1.tr_state_relays_g WHERE district='$distid' AND relay='$event' ORDER BY place";
         $result=mysql_query($sql);
         $ix=0;
         while($row=mysql_fetch_array($result))
         {
      ...
      if(!preg_match("/Meter/",$treventslong_g[$eventnum]) && !preg_match("/Extra/",$treventslong_g[$eventnum]) && !preg_match("/Team/",$treventslong_g[$eventnum]))
      {
         $sql="SELECT * FROM $db1.tr_state_place_g WHERE district='$distid'";
         $result=mysql_query($sql);
         $row=mysql_fetch_array($result);
      ...
      if($treventslong_g[$eventnum]=="High Jump" || $treventslong_g[$eventnum]=="Pole Vault")  //SPECIAL CASE: allow for ties
      {
	 ...
	 for($i=0;$i<$max;$i++)
	 {
	    ...
	    {               
	       $sql2="SELECT * FROM $db1.trgschool WHERE sid='".$sch[$event][$i]."'";               
	       $result2=mysql_query($sql2);               
	       $row2=mysql_fetch_array($result2);               
	       $sql="SELECT DISTINCT t2.id,t2.last,t2.first,t2.middle,t2.semesters FROM $db1.eligibility AS t2, $db1.headers AS t3 WHERE t2.school=t3.school AND tr='x' AND gender='F' AND (t3.id='$row2[mainsch]'";               
      ...
      else 
      {
         for($i=0;$i<$max;$i++) //for each place, one row:
         {
            ...
            if($event=="400r" || $event=="1600r" || $event=="3200r")  //relays
            {
               ...
               $sql="SELECT * FROM $db1.tr_state_relays_g WHERE district='$distid' AND relay='$event' ORDER BY place";
               $result=mysql_query($sql);
               $ix=0;
               while($row=mysql_fetch_array($result))
               {
   ...
   echo "</table>";
}//end if not teamscores 
if($event=="pv" || $event=="hj" || $event=="lj" || $event=="sp" || $event=="d" || $event=="tj")
{
   //Extra field event qualifiers
   echo "<table cellspacing=0 cellpadding=3 style=\"border:#808080 1px solid;width:100%;\" frame=\"all\" rules=\"all\"><caption><h3 style=\"color:red;\"><u>Extra ".strtoupper($treventslong_g[$eventnum])." Qualifiers:</u></h3>";
   echo "<ul><li class=notbold>Any student who is <b><u>NOT</u></b> listed above, in the top places for <b><u>".$treventslong_g[$eventnum]."</b></u>, but who met the qualifying mark of <u><b>$standard</b></u> at the District Meet, should be entered below as an \"Extra ".$treventslong_g[$eventnum]." Qualifier.\"</li>";
   ...
   $i2=$i; $i=0;
   $sql="SELECT * FROM $db1.tr_state_extra_g WHERE district='$distid' AND eventnum='$event'";
   $result=mysql_query($sql);
   while($row=mysql_fetch_array($result))
   {
      ...
      $sql2="SELECT * FROM $db1.trgschool WHERE sid='$extrasch[$i]'";
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
      $sql3="SELECT DISTINCT t2.id,t2.last,t2.first,t2.middle,t2.semesters,t2.school FROM $db1.eligibility AS t2, $db1.headers AS t3 WHERE t2.school=t3.school AND tr='x' AND gender='F' AND (t3.id='$row2[mainsch]'";
   ...
   while($i<10)
   {
      ...
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
      $sql="SELECT DISTINCT t2.id,t2.last,t2.first,t2.middle,t2.semesters,t2.school FROM $db1.eligibility AS t2, $db1.headers AS t3 WHERE t2.school=t3.school AND tr='x' AND gender='F' AND (t3.id='$row2[mainsch]'";
      if($row2[othersch1]) $sql.=" OR t3.id='$row2[othersch1]'";
      if($row2[othersch2]) $sql.=" OR t3.id='$row2[othersch2]'";
      if($row2[othersch3]) $sql.=" OR t3.id='$row2[othersch3]'";
      $sql.=") ORDER BY t2.school,t2.last,t2.first";
   ...
   echo "</table>";
}//end if field events (extra qualifiers)
else if($event=="teamscores")
{
   //text box for team scores:
   $sql="SELECT teamscores,teams,indys FROM $db1.tr_state_dist_g WHERE dist='$distid'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
