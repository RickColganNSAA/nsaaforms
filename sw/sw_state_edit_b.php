...
$sql="SELECT school,hytekabbr,stateform_b,sid FROM swschool WHERE school='$school2'";
...
//also get school name from headers (this all needs to be fixed the right way this year!!)
   ...
   for($i=0;$i<count($sw_events);$i++)
   {
      if(!ereg("Relay",$sw_events[$i]))
      {
	    ...
	    if($swstud[$sw_events[$i]][$j]!="Choose Student")
	    {
	       ...
	       $sql="INSERT INTO sw_state_b (schoolid,hytekabbr,event,entry,studs) VALUES ('$schoolid','$hytekabbr','$sw_events[$i]','$stud_time[1]','$stud_time[0]')";
	       $result=mysql_query($sql);
	       /*
	 ...
	 $sql="INSERT INTO sw_state_b (schoolid,hytekabbr,event,entry,studs) VALUES ('$schoolid','$hytekabbr','$sw_events[$i]','".$relaytime[$sw_events[$i]]."','$relaylist')";
	 $result=mysql_query($sql);
      }
   }
   /*
...
echo "<table>";
echo "<caption><b>$school Boys State Swimming Entry Form:<br>";
...
for($i=0;$i<count($sw_events);$i++)
{
   if(!ereg("Relay",$sw_events[$i]))
   {
	 ...
	 echo "<font style=\"font-size:8pt\">(You must fax diver's scoresheet with<br>BOTH coach and divers signatures)</font></td></tr>";
      ...
      $sql="SELECT t1.* FROM sw_verify_perf_b AS t1, sw_verify_b AS t2 WHERE ((t1.formid=t2.id AND t2.approved='y') OR t1.formid='0') AND (t1.school='$sw_sch2' OR t1.school='$sw_sch3') AND t1.event='$sw_events[$i]' ORDER BY t1.performance";
	 ...
	 $cix=0;
      ...
      { 
	    ...
	    if($qualify!="no")
	    {
	       ...
	       $sql3="SELECT id FROM sw_state_b WHERE schoolid='$schoolid' AND entry='".$cur_studs[mark][$k]."' AND studs='".$cur_studs[id][$k]."'";
...
for($i=0;$i<count($sw_events);$i++)
{
   if(ereg("Relay",$sw_events[$i]))
   {
      ...
      //$sql="SELECT t1.formid,t1.performance,t1.id FROM sw_verify_perf_b AS t1,sw_verify_b AS t2 WHERE ((t2.id=t1.formid AND t2.approved='y') OR t1.formid='0') AND (t1.school='$sw_sch2' OR t1.school='$sw_sch3') AND t1.event='$sw_events[$i]' LIMIT 10";
      $sql="SELECT formid,performance,id FROM sw_verify_perf_b WHERE (school='$sw_sch2' OR school='$sw_sch3') AND event='$sw_events[$i]' ORDER BY performance";
      $result=mysql_query($sql);
      $relaytimes=array();
      $rix=0;
      while($row=mysql_fetch_array($result))
      {
	 $sql2="SELECT approved FROM sw_verify_b WHERE id='$row[0]'";
	 $result2=mysql_query($sql2);
	 $row2=mysql_fetch_array($result2);
	 if($row2[0]=='y' || $row[0]=='0')
	 {
