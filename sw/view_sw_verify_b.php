...
{
   if($level==1)
      echo "<a href=\"edit_sw_verify_b.php?session=$session&school_ch=$school_ch&formid=$formid\" class=small>Edit This Form</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"view_sw_b.php?session=$session&school_ch=$school\" class=small>$school Swimming Home</a>";
   echo "&nbsp;&nbsp;&nbsp;<a href=\"verifyadmin.php?session=$session\" class=small>Verification Forms Admin</a>";
   echo "&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"view_sw_verify_b.php?session=$session&school_ch=$school_ch&formid=$formid&print=1\" target=new class=small>Printer-Friendly Version</a>";
}

//get meet info out of database
$sql="SELECT t1.id,t1.submitter,t1.email,t1.referee,t1.approved,t1.meetid,t2.* FROM sw_verify_b AS t1,swsched AS t2 WHERE t1.meetid=t2.id AND t1.id='$formid'";
...
$info="<br><br><table><caption><b>BOYS VERIFICATION FORM<br>of State Swimming and Diving Qualifying Performances</b><hr></caption>";
...
if($print!=1)
{
   ...
   if($level==1)
      echo "<a href=\"edit_sw_verify_b.php?session=$session&school_ch=$school_ch&formid=$formid\" class=small>Edit This Form</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"view_sw_b.php?session=$session&school_ch=$school_ch\" class=small>$school Swimming Home</a>";
   echo "&nbsp;&nbsp;&nbsp;<a href=\"verifyadmin.php?session=$session\" class=small>Verification Forms Admin</a>&nbsp;&nbsp;";
   echo "<a href=\"../welcome.php?session=$session\" class=small>Return to Home</a>";
   echo "&nbsp;&nbsp;&nbsp;";
   echo "<a class=small href=\"view_sw_verify_b.php?session=$session&school_ch=$school_ch&formid=$formid&print=1\" target=new>Printer-Friendly Version</a>";
}

//write to .html file
$filename="swverify".$formid."_b.html";
...
{
   $sql="SELECT senttoNSAA FROM sw_verify_b WHERE id='$formid'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   if($row[0]!='y')
   {
      ...
      $Html="Attached is $school's Boys Swimming Verification Form.<br><br>";
      ...
      $sql="UPDATE sw_verify_b SET senttoNSAA='y' WHERE id='$formid'";
      $result=mysql_query($sql);
   }
}
