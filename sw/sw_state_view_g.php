<?php
//sw_state_view_g.php: View SW State Entry Form (Girls)
...
$schid=$row[id];
$sql="SELECT school,hytekabbr,stateform_g,sid FROM swschool WHERE mainsch='$schid' OR othersch1='$schid' OR othersch2='$schid' OR othersch3=
'$schid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$sw_sch=$row[school];
...
if($print!=1)
{
   echo "<a class=small href=\"sw_state_view_g.php?session=$session&school_ch=$school_ch&print=1\" target=new>Printer-Friendly Version</a>";
   if($level==1)
   {
      echo "&nbsp;&nbsp;&nbsp;<a class=small href=\"sw_state_edit_g.php?session=$session&school_ch=$school_ch\">Edit this Form</a>";
...
if($final=='y' && $print!=1)
{
   echo "<font style=\"color:red\"><b>The following Girls State Swimming Entry Form has been submitted to the NSAA:<br><br></font></b>";
   $now=time();
   $sql="UPDATE swschool SET stateform_g='$now' WHERE sid='$schoolid'";
   $result=mysql_query($sql);
}
$info.="<table border=1 bordercolor=#000000 cellspacing=0 cellpadding=3>";
$info.="<caption><b>$sw_sch Girls State Swimming Entry Form:</b></caption>";
for($i=0;$i<count($sw_events);$i++)
{
   if(!ereg("Relay",$sw_events[$i]))
   {
	 ...
	 $info.="<br><font style=\"font-size:8pt\">(You must fax diver's scoresheet with<br>BOTH coach and divers signature)</font>";
      $info.="</caption>";
      $sql2="SELECT t1.entry,t2.first,t2.last,t2.semesters FROM sw_state_g AS t1,eligibility AS t2 WHERE t1.studs=t2.id AND t1.schoolid='$schoolid' AND event='$sw_events[$i]' ORDER BY entry";
	 ...
	 $qualify=DoesQualify("Girls ".$sw_events[$i],$row2[0]);
...
for($i=0;$i<count($sw_events);$i++)
{
   if(ereg("Relay",$sw_events[$i]))
   {
      ...
      $sql2="SELECT entry FROM sw_state_g WHERE event='$sw_events[$i]' AND schoolid='$schoolid'";
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
      $entry=$row2[0];
      $entry2=ConvertFromSec($entry);
      $info.=$entry2." - ";
      $qualify=DoesQualify("Girls ".$sw_events[$i],$entry);
      if($qualify=="Automatic") $info.="AUTO";
      else $info.="SEC";
      $info.="</td></tr>";
...
if($print!=1)
{
 ...
 citgf_makepublic("/home/nsaahome/attachments/$filename");
   ...
   echo "<br><a class=small target=new href=\"sw_state_view_g.php?session=$session&school_ch=$school_ch&print=1\">Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
   if($level==1)
      echo "<a class=small href=\"sw_state_edit_g.php?session=$session&school_ch=$school_ch\">Edit this Form</a>&nbsp;&nbsp;&nbsp;";
...
if($final=='y')
{
   ...
   $To=GetEmail("sw");
   $ToName="NSAA";
   $Subject="$sw_sch ($hytekabbr) Girls State Swimming Entry Form";
   $Html="$school has submitted the $sw_sch Girls State Swimming Entry Form.<br><br>Their entry form is attached in .html format.<br><br>Thank You!";
