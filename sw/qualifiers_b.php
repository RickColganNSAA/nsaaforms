<?php
//qualifiers_b.php: get boys state meet qualifiers from verification forms and uploaded TM files
//	write results to nsaahome.org/textfile/swim/bbest.htm
...
$info="<html><head><title>Swimming | Boys Swimming Season Best Performances</title>";
...
$info.="<table><caption><b>Boys Swimming Season Best Performances:</b><br>";
...
//get qualifiers from sw_verify_perf_b
for($x=0;$x<count($sw_events);$x++)
{
   if(!preg_match("/Relay/",$sw_events[$x]))	//non-Relay
   {
      $sql="SELECT DISTINCT t2.* FROM sw_verify_b AS t1, sw_verify_perf_b AS t2 WHERE ((t1.id=t2.formid AND t1.submitted='y' AND t1.approved='y') OR t2.formid='0') AND t2.event='$sw_events[$x]' ORDER BY t2.performance";
      ...
      {
         $formid=$row[1];
         $qualtype=DoesQualify("Boys $row[3]",$row[5]);
	    ...
	    /*
	       ...
	       $host="none";
            ...
            else		//get meet from sw_verify_b table
            {
	       $sql2="SELECT meet,meetdate,school FROM sw_verify_b WHERE id='$row[1]'";
	    ...
	    */
            $sql2="SELECT meetid FROM sw_verify_b WHERE id='$formid'";
	    $result2=mysql_query($sql2);
	    $row2=mysql_fetch_array($result2);
   ...
   else	//RELAY
   {
	 ...
	 if($qualtype!='no')
	 {
	    ...
	    if($inarray==0)	//first entry for this school in array
	    {
	       ...
	       */
	       $sql2="SELECT meetid FROM sw_verify_b WHERE id='$row[1]'";
	       $result2=mysql_query($sql2);
	       $row2=mysql_fetch_array($result2);
	       $qual[meet][$qix]=GetMeetName($row2[0]);
	       $qual[meetdate][$qix]=GetMeetDate($row2[0]);
...
$open=fopen(citgf_fopen("../../textfile/swim/bbest.htm"),"w");
fwrite($open,$info);
fclose($open); 
 citgf_makepublic("../../textfile/swim/bbest.htm");

echo "The <i>Boys'</i> Swimming Season Best Performances file has been generated!<br><br>";
echo "<a target=new href=\"../../textfile/swim/bbest.htm\">Click Here to Preview Boys' File</a>";
echo "<br><br><a href=\"qualifiers_g.php?session=$session\">Click Here to Generate Girls' File</a>";
echo $end_html;
exit();
