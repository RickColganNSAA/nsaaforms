<?php
//edit_sw_verify_g.php: SW Verification Form: blank version
...
{
   $sql="DELETE FROM sw_verify_perf_g WHERE id='$delete'";
   //echo "$sql<br>";
...
if($save)	//user clicked Save button
{
   ...
   $sql="SELECT id FROM sw_verify_g WHERE id='$formid'";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0)	//meet not in DB yet
   {
      $sql2="INSERT INTO sw_verify_g (submitted,school,meetid,meetdate,referee,datesub,submitter,email) VALUES ('$send','$school2','$meetid','$meetdate','$referee','$today','$submitter','$subemail')";
      $result2=mysql_query($sql2);

      //get formid out
      $sql2="SELECT id FROM sw_verify_g WHERE school='$school2' AND datesub='$today'";
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
      $formid=$row2[0];
   }
   ...
   {
      $sql2="UPDATE sw_verify_g SET submitted='$send', meetid='$meetid', meetdate='$meetdate', referee='$referee', datesub='$today',submitter='$submitter',email='$subemail' WHERE id='$formid'";
      $result2=mysql_query($sql2);
   }
   ...
   for($i=0;$i<10;$i++)
   {
	 ...
	 if($studevent[$i]!="Diving")
	 {
	    ...
	    else	//if student did qualify, put in DB
	    {
		  ...
		  $performance+=$sec[$i];
		  $performance.=".".$tenth[$i];
		  $sql2="INSERT INTO sw_verify_perf_g (formid,school,event,studentid,performance) VALUES ('$formid','$school2','$studevent[$i]','$stud[$i]','$performance')";
		  $result2=mysql_query($sql2);
		  echo mysql_error();
	       }
	       else
	 ...
	 else	//Diving
	 {
	    ...
	    else	//if student did qualify, put in DB
	    {
		  ...
		  $stud[$i]=substr($stud[$i],0,strlen($stud[$i])-1);
	       ...
	       $sql2="INSERT INTO sw_verify_perf_g (formid,school,event,studentid,performance) VALUES ('$formid','$school2','$studevent[$i]','$stud[$i]','$diving[$i]')";
	       $result2=mysql_query($sql2);
	    }
	 }
      }//end if data entered
   ...
   {
      header("Location:view_sw_verify_g.php?session=$session&school_ch=$school_ch&formid=$formid&send=y");
      exit();
   }
   /*
   else
   {
      header("Location:edit_sw_verify_g.php?session=$session&school_ch=$school_ch&formid=$formid&error=$error");
      exit();
   }
   */
}
...
$sql="SELECT * FROM sw_verify_g WHERE id='$formid'";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result))
{
   $meetid=$row[meetid];
...
echo "<a href=\"view_sw_g.php?session=$session&school_ch=$school_ch\" class=small>Return to Swimming Home</a><br><br>";
...
echo "<form name=swform method=post action=\"edit_sw_verify_g.php\">";
echo "<input type=hidden name=session value=$session>";
echo "<input type=hidden name=formid value=$formid>";
echo "<input type=hidden name=school_ch value=\"$school_ch\">";
echo "<input type=hidden name=delete value=''>";
echo "<table><caption><b>GIRLS VERIFICATION FORM<br>of State Swimming and Diving Qualifying Performances</b><hr></caption>";
...
$sql="SELECT eventfull,qualmark,automark FROM sw_qualify WHERE eventfull LIKE 'Girls%' ORDER BY id";
$result=mysql_query($sql);
...
if($formid)
{
   ...
   while($row=mysql_fetch_array($result))
   {
      echo "<tr align=left><td align=center><a href=\"edit_sw_verify_g.php?session=$session&formid=$formid&delete=$row[id]&school_ch=$school_ch\">X</a></td>";
      echo "<td>$row[3]</td>";
      if(!ereg("Relay",$row[3]))
...
for($i=0;$i<10;$i++)
{
   ...
   { 
      $sql2="SELECT t2.performance FROM sw_verify_g AS t1, sw_verify_perf_g AS t2 WHERE ((t1.id=t2.formid AND t1.submitted='y' AND t1.approved='y') OR t2.formid='0') AND t2.event='$studevent[$i]' AND t2.studentid='".$studch[id][$k]."' ORDER BY t2.performance LIMIT 1";
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
