...
/****************************************
programadmin.php
Cornerstone Admin for Dual Wrestling
1/19/16
Ann Gaffigan
*****************************************/
...
echo "<br><table class='nine' cellspacing=0 cellpadding=5 frame=all rules=all style=\"border:#a0a0a0 1px solid;\"><caption><b>$sportname Roster Forms Admin</b><br>";
if($errors && $errors!='')
{
...
$sql="SELECT DISTINCT school,submitted FROM $table"; 
...
{
   echo "<i>The following Dual Wrestling Rosters have been Approved by the NSAA:</i>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $sch=preg_replace("/[^0-9a-zA-Z]/","",$row[school]);
   $sch=strtolower($sch);
   $filename="$sch$activ_lower";
   $filename.="state.csv";

   $schs[$ix]=$row[school];
   $links[$ix]="<a href=\"view_wrd.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>";
   $subtimes[$ix]=$row[submitted];
   if(mysql_num_rows($result2)==0)
   {
      $orders[$ix]="&nbsp;";
...
for($i=0;$i<count($orders);$i++)
{
   ...
   echo "<td>".date("m/d/y",$subtimes[$i])." at ".date("g:ia T",$subtimes[$i])."</td>";
   if(mysql_num_rows($result2)==0)
   {
