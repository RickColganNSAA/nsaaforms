...
{	
	...
	if ( !empty($_FILES) )
	{
		...
		$uniqueFileName = sprintf("%s/%s_1.%s",$pathinfo['dirname'],$sid,$pathinfo['extension']); //$fileName;
		for($k = 1; $k < 100; $k++)
		{
			...
			else
			{
				...
				$uniqueFileName = sprintf("%s/wrd_teamphoto_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
                                $uniqueThumbName = sprintf("%s/wrd_teamphoto_resized_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
			}
...
/*
	...
	*/
			...
			$sql="UPDATE wrschool SET filename='$useFileName' WHERE sid='$sid'";
	...
	{
		echo "<eaferror>Request didn't contain the file. Usually this situation occurs if request size exceeds the post_max_size directive in php.ini.</eaferror>";
	}
