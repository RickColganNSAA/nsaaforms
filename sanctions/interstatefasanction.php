<?php
/*******************************************************
interstatefasanction.php

Application for Sanction of Interstate Fine Arts Events
...
{
   $sql="SELECT * FROM interstatefasanctions WHERE id='$copyfromid'";
   ...
   while($row=mysql_fetch_array($result))
   {
      ...
      $sql2="INSERT INTO interstatefasanction_invitees (appid,schoolname,address,city,state,zip,contact,phone,nfhsmember) VALUES ('$appid','$schoolname','$address','$city','$row[state]','$row[zip]','$contact','$row[phone]','$row[nfhsmember]')";
      $result2=mysql_query($sql2);
   }
   header("Location:interstatefasanction.php?session=$session&appid=$appid&edit=1&copied=1");
   exit();
}

if($level==1 && $open==1 && $appid)
{
   $sql="UPDATE interstatefasanctions SET opened='".time()."' WHERE id='$appid'";
   $result=mysql_query($sql);
}
else if($level==1 && $close==1 && $appid)
{
   $sql="UPDATE interstatefasanctions SET opened=0 WHERE id='$appid'";
   $result=mysql_query($sql);
}

if($savemywork || ($edit==1 && $nsaasubmit) || $send || $save || $addmore)	//AD SUBMITTED FORM - CHECK FOR ERRORS, ADD TO DB FOR NSAA TO REVIEW *OR* FORM WAS UPDATED - SAVE CHANGES
{
   if($send)
   {
   ...
   {
      $sql="UPDATE interstatefasanctions SET sport='$sport',startdate='$startdate',enddate='$enddate',eventtime='$eventtime',eventname='$eventname',sponsor='$sponsor',sponsoraddress='$sponsoraddress',sponsorcity='$sponsorcity',sponsorstate='$sponsorstate',sponsorzip='$sponsorzip',entryfee='$entryfee',admissionfee='$admissionfee',studentawards='$studentawards',teamawards='$teamawards',coachawards='$coachawards',studentawardsvalue='$studentawardsvalue',teamawardsvalue='$teamawardsvalue',coachawardsvalue='$coachawardsvalue',signature='$signature',pphone='$pphone',pemail='$pemail'";
   ...
   {
      $sql="INSERT INTO interstatefasanctions (sport,school,startdate,enddate,eventtime,eventname,sponsor,sponsoraddress,sponsorcity,sponsorstate,sponsorzip,entryfee,admissionfee,studentawards,teamawards,coachawards,studentawardsvalue,teamawardsvalue,coachawardsvalue,signature,pphone,pemail";
      if($send && $errors=="") $sql.=",submitted";
      $sql.=") VALUES ('$sport','$school2','$startdate','$enddate','$eventtime','$eventname','$sponsor','$sponsoraddress','$sponsorcity','$sponsorstate','$sponsorzip','$entryfee','$admissionfee','$studentawards','$teamawards','$coachawards','$studentawardsvalue','$teamawardsvalue','$coachawardsvalue','$signature','$pphone','$pemail'";
   ...
   //INVITEES
   $sql="DELETE FROM interstatefasanction_invitees WHERE appid='$appid'";
   $result=mysql_query($sql);
   for($i=0;$i<count($schoolname);$i++)
   {
      if(trim($schoolname[$i])!='')
	 ...
	 $contact[$i]=addslashes($contact[$i]);
         $sql="INSERT INTO interstatefasanction_invitees (appid,schoolname,address,city,state,zip,contact,phone,nfhsmember) VALUES ('$appid','$schoolname[$i]','$address[$i]','$city[$i]','$state[$i]','$zip[$i]','$contact[$i]','$phone','$nfhsmember[$i]')";
	 $result=mysql_query($sql);
      }
   }

   if($send && $errors=="")	//SHOW CONFIRMATION
   {
      echo $init_html;
      echo $header;
      echo "<br /><br /><div class='alert' style='width:500px'><b>Thank you for submitting an Application for Sanction of Interstate Fine Arts Events.</b><br /><br />Your application has been sent to the NSAA. The NSAA will notify you when final approval of this event has been made.</div><br /><br /><a href=\"sanctionslist.php?eventtype=interstatefasanctions&session=$session\">Return to Sanctions for Interstate/International Athletic & Fine Arts Events MAIN MENU</a>";
      echo $end_html;
      exit();
   }
   $edit=0;
...
else if($saveaction)	//STATE ASSOCIATION (OR OTHER LEVEL 7 USER) TOOK ACTION
{
   ...
   for($i=0;$i<count($inviteeid);$i++)
   {
      ...
      $sql="UPDATE interstatefasanction_invitees SET membership='$membership[$i]',action='$action[$i]',nojurisdiction='$nojurisdiction[$i]',comments='$comments[$i]',signature='$signature[$i]' WHERE id='$inviteeid[$i]'";
      $result=mysql_query($sql);
      //ONLY UPDATE datesigned IF THIS IS THE FIRST TIME ACTION's BEEN TAKEN ON THIS INVITEE
      $sql="UPDATE interstatefasanction_invitees SET datesigned='$datesigned' WHERE id='$inviteeid[$i]' AND datesigned=0";	
      $result=mysql_query($sql);
      $sql="UPDATE interstatefasanction_invitees SET datesigned=0 WHERE signature=''";	//If NO SIGNATURE, make sure datesigned is 0
      $result=mysql_query($sql);
   }
   //SEE IF USER HAS TAKEN ACTION ON ALL SCHOOLS FOR THIS APPLICTION; IF SO, MARK $state.approved FIELD IN interstatefasanctions TABLE
   $sql="SELECT id FROM interstatefasanction_invitees WHERE appid='$appid' AND (membership='' OR action='' OR signature='' OR datesigned=0) AND state='$state'";
   $result=mysql_query($sql);
   $actiondone=0;
   if(mysql_num_rows($result)==0)	//ALL HAVE BEEN TAKEN ACTION ON
   {
      $sql="UPDATE interstatefasanctions SET ".$state."approved='".time()."' WHERE id='$appid' AND ".$state."approved<=1";
      $result=mysql_query($sql);
      $actiondone=1;
   }
}
if($nsaasubmit && $level==1)	//NSAA APPROVED REQUEST
{
   ...
   $sql="UPDATE interstatefasanctions SET membership='$hostmembership',action='$hostaction',nojurisdiction='".addslashes(trim($hostnojurisdiction))."',comments='".addslashes(trim($hostcomments))."' WHERE id='$appid'";
   $result=mysql_query($sql);
   //ONLY UPDATE APPROVAL DATE IF IT WAS 0 SO FAR
   $sql="UPDATE interstatefasanctions SET NSAAapproved='$NSAAapproved' WHERE id='$appid' AND NSAAapproved=0";
   $result=mysql_query($sql);
}
else if($level==1 && $nsaafinal)	//NSAA COMPLETED FINAL APPROVAL
{
   //ONLY UPDATE FINAL APPROVAL DATE IF IT WAS 0 SO FAR
   $sql="UPDATE interstatefasanctions SET NSAAfinal='".time()."' WHERE id='$appid' AND NSAAfinal=0";
   $result=mysql_query($sql);
   $finaldone=1;
}
else if($level==1 && $unapprove==1)
{
   $sql="UPDATE interstatefasanctions SET NSAAfinal=0 WHERE id='$appid'";
   $result=mysql_query($sql);
}
if($pdf!=1)
{
...
<body onload="Sanctions.initialize('interstatefasanctions');">
<?php
echo $header;
}
...
if($savemywork)
{
   ...
   echo "<div class='help'>Your work below has been saved. You may continue working on this form or <a style='color:#8b0000;' href=\"sanctionslist.php?eventtype=interstatefasanctions&session=$session\">Return to your Applications for Sanction Main Menu</a>.</div>";
}

//IF THIS FORM HAS ALREADY BEEN INITIATED, WE WANT TO GET THE DATA:
$sql="SELECT * FROM interstatefasanctions WHERE id='$appid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);

if($pdf!=1)
{
echo "<form method='post' action='interstatefasanction.php'>";
...
echo "<caption><b>Application for Sanction of Interstate Fine Arts Event:</b><br><br>";
...
if(!$appid)
{
   ...
   $sql2="SELECT * FROM interstatefasanctions WHERE school='$school2' AND submitted>0 ORDER BY startdate DESC,enddate DESC";
   $result2=mysql_query($sql2);
...
else if($nsaafinal)
{
   ...
   if($finaldone==1)
      echo "<b>You have COMPLETED ACTION for this application.</b> <a href=\"sanctionsadmin.php?eventtype=interstatefasanctions&session=$session\" class=small>Return to Sanctions MAIN MENU</a>";
   echo "</div><br><br>";
}
else if($level==1 && $unapprove==1)
{
...
if($pdf!=1)
{
   ...
   if($row[submitted]==0 && $level!=1)
      echo "<p><br><a onClick=\"return confirm('Are you sure you want to delete this application? This action cannot be undone.');\" href=\"sanctionslist.php?session=$session&table=interstatefasanctions&delete=$row[id]\">Delete this Application</a></p>";
...
if($row[submitted]>0 && $pdf!=1)
{
   ...
   if($level==1)
   {
      if(!$edit) echo "&nbsp;&nbsp;<a href=\"interstatefasanction.php?session=$session&appid=$appid&edit=1\">EDIT SECTION 1</a>";
      if($row[opened]>0)
      {
         echo "<br><font style='color:#333333;'>[Host school currently can EDIT this form. It will lock again once the AD submits it. Or you can <a class=small href=\"interstatefasanction.php?session=$session&appid=$appid&edit=$edit&close=1\">Lock this Form</a>]</font>";
      }
      else
      {
         echo "<br><font style='color:#333333;'>[Host school is LOCKED from editing this form. <a class=small href=\"interstatefasanction.php?session=$session&appid=$appid&edit=$edit&open=1\">Unlock this Form</a>]</font>";
      }
   }
   else
   {
      if($row[opened]>0 && $edit!=1)
         echo "&nbsp;&nbsp;<a href=\"interstatefasanction.php?session=$session&appid=$appid&edit=1\">EDIT THIS SECTION</a>";
...
$string.="<tr align=left><td><b>Activity:</b>&nbsp;";
if($pdf==1) $pdfhtml.=$string;
else echo $string;
if($edit && $pdf!=1)
{
   echo "<select name='sport' id='sport'><option value=''>Select an Activity</option>";
   for($i=0;$i<count($sanctionact);$i++)
   {
         echo "<option value=\"$sanctionact2[$i]\"";
         if($row[sport]==$sanctionact2[$i])  echo " selected"; 
         echo ">$sanctionact[$i]</option>";
   }
   echo "</select>";
}
else 
...
$string="<tr align=center><td>";	//BEGIN TABLE OF INVITED SCHOOLS
	...
	$sql2="SELECT * FROM interstatefasanction_invitees WHERE appid='$appid' ORDER BY schoolname";
	$result2=mysql_query($sql2);
 	$invitees=array(); $i=0;
	while($row2=mysql_fetch_array($result2))
	{
...
$string="<tr align=left><td>Is there an <b>entry fee</b> for this event?&nbsp;";
if($pdf==1) $pdfhtml.=$string;
else echo $string;
if($edit && $pdf!=1)
{
...
$string="</td></tr><tr align=left><td><br><b>Description of Awards and Other Compensations and Maximum Retail Value:</b><br>(ribbons, trophies, t-shirts, waiver of entry fee, waiver of travel expenses, etc.)</td></tr>";
$string.="<tr align=center><td><table border='1'>";
$string.="<tr align=left><td>Individual Student Participant Awards:<br>";
if($pdf==1) $pdfhtml.=$string;
else echo $string;
if($edit && $pdf!=1)
...
{
   $string="<br>$row[teamawards]<br><br>Maximum retail value: $".$row[teamawardsvalue];
   if($pdf==1) $pdfhtml.=$string;
   else echo $string;
}
$string="</td><td>Directors Awards:<br>";
if($pdf==1) $pdfhtml.=$string;
else echo $string;
if($edit && $pdf!=1)
...
if($level==1 && $appid && $row[NSAAapproved]>0 && $edit!=1) //NSAA: LIST OF STATES OF INVITED SCHOOLS, LINK TO EMAIL PASSWORD, CHECKBOX IF SENT EMAIL, RESPONSE RECEIVED
{   
   ...
   else
   {
      ...
      if($level==1)
      {
         echo "<div class=alert><a href=\"interstatefasanction.php?session=$session&appid=$appid&edit=1#section3\">EDIT SECTION 3</a></div>";
      }
      echo "<table border='1'>";
   }
   //GET INVITED STATES' LOGIN INFO   
   $sql2="SELECT DISTINCT t2.* FROM interstatefasanction_invitees AS t1, logins AS t2 WHERE t1.state=t2.sport AND t2.level='7' AND t1.appid='$appid' ORDER BY t1.state";   
   $result2=mysql_query($sql2);   
   $i=0;   
   while($row2=mysql_fetch_array($result2))   
   {      
	 ...
	 echo "Password: $passcode&nbsp;";
      ...
      $sql3="SELECT * FROM interstatefasanction_invitees WHERE appid='$appid' AND state='$row2[sport]' ORDER BY schoolname";
      $result3=mysql_query($sql3);
      ...
      while($row3=mysql_fetch_array($result3))
      {
         ...
         if($row3[datesigned]==0)
            $string.="<td><b>No Action Taken Yet.</b></td>";
         else
...
else if((($level==1 && $edit==1) || $level==7) && $appid)    //ALLOW ASSOCIATION OF INVITED SCHOOL TO APPROVE/DENY
{
   ...
   //GET LIST OF SCHOOLS THIS STATE NEEDS TO APPROVE, SHOW ACTION FOR EACH
   $sql3="SELECT * FROM interstatefasanction_invitees WHERE appid='$appid' ";
...
if($row[NSAAfinal]>0)
{
   ...
   if($pdf!=1 && $level==1) $string.="&nbsp;&nbsp;<a href=\"interstatefasanction.php?session=$session&appid=$appid&unapprove=1\">Retract Approval for this Application</a>";
   $string.="</div></td></tr>";
   if($pdf==1) $pdfhtml.=$string;
   else echo $string;
}
...
{
   $string="<tr align=left><td><br><b><u>Final Action by the NSAA:</b></u><br>
   	<br>[AWAITING FINAL APPROVAL BY NSAA]</td></tr>";
   echo $string;
}
...
$pdf->writeHTML("<b>NSAA APPLICATION FOR<br>SANCTION OF INTERSTATE FINE ARTS EVENT</b>",true,0,true,0,"C");
