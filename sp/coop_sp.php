...
for($i=0;$i<8;$i++)
{
   if($check[$i]=='y' && $coop_school[$i]!="Choose School" && $coop_student[$i]!="Choose Student")
   {
      ...
      {
         $sql="INSERT INTO $table (student_id, checked, co_op) VALUES ('$coop_student[$i]','$check[$i]','$school2')";
         $result=mysql_query($sql);
      }
   }
echo mysql_error();
...
<body>
<center>
<form name=form1 method="post" action="coop_sp.php">
...
for($i=0;$i<8;$i++)
{
   ...
   if($coop_school[$i] && $coop_school[$i]!="Choose School")
   {
      ...
      $sql="SELECT * FROM eligibility WHERE sp='x' AND school='$coop_school[$i]' ORDER BY last";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
