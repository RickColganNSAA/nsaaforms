...
if($sid1 && $sid1!='' && $sid2 && $sid2!='')
   $sql0="SELECT * FROM ppschool WHERE (sid='$sid1' OR sid='$sid2') ORDER BY programorder";
else if($sid1 && $sid1!='')
...
while($row0=mysql_fetch_array($result0))
{
   ...
   $extras=array(); $e=0;
   $tablewidth=216-($xorig+$teamphotowidth+1)-5;
   $extrascol1=65;
   $extrascol2=$tablewidth-$extrascol1;
   ...
   while($row2=mysql_fetch_array($result2))
   {
      ...
      if($namect<=2)
      {
         $cast.="<tr valign=\"top\"><td align=\"left\"><b>$row2[part]:</b></td><td align=\"right\">$names</td></tr>";
      }
      else
      {
         $extras[$e]="<tr valign=\"top\"><td align=\"left\"><b>$row2[part]</b> ............... $names</td></tr>";
         $e++;
      }
   }
   $cast.="</table>";
   ...
   $pdf->writeHTMLCell(130,"",$x,$y,"$playinfo",0,1,0,true,"L");
   ...
   $ybottom=$ytop+$teamphotoheight+3;
   ...
   $width=216-$x-5;
   $pdf->writeHTMLCell("$width","",$x,$y,"$cast",0,1,0,true,"L");
   $y=$pdf->GetY();
   $y+=3;
   $pdf->SetFont("berthold","",8);
   if($ybottom<=$y) 
      $crewwidth=$teamphotowidth;
   else
      $crewwidth=216-10;
   $x=$origx; $y=$ybottom;
   ...
   $pdf->SetFont("berthold","",8);
   $y++;
   $pdf->writeHTMLCell($crewwidth,"",$x,$y,"$crew",0,0,0,true,"L");
