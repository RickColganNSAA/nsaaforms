...
$table=$sport;
if(!$sort) $sort="statequalifier DESC";
$sql="SELECT DISTINCT school,statequalifier FROM $table WHERE statequalifier='x'";
...
{
   echo "<i>The following State ".GetActivityName($sport)." Entry Forms have been Submitted to the NSAA:</i>";
...
$ix=0;
$schs=array(); $links=array(); $orders=array(); $approved=array();
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a href=\"view_".$sport.".php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>";
   if(mysql_num_rows($result2)==0)
   {
      $orders[$ix]="&nbsp;";
...
{
   array_multisort($orders,SORT_NUMERIC,SORT_ASC,$schs,$links,$approved);
}
