...
//show list of choices for each sport:
   ...
   $sql="SELECT DISTINCT class FROM $db_name2.trdistricts WHERE class!='' ORDER BY class";
   $result=mysql_query($sql);
   $classes=array(); $i=0;
   ...
   for($j=0;$j<count($classes);$j++)
   {
	 if($level==1)
	 {
	    ...
	    $rnd=2; 
	       echo "<td>";
	       $sql="SELECT t1.school,t1.id,t1.note,t1.distid,t2.class,t2.type,t2.district FROM finance_tr AS t1,$db_name2.trdistricts AS t2 WHERE t1.distid=t2.id AND t2.class='$classes[$j]' AND t1.round='$rnd' ORDER BY t2.type,t2.district";
	       $result=mysql_query($sql);
	       while($row=mysql_fetch_array($result))
	       {
		  //checkbox to delete
...
echo "</table>";
if($level==1) echo "<p><input type=checkbox name=\"deleteall\" value=\"x\"> Check here to DELETE ALL Boys Track & Field financial reports. Then click \"Delete Checked\" below.</p><input type=submit name=submit value=\"Delete Checked\">&nbsp&nbsp<input type=submit name=submit value=\"Save Notes\"></form>";
echo $end_html;
?>
