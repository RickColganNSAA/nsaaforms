...
else if($session=="1424366492")
{
   $distid=7; $school_ch="Omaha Westside";
}
if(!$distid || $distid=='')
{
   echo $init_html;
   echo "<table width=100%><tr align=center><th><br><br>ERROR: No Subdistrict/Districtfinal Selected!";
   if($level==1)
      echo "<br><br><a href=\"soindex.php?session=$session\">Return to Financial Reports</a>";
   else
...
$sosch=split(", ",$row[schools]);
//if($row[type]=="Substate")
if($row[type]=="District Final")
   $sosch=split(" VS ",$row[schools]);
sort($sosch);
$type=$row[type];
if($type=="Subdistrict") $round=1;

else if($type=="District") $round=1;
else if($type=="District Final") $round=2;
else
{
   echo $init_html;
   echo "<table><tr align=center><th><br><br>ERROR: $type $class-$district is NOT a $sportname Subdistrict/Districtfinal.<br><br>";
   if($level==1)
      echo "<a href=\"soindex.php?session=$session\">Return to Financial Reports</a>";
   else
      echo "<a href=\"../welcome.php?session=$session\">Home</a>";
   echo $end_html;
   exit();
...
$sql="SELECT school FROM finance_".$sport." WHERE school='$school2' AND distid='$distid'";
$result=mysql_query($sql);
...
if($hiddenupdate || $update || $submitted==1) //submit report AND/OR show submitted report
{
   ...
   if($hiddenupdate || $update) //if report was submitted, add info to database & show printer-friendly version
   {
      ...
      if(($winner=='' || $runnerup=='') && $school!="Test's School")
      ...
      if($error=='')
      {
      $sql="SELECT * FROM finance_".$sport." WHERE school='$school2' AND distid='$distid'";
      $result=mysql_query($sql);
      $datesub=time();
      $winner2=addslashes($winner); $runnerup2=addslashes($runnerup);
      if(mysql_num_rows($result)==0)    //INSERT, THEN CAN UPDATE
      {
         $sql2="INSERT INTO finance_".$sport." (datesub,school,distid) VALUES ('$datesub','$school2','$distid')";
         $result2=mysql_query($sql2);
      }
      $sql2="UPDATE finance_".$sport." SET nocasts='$nocasts',localmedia_bcasts='$localmedia_bcasts',localmedia_bfee='$localmedia_bfee', othermedia_wcasts='$othermedia_wcasts', othermedia_wfee='$othermedia_wfee', othermedia_tcasts='$othermedia_tcasts', othermedia_tfee='$othermedia_tfee', datesub='$datesub',round='$round',winner='$winner2',runnerup='$runnerup2',attendance='$attendance',grossreceipts='$grossreceipts',offfees='$offfees',offmiles='$offmiles',offmilespaid='$offmilespaid',offtotal='$offtotal',insurance='$insurance',balance='$balance',hostallow='$hostallow',nsaaallow='$nsaaallow',gate='$gate',b_cast='$b_cast' WHERE school='$school2' AND distid='$distid'";
      $result2=mysql_query($sql2);
      }
   }//end if update
   ...
   if($print!=1)
   {
      ...
      {
         echo "<html><head><title>NSAA Home</title><link href=\"/css/nsaaforms.css\" rel=stylesheet type=\"text/css\"></head><body onload=\"window.open('".$sport."finance.php?school_ch=$school_ch&session=$session&distid=$distid&print=1');\">";
      }
      else echo $init_html;
      echo GetHeader($session);
      echo "<br>";
      if($level==1)
         echo "<a href=\"soindex.php?session=$session\" class=small>Return to Financial Reports</a>&nbsp;&nbsp;&nbsp;";
      echo "<a href=\"".$sport."finance.php?session=$session&distid=$distid&print=1&school_ch=$school_ch\" class=small target=new>Printer-Friendly Version</a><br><br>";
   ...
   echo "<table width=700 class=nine><caption><b>NSAA $sportname Financial Report</b><hr></caption>";
   ...
   $sql2="SELECT * FROM $db_name2.".$sport2."districts WHERE id='$distid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
   $dates="";
...
/*    echo "<table width=450 border=1 bordercolor=\"red\" cellspacing=0 cellpadding=5><tr align=left><td class=nine>";
   ...
   echo "Soccer<br>602-12 <u>$space $space $space $space $space $space $space $space $space</u><br><br>";
...
?>
<body onload="Finance2.initialize('<?php echo $session; ?>','$sport');">
<?php
echo "<br>";
if($level==1)
   echo "<a href=\"soindex.php?session=$session\" class=small>Financial Reports Home</a><br><br>";
echo "<form method=post action=\"".$sport."finance.php\" name=\"".$sport."form\">";
...
echo "<table width=90% cellspacing=2 cellpadding=2 class=nine><caption><b>NSAA ".strtoupper($sportname)." DISTRICT FINANCIAL REPORT</b><hr>";
if($error!='')
{
...
echo "<td width=100 bgcolor=yellow align=center>$<input type=text class=tiny size=7  name=\"gate\" id=\"gate\" value=\"".number_format($gate,2,'.','')."\" onblur=\"Finance2.Calculate(this.id,this.value);\"></td></tr>";

echo "<tr><td></td><td align=right><b>2. Broadcast Receipts&nbsp;&nbsp;</b><br>";
echo "<td width=100 bgcolor=yellow align=center>$<input type=text class=tiny size=7  name=\"b_cast\" id=\"b_cast\" value=\"".number_format($b_cast,2,'.','')."\" onblur=\"Finance2.Calculate(this.id,this.value);\"></td></tr>";

echo "<tr align=left valign=center><td><table><tr align=left bgcolor=yellow><td><b>Winner:</b>&nbsp;";
echo "<select name=\"winner\"><option value=''>~</option>";
for($i=0;$i<count($sosch);$i++) 
{
   echo "<option";
   if($winner==$sosch[$i]) echo " selected";
   echo ">$sosch[$i]</option>";
}
echo "</select></td>";
echo "<td><b>Runner-up:</b>&nbsp;";
echo "<select name=\"runnerup\"><option value=''>~</option>";
for($i=0;$i<count($sosch);$i++) 
{
   echo "<option";
   if($runnerup==$sosch[$i]) echo " selected";
   echo ">$sosch[$i]</option>";
}
echo "</select></td>";
echo "<td><b>Attendance:</b>&nbsp;<input type=text class=tiny size=6 name=\"attendance\" value=\"$attendance\"></td></tr></table></td>";
...
echo "<td width=100  align=center>$<input type=text class=tiny size=7 tabindex=\"400\" name=\"grossreceipts\" id=\"grossreceipts\" value=\"".number_format($grossreceipts,2,'.','')."\" onblur=\"Finance2.Calculate(this.id,this.value);\" readOnly=TRUE></td></tr>";
echo "<tr align=left><td colspan=3>Expenses are to be paid in full in order listed, using funds available.</td></tr>";
echo "<tr align=left><td colspan=3><b>Officials:</b><br><table>";
echo "<tr align=left valign=center><td>a.&nbsp;Fees&nbsp;<span style=\"color:red;font-size:90%;font-style: italic;\">($55 per game)</span></td>";
echo "<td bgcolor=yellow>$<input type=text class=tiny size=6 name=\"offfees\" id=\"offfees\" value=\"".number_format($offfees,2,'.','')."\" onblur=\"Finance2.Calculate(this.id,this.value);\"></td></tr>";
echo "<tr align=left><td><table cellspacing=0 cellpadding=0><tr align=left><td>b.&nbsp;Total Mileage One Way for All Officials:&nbsp;</td><td bgcolor=yellow><input type=text class=tiny size=3 name=\"offmiles\" id=\"offmiles\" value=\"$offmiles\" onblur=\"Finance2.Calculate(this.id,this.value);\"> miles</td><td>&nbsp;x $1.00</td></tr></table></td>";
...
echo "<br />
        ...
        echo "<tr bgcolor='yellow'><td align=right>Number of GAMES Broadcasted:</td><td align=left><input type=text size=2 name=\"othermedia_wcasts\" value=\"$othermedia_wcasts\" id=\"othermedia_wcasts\" onBlur=\"CalculateBcasts();\"></td></tr>";
