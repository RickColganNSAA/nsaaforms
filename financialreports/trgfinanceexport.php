<?php
//trgfinanceexport.php: export files of all trg financial report data
...
$sql="SELECT DISTINCT class FROM ".$db_name2.".trgdistricts WHERE class!='' ORDER BY class";
...
$trfields=array("hostschool","location","classdist","attendance","grossreceipts","offtotal","insurance","balance","nsaaallow","hostallow","check");
$trtitles=array("Host","Site","Class/District","Attendance","Total Receipts","Officials","Insurance-10%","Balance","NSAA-25%","Host School-75%","Check Amount");
...
for($file=0;$file<count($trfiles);$file++)
{
   ...
   $csv="";
    ...
    $sql="SELECT t1.*,t2.class,t2.district,t2.hostschool,t2.site FROM finance_tr AS t1,".$db_name2.".trdistricts AS t2 WHERE t2.class='$class' AND t1.distid=t2.id ORDER BY t2.district";
    $result=mysql_query($sql);
    $tid=0;	//tournament id
    $tourn=array();
    while($row=mysql_fetch_array($result))
...
$filename="trgsummary$curryear.csv";
$open=fopen(citgf_fopen("/home/nsaahome/reports/$filename"),"w");
fwrite($open,$csv);
fclose($open); 
 citgf_makepublic("/home/nsaahome/reports/$filename");
