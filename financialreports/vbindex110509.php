...
for($i=0;$i<count($sports);$i++)
{
   ...
   for($j=0;$j<count($classes);$j++)
   {
	 ...
	 if($level==1)
	 {
	    ...
	    {
	       echo "<td>";
	       $sql="SELECT school,id,note,class,district FROM finance_vb WHERE class='$classes[$j]' AND round='$rnd' ORDER BY class,district";
	       $result=mysql_query($sql);
	       while($row=mysql_fetch_array($result))
	       {
		  //checkbox to delete
		  ...
		  //link to form
		  echo "<a class=green href=\"vbfinance.php?session=$session&class=$classes[$j]&round=$rnd&school_ch=$row[0]\" class=small>$row[class]-$row[district]: $row[0]</a><input type=hidden name=\"vbsch[$vix]\" value=\"$row[1]\">";
		  //text box for note
