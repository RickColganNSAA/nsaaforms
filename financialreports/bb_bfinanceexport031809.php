...
$bb_bfields=array("classdist","location","attendance","grossreceipts","offtotal","insurance","hostgiven","hostallow","nsaaallow","balance","mileagedue","prorate","mileagepaid","bonus","hostperc","hostbonus","nsaabonus","visperc","visbonus","grossreceipts","offtotal","hosttotal","vistotal","nsaatotal","insurance","nsaacheck");

$bb_btitles=array("District","Site","Attendance","Total Receipts","Officials","Insurance-10%","Host Allowance","Host School","NSAA","Balance","Expenses","Prorate","Expenses","Balance Bonus","% Host","Host School","NSAA","% Teams","Teams","Total Receipts","Officials","Host","Teams","NSAA","Insurance","Check Amount");
...
for($file=0;$file<count($bb_bfiles);$file++)
{
    ...
    $sql="SELECT t1.*,t2.id AS distid,t2.class,t2.district,t2.hostschool,t2.site FROM finance_bb_b AS t1,$db_name2.bbbdistricts AS t2 WHERE t2.class='$class' AND t1.distid=t2.id AND t2.type='$type' ORDER BY t2.district";
   ...
   {
      // FOR EACH DATA FIELD
      if($bb_bfields[$field]=="mileagedue")	//miles paid to each team (non-prorated)
      {
	 ...
	 $csv.="\r\n";
      ...
      else if($bb_bfields[$field]=="mileagepaid")	//prorated miles paid
      {
	 for($i=0;$i<6;$i++)
	 {
	    $csv.="Expenses,";
	 ...
	 for($t=0;$t<$tid;$t++)
	 {
	    if($bb_bfields[$field]=="grossreceipts")
	       $totalrec+=$tourn[$bb_bfields[$field]][$t];
            else if($bb_bfields[$field]=="attendance")
               $totalatt+=$tourn[$bb_bfields[$field]][$t];
	    ...
	    else if($bb_bfields[$field]=="vistotal")
	       $totalvis+=$tourn[$bb_bfields[$field]][$t];
	    else if($bb_bfields[$field]=="nsaatotal")
	       $totalnsaa+=$tourn[$bb_bfields[$field]][$t];
	    else if($bb_bfields[$field]=="insurance")
