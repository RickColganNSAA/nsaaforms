 ...
 if(!$distid || $distid=='')
{
   ...
   if($level==1)
      echo "<a href=\"vbindex.php?session=$session\">Return to Financial Reports</a>";
   else
...
else
{
   ...
   {
      $sql="SELECT school FROM logins AS t1, $db_name2.vbdistricts AS t2 WHERE t1.id=t2.hostid AND t2.id='$distid'";
...
$sql="SELECT * FROM $db_name2.vbdistricts WHERE id='$distid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
...
$type=$row[type];
$vbsch=split(", ",$row[schools]);
if($type=="District Final")
   $vbsch=split(" VS ",$row[schools]);
if($row[sids]!='')
{
   $sids=split(",",$row[sids]);
   $vbsch=array(); 
   ...
   {
      $vbsch[$i]=GetSchoolName($sids[$i],'vb');
   }
}
sort($vbsch);
...
else if($type=="Substate") $round=2;
/* else
{
   echo $init_html;
   echo "<table><tr align=center><th><br><br>ERROR: $type $class-$district is NOT a Volleyball District.<br><br>";
   if($level==1)
      echo "<a href=\"vbindex.php?session=$session\">Return to Financial Reports</a>";
   else
      echo "<a href=\"../welcome.php?session=$session\">Home</a>";
...
$sql="SELECT * FROM finance_vb WHERE distid='$distid'";
$result=mysql_query($sql);
...
if($hiddenupdate || $submitted==1) //submit report AND/OR show submitted report
{
   if($hiddenupdate)//if report was submitted, add info to database & show printer-friendly version
   {
      ...
      {
         $sql2="INSERT INTO finance_vb (datesub,school,distid) VALUES ('$datesub','$school2','$distid')";
         $result2=mysql_query($sql2);
      }
      $sql2="UPDATE finance_vb SET nocasts='$nocasts',localmedia_bcasts='$localmedia_bcasts',localmedia_bfee='$localmedia_bfee', othermedia_wcasts='$othermedia_wcasts', othermedia_wfee='$othermedia_wfee', othermedia_tcasts='$othermedia_tcasts', othermedia_tfee='$othermedia_tfee', datesub='$datesub',round='$round',winner='$winner2',runnerup='$runnerup2',attendance='$attendance',grossreceipts='$grossreceipts',offfees='$offfees',offmiles='$offmiles',offmilespaid='$offmilespaid',offtotal='$offtotal',insurance='$insurance',hostgiven='$hostgiven',hostallow='$hostallow',nsaaallow='$nsaaallow',balance='$balance',vismileagepaid='$vismileage',prorate='$prorate',bonus='$bonus',nsaabonus='$nsaabonus',hostpart='$hostpart',hostgames='$hostgames',hostbonus='$hostbonus',visbonus='$visbonus',hosttotal='$hostsum',vistotal='$vissum',nsaatotal='$nsaasum',gate='$gate',b_cast='$b_cast' WHERE distid='$distid'";
      $result2=mysql_query($sql2);
      if(mysql_error())
      {
      ...
      //visiting schools:
      $sql2="DELETE FROM finance_vb_exp WHERE distid='$distid'";
      $result2=mysql_query($sql2);
      for($i=0;$i<count($vbschool);$i++)
      {
         $var1="row_".$i."_1";
         $var2="row_".$i."_2";
         $var3="row_".$i."_3";
         ...
         $var7="row_".$i."_7";
         $var8="row_".$i."_8";
  	 $cursch2=addslashes($vbschool[$i]);
	 $sql="INSERT INTO finance_vb_exp (distid,school,miles1way,trips,miles,mileagedue,mileagepaid,matches,bonus,totalpaid) VALUES ('$distid','$cursch2','".$$var1."','".$$var2."','".$$var3."','".$$var4."','".$$var5."','".$$var6."','".$$var7."','".$$var8."')"; 
	 $result=mysql_query($sql);
      }
   }//end if update
   ...
   if($print!=1)
   {
      if($hiddenupdate)
      {
         echo "<html><head><title>NSAA Home</title><link href=\"/css/nsaaforms.css\" rel=stylesheet type=\"text/css\"></head><body onload=\"window.open('vbfinance.php?session=$session&distid=$distid&print=1&school_ch=$school_ch');\">";
      }
      else echo $init_html;
      echo GetHeader($session);
      echo "<br>";
      if($level==1)
         echo "<a href=\"vbindex.php?session=$session\" class=small>Return to Financial Reports</a>&nbsp;&nbsp;&nbsp;";
      else if($school=="Test's School")
         echo "<a href=\"index.php?session=$session\" class=small>Return to Financial Reports</a>&nbsp;&nbsp;&nbsp;";
      echo "<a href=\"vbfinance.php?session=$session&distid=$distid&print=1&school_ch=$school_ch\" class=small target=new>Printer-Friendly Version</a><br><br>";
   ...
   echo "<table width=700 class=nine><caption><b>NSAA Volleyball Financial Report</b><hr></caption>";
   ...
   $sql2="SELECT * FROM $db_name2.vbdistricts WHERE id='$distid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
   $dates="";
   ...
   echo "<tr align=left valign=center><td>a.&nbsp;Fees ($55.00 each per game):&nbsp;</td>";
   ...
   echo "<tr><td align=left><b>Insurance Deduction</b> (10% of Total Receipts #3, to be sent to NSAA)</td>";
   ...
   if($type=="District Final" || $type=="Substate")
      ...
      echo "<tr><td align=left><b>Host School Allowance</b> ($100 for 4 teams; $125 for 5 teams; and $150 for 6 teams)</td>";
   echo "<td align=right><b>6. Host School Allowance&nbsp;&nbsp; $space $</b></td><td align=right>".number_format($row[hostgiven],2,'.','')."</td></tr>";
   echo "<tr><td align=left><b>Host School</b> (25% of #3, Gross Receipts)</td>";
   echO "<td align=right><b>7.Host School&nbsp;&nbsp; $space $</b></td><td align=right>".number_format($row[hostallow],2,'.','')."</td></tr>";
   echo "<tr><td align=left><b>NSAA</b> (25% of #3, Gross Receipts)</td>";
   echo "<td align=right><b>8. NSAA&nbsp;&nbsp; $space $</b></td><td align=right>".number_format($row[nsaaallow],2,'.','')."</td></tr>";
   ...
   $sql2="SELECT * FROM finance_vb_exp WHERE distid='$distid' ORDER BY id";
   $result2=mysql_query($sql2);
   ...
   {
      echo "<table><tr align=left><td>Host School, 15% of #11</td><td>$".number_format($row[hostbonus],2,'.','')."</td></tr>";
      echo "<tr align=left><td>Schools, 75% of #11</td><td>$".number_format($row[visbonus],2,'.','')."</td></tr>";
      if($row[visbonus]>0)
         echo "<tr align=left><td colspan=2>(Each school will receive $".number_format($row[visbonus]/$matches,2,'.','')." per game.)</td></tr>";
      echo "<tr align=left><td>NSAA, 10% of #11</td><td>$".number_format($row[nsaabonus],2,'.','')."</td></tr>";
      echo "</table>";
   }
   echo "</td><td align=right><b>11. Bonus&nbsp;&nbsp; $space $</b></td><td align=right>".number_format($row[bonus],2,'.','')."</td></tr>";
...
/*    echo "<table width=450 border=1 bordercolor=\"red\" cellspacing=0 cellpadding=5><tr align=left><td class=nine>";
   ...
   echo "VB 602-19 <u>$space $space $space $space $space $space $space $space $space $space</u><br><br>";
...
{
   var x=Utilities.getElement('gate').value;
   var y=Utilities.getElement('b_cast').value;
   var bfee=parseInt(x) + parseInt(y);
   Utilities.getElement('grossreceipts').value=bfee;
...
?>
<body onload="Finance3.initialize('<?php echo $session; ?>','vb');">
<?php
echo "<br><a href=\"index.php?session=$session\" class=small>Financial Reports Home</a><br><br>";
echo "<form method=post action=\"vbfinance.php\" name=vbform>";
...
echo "<table width=90% cellspacing=2 cellpadding=2 class=nine><caption><b>NSAA DISTRICT VOLLEYBALL TOURNAMENT FINANCIAL REPORT</b><hr>";
echO "</caption>";
echo "<tr align=left><td colspan=3><b>$type $class-$district at $hostschool.<br>";
...
//echo "<td width=100 bgcolor=yellow align=center>$<input type=text class=tiny size=7 tabindex=1 name=\"gate\" id=\"gate\" value=\"".number_format($gate,2,'.','')."\" onblur=\"CalculateTotal();\"></td></tr>";
...
//echo "<td width=100 bgcolor=yellow align=center>$<input type=text class=tiny size=7 tabindex=2 name=\"b_cast\" id=\"b_cast\" value=\"".number_format($b_cast,2,'.','')."\" onblur=\"CalculateTotal();\"></td></tr>";
...
{
   echo "<option";
   if($winner==$vbsch[$i]) echo " selected";
   echo ">$vbsch[$i]</option>";
}
echo "</select></td></tr>";
...
{
   echo "<option";
   if($runnerup==$vbsch[$i]) echo " selected";
   echo ">$vbsch[$i]</option>";
}
echo "</select></td></tr>";
echo "</table></td></tr>";
...
echo "<td width=100  align=center>$<input tabindex=\"600\" type=text class=tiny size=7 name=\"grossreceipts\" id=\"grossreceipts\" value=\"".number_format($grossreceipts,2,'.','')."\" onblur=\"Finance3.Calculate(this.id,this.value);\" readOnly=TRUE></td></tr>";
// Officials
...
// Host Given
//(3 team $75.00, 4 teams $100.00, 5 teams $125.00, 6 teams $150.00)
...
if($type=="District Final" || $type=="Substate")
{
   ...
   echo "<td align=right><b>Host School Allowance&nbsp;&nbsp;#4</b></td>";
   $tab=7;
   echo "<td align=center >$<input type=text tabindex=$tab class=tiny size=7 name=\"hostgiven\" id=\"hostgiven\" value=\"".number_format(50,2,'.','')."\" readOnly=true></td></tr>";
}
else
{
   echo "<tr><td align=left><b>Host School Allowance</b><br>(4 teams $100.00, 5 teams $125.00, 6 teams $150.00)</td>";
   echo "<td align=right><b>6. Host School Allowance&nbsp;&nbsp;</b></td>";
   $tab=7;
   echo "<td align=center bgcolor=>$<input type=text tabindex=0 class=tiny size=7 name=\"hostgiven\" id=\"hostgiven\" value=\"".number_format($hostgiven,2,'.','')."\" onblur=\"Finance3.Calculate(this.id,this.value);\"></td></tr>";
}
echo "<tr><td align=left><b>Host School</b> (25% of #3, Gross Receipts)</td>";
...
if($type=="District Final" || $type=="Substate")
   $max=2;
else if($round=="1" || $class=="A" || $class=="B")
   $max=6;
else
   $max=2;
...
{
   $tab++;
   echo "<tr align=center><td bgcolor=yellow><select tabindex=\"$tab\" id=\"vbschool".$i."\" name=\"vbschool[$i]\"><option value=''>~</option>";
   for($j=0;$j<count($vbsch);$j++)
   {
      //if($vbsch[$j]!=$school)
      //{
         echo "<option";
         if($vbschool[$i]==$vbsch[$j]) echo " selected";
         echo ">$vbsch[$j]</option>";
      //}
   }
   echo "</select></td>";
   ...
   $var="row_".$i."_4";
   if(!$$var) $$var="0.00";
   echo "<td>$<input type=text size=6 name=\"$var\" id=\"$var\" value=\"".$$var."\" readOnly=TRUE></td>";
   $var="row_".$i."_5";
   if(!$$var) $$var="0.00";
...
echo "</tr>";
echo "</table></td></tr>";
echo "<tr align=left valign=top><td width=700><b>Balance for Bonus</b> (#9 minus #10) To be distributed as specified below.<br>";
...
echo "<br />
        ...
        <tr bgcolor='yellow'><td align=right>Number of DAYS Broadcasted:</td><td align=left><input  tabindex=35 type=text size=2 name=\"localmedia_bcasts\" value=\"$localmedia_bcasts\" id=\"localmedia_bcasts\" onBlur=\"CalculateBcasts();\"></td></tr>
        ...
        echo "<tr bgcolor='yellow'><td align=right>Number of DAYS Broadcasted:</td><td align=left><input type=text tabindex=36 size=2 name=\"othermedia_wcasts\" value=\"$othermedia_wcasts\" id=\"othermedia_wcasts\" onBlur=\"CalculateBcasts();\"></td></tr>";
        ...
        echo "<tr bgcolor='yellow'><td align=right>Number of DAYS Telecasted (TV):</td><td align=left><input type=text size=2 tabindex=37 name=\"othermedia_tcasts\" value=\"$othermedia_tcasts\" id=\"othermedia_tcasts\" onBlur=\"CalculateBcasts();\"></td></tr>";
