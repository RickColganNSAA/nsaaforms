...
if(!$distid || $distid=='')
{
   ...
   if($level==1)
      echo "<br><br><a href=\"sbindex.php?session=$session\">Return to Financial Reports</a>";
   else
...
$sql="SELECT * FROM $db_name2.sbdistricts WHERE id='$distid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
...
{
   echo $init_html;
   echo "<table><tr align=center><th><br><br>ERROR: $type $class-$district is NOT a Softball District.<br><br>";
   if($level==1)
      echo "<a href=\"sbindex.php?session=$session\">Return to Financial Reports</a>";
   else
      echo "<a href=\"../welcome.php?session=$session\">Home</a>";
   echo $end_html;
   exit();
...
$sql="SELECT school FROM finance_sb WHERE school='$school2' AND distid='$distid'";
$result=mysql_query($sql);
...
if($hiddenupdate || $update || $submitted==1) //submit report AND/OR show submitted report
{
   ...
   if($hiddenupdate || $update)//if report was submitted, add info to database & show printer-friendly version
   {
      ...
      if($error=='')
      {
         $sql="SELECT * FROM finance_sb WHERE school='$school2' AND distid='$distid'";
         $result=mysql_query($sql);
         $datesub=time();
         $winner2=addslashes($winner); $runnerup2=addslashes($runnerup);
         if(mysql_num_rows($result)==0)    //INSERT, THEN CAN UPDATE
         {
            $sql2="INSERT INTO finance_sb (datesub,school,distid) VALUES ('$datesub','$school2','$distid')";
            $result2=mysql_query($sql2);
         }
         $sql2="UPDATE finance_sb SET nocasts='$nocasts',localmedia_bcasts='$localmedia_bcasts',localmedia_bfee='$localmedia_bfee', othermedia_wcasts='$othermedia_wcasts', othermedia_wfee='$othermedia_wfee', othermedia_tcasts='$othermedia_tcasts', othermedia_tfee='$othermedia_tfee', datesub='$datesub',round='$round',winner='$winner2',runnerup='$runnerup2',attendance='$attendance',grossreceipts='$grossreceipts',offfees='$offfees',offmiles='$offmiles',offmilespaid='$offmilespaid',offtotal='$offtotal',insurance='$insurance',balance='$balance',hostallow='$hostallow',nsaaallow='$nsaaallow',gate='$gate',b_cast='$b_cast' WHERE school='$school2' AND distid='$distid'";
         $result2=mysql_query($sql2);
      }
   }//end if update
   ...
   if($print!=1)
   {
      ...
      {
         echo "<html><head><title>NSAA Home</title><link href=\"/css/nsaaforms.css\" rel=stylesheet type=\"text/css\"></head><body onload=\"window.open('sbfinance.php?session=$session&distid=$distid&print=1&school_ch=$school_ch');\">";
      }
      else echo $init_html;
      echo GetHeader($session);
      echo "<br>";
      if($level==1)
         echo "<a href=\"sbindex.php?session=$session\" class=small>Return to Financial Reports</a>&nbsp;&nbsp;&nbsp;";
      echo "<a href=\"sbfinance.php?session=$session&distid=$distid&print=1&school_ch=$school_ch\" class=small target=new>Printer-Friendly Version</a><br><br>";
   ...
   echo "<table width=700 class=nine><caption><b>NSAA Softball Financial Report</b><hr></caption>";
   ...
   $sql2="SELECT * FROM $db_name2.sbdistricts WHERE id='$distid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
   $dates="";
...
/*    echo "<table width=450 border=1 bordercolor=\"red\" cellspacing=0 cellpadding=5><tr align=left><td class=nine>";
   ...
   echo "<tr align=right><td><table width=450 border=1 bordercolor=\"red\" cellspacing=0 cellpadding=5><tr align=left><td class=nine>";
   ...
   echo "Softball<br>602-13 <u>$space $space $space $space $space $space $space $space $space</u><br><br>";
...
?>
<body onload="Finance2.initialize('<?php echo $session; ?>','sb');">
<?php
echo "<br>";
if($level==1)
   echo "<a href=\"sbindex.php?session=$session\" class=small>Financial Reports Home</a><br><br>";
echo "<form method=post action=\"sbfinance.php\" name=sbform>";
...
echo "<table width=90% cellspacing=2 cellpadding=2 class=nine><caption><b>NSAA SOFTBALL DISTRICT FINANCIAL REPORT</b><hr>";
if($error!='')
{
...
{
   echo "<option";
   if($winner==$sbsch[$i]) echo " selected";
   echo ">$sbsch[$i]</option>";
}
echo "</select></td>";
...
{
   echo "<option";
   if($runnerup==$sbsch[$i]) echo " selected";
   echo ">$sbsch[$i]</option>";
}
echo "</select></td>";
