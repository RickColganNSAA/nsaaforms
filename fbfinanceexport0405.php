...
for($rnd=1;$rnd<5;$rnd++)
{
   $csv="";
    ...
    for($cls=$cl0;$cls<count($fbclass);$cls++)
    {
	  ...
	  $game[$cls][pro][$ix]=number_format($row[13]/$row[17],2,'.',''); //$row[18];
   ...
   $filename2=ereg_replace("csv","txt",$filename);
 ...
 citgf_makepublic("financialreports/$filename");
   citgf_copy("financialreports/$filename","financialreports/$filename2");
   echo "<a href=\"financialreports/$filename\" target=new>$filename</a><br>";
   echo "<a href=\"financialreports/$filename2\" target=new>$filename2</a><br>";
