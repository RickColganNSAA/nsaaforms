<?php
//state_cc_g_edit.php: district hosts submit their individual and team state qualifiers
...
if($store=="Save & Keep Editing" || $store=="Save & View Form" || $hiddensave)
{
   //submit information to database
   //Individuals:
   $sql="DELETE FROM cc_g_state_indy WHERE district_id='$dist'";
   $result=mysql_query($sql);
   for($i=0;$i<15;$i++)
   {
      ...
      {
	 $place=$i+1; 
         $sql="INSERT INTO cc_g_state_indy (district_id, sid, student_id, place,finishtime) VALUES ('$dist','".$$schvar."','".$$indyvar."','$place','".$time."')";
	 $result=mysql_query($sql);
      }
   }
   //Qualifying Teams:
   $sql="DELETE FROM cc_g_state_team WHERE district_id='$dist'";
   $result=mysql_query($sql);
   for($i=1;$i<=3;$i++)
   {
      $students=""; $times="";
	 ...
	 if($check[$i][$j]=='y')
	 {
      ...
      $sql="INSERT INTO cc_g_state_team (district_id, sid, student_ids, place, score,finishtimes) VALUES ('$dist','$team[$i]','$students','$i','$score[$i]','$times')";
      $result=mysql_query($sql);
   }
   for($i=4;$i<=15;$i++)
   { 
      $sql="INSERT INTO cc_g_state_team (district_id, sid, place, score) VALUES ('$dist','$team[$i]','$i','$score[$i]')";
      $result=mysql_query($sql);
   }
   if($store=="Save & View Form" || $final=='y')
   {
      header("Location:state_cc_g_view.php?session=$session&school_ch=$school_ch&final=$final&dist_select=$dist");
      exit();
   }
}
echo $init_html_ajax;
...
<body onload="Team2.initialize('<?php echo $session; ?>','ccg','sch','indy','15');">
<?php
echo GetHeader($session);

//get district info
$sql="SELECT * FROM $db_name2.ccgdistricts WHERE id='$dist'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$schoollist=$row[schools];
echo "<br><table width=95%><!--Table of Tables-->";
echo "<caption><b>GIRLS CROSS-COUNTRY DISTRICT RESULTS</b><br>";
echo "<a class=small href=\"state_cc_b_edit.php?session=$session\">Boys Cross-Country District Results</a></caption>";
...
echo "<form method=post action=\"state_cc_g_edit.php\">";
...
for($i=0;$i<count($schnames);$i++)
{
   ...
   $sql="SELECT * FROM ccgschool WHERE school='$cur' ORDER BY school";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
...
$sql="SELECT * FROM cc_g_state_indy WHERE district_id='$dist' ORDER BY place";
$result=mysql_query($sql);
$ix=0;
...
for($i=0;$i<15;$i++)
{
   ...
   if($$var)
   {
      $sql="SELECT * FROM ccgschool WHERE sid='".$$var."'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
      ...
      $sql2=substr($sql2,0,strlen($sql2)-1).") AND t1.cc='x' AND t1.gender='F' ORDER BY t1.school,t1.last,t1.first,t1.middle";
...
$sql2="SELECT * FROM ccgschool WHERE sid='$team[$x]'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
$sql="SELECT t2.id,t2.last,t2.first,t2.middle,t2.semesters FROM eligibility AS t2, headers AS t3 WHERE t2.school=t3.school AND t2.cc='x' AND t2.gender='F' AND (t3.id='$row2[mainsch]'";
...
{
   $submitted=0;
   for($j=0;$j<count($checks[$x]);$j++)	//Check if student has already been saved on this team
   {
      if($checks[$x][$j]==$row[0])	
      {
	 $submitted=1;
   ...
   {
      $sql2="SELECT * FROM cc_g_state_indy WHERE student_id='$row[id]'";
      $result2=mysql_query($sql2);
...
for($i=1;$i<=3;$i++)
{
   echo "<tr align=center>";
   echo "<th>$i</th>";
   $sql2="SELECT school FROM ccgschool WHERE sid='$team[$i]'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
