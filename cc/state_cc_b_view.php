<?php
//state_cc_b_view.php: view submitted qualifiers (dist host only)
...
if($count>0 && !$dist_select && $print!=1)
{
   ...
   $sql="SELECT id,class,district FROM $db_name2.ccbdistricts WHERE hostschool='$school2' ORDER BY class,district";
   $result=mysql_query($sql);
   while($row=mysql_fetch_array($result))
   {
      echo "<a href=\"state_cc_b_view.php?session=$session&school_ch=$school_ch&dist_select=$row[0]\">District $row[class]-$row[district]</a><br>";
   }
   exit();
}

//check to see if anything has been submitted yet:
$sql="SELECT * FROM cc_b_state_indy WHERE district_id='$dist'";
$result=mysql_query($sql);
$ct=mysql_num_rows($result);
$sql="SELECT * FROM cc_b_state_team WHERE district_id='$dist'";
$result=mysql_query($sql);
$ct2=mysql_num_rows($result);
$total=$ct+$ct2;
if($total==0)
{
   header("Location:state_cc_b_edit.php?session=$session&school_ch=$school_ch&dist_select=$dist");
   exit();
}
...
if($print!=1)
{
   echo "<a href=\"state_cc_b_view.php?session=$session&school_ch=$school_ch&dist_select=$dist&print=1\" class=small target=new>Printer/E-mail Friendly Version</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"state_cc_b_edit.php?session=$session&school_ch=$school_ch&dist_select=$dist\" class=small>Edit this Form</a><br><br>";
}//end if not print
...
$info.="</tr>";
$sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters, t2.gender FROM cc_b_state_indy AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND t1.district_id='$dist' ORDER BY t1.place";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result))
{
   $co_op=0;
   ...
   $sid=$row[sid];
   //get student's coach
   $sql2="SELECT t1.name,t3.school FROM logins AS t1, headers AS t2, ccbschool AS t3 WHERE t1.school=t2.school AND t2.id=t3.mainsch AND t1.sport='Boys Cross-Country' AND t3.sid='$sid'";
   $result2=mysql_query($sql2);
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT * FROM ccbschool WHERE sid='$row[sid]'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
   $sid=$row2[sid];
...
if($print!=1)
{
   ...
   echo "<a href=\"state_cc_b_view.php?session=$session&school_ch=$school_ch&dist_select=$dist&print=1\" class=small target=new>Printer/E-mail Friendly Version</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"state_cc_b_edit.php?session=$session&school_ch=$school_ch&dist_select=$dist\" class=small>Edit this Form</a>&nbsp;&nbsp;&nbsp;";
...
if($final=='y')
{
   ...
   $sql="UPDATE $db_name2.ccbdistricts SET submitted_b='x' WHERE id='$dist'";
   $result=mysql_query($sql);
   /*
   for($i=0;$i<count($qualify[0]);$i++)
   {
      $sql="SELECT school FROM ccbschool WHERE sid='".$qualify[1][$i]."'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
      $school=$row[school];
      ...
      $row=mysql_fetch_array($result);
      $year=GetYear($row[semesters]);
      //$teamcode=GetTeamCode($qualify[1][$i],'cc_b'); //generate code for co-op
      $cursid=$qualify[1][$i];
 ...
 citgf_makepublic($filename);
   ...
   $To=$cc_email;
   $ToName="NSAA";
   $Subject="$class_dist Boys Cross-Country State Qualifiers";
   $Text="Attached is the semicolon-delimited Hytek file for the $class_dist Boys Cross-Country District State Qualifier Info.  Thank you!";
   $Html="<font size=2 family=arial>Attached is the semicolon-delimited Hytek file for the $class_dist Boys Cross-Country District State Qualifier Info.<br><br>They have approved this as their final submission.<br><br>Thank you!</font>";
   $AttmFiles=array($filename);
	*/
   $Subject="$school has submitted Boys Cross-Country District Results";
