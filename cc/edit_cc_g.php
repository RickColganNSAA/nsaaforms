<?php
//edit_cc_g.php: Girls Cross-Country entry form
...
$sql="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Girls Cross-Country'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
...
if(PastDue($duedate,0) && $level!=1)
{
   ...
   $sql="SELECT * FROM cc_g WHERE school='$school2'";
   $result=mysql_query($sql);
   $ct=mysql_num_rows($result);
   if($ct>0)
   {
      echo "<a href=\"view_cc_g.php?session=$session&school_ch=$school_ch\">";
...
$sql="SELECT * FROM cc_g WHERE school='$school2' OR co_op='$school2'";
$result=mysql_query($sql);
$ix=0;
...
$sql="SELECT choices FROM classes_districts WHERE sport='cc_g'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$class_array=$row[0];
...
?>
<form method=post action="submit_cc_g.php">
<input type=hidden name=session value=<?php echo $session; ?>>
<input type=hidden name=school_ch value="<?php echo $school_ch; ?>">
<table width=80%>
<tr align=center>
<th>GIRLS CROSS-COUNTRY DISTRICT ENTRY</th>
</tr>
<tr align=center>
...
<?php
   //get Girls cross-country participants from eligibility table in db
   $ix=0;
