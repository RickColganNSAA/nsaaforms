<?php
//view_cc_g.php: Show submitted district
//   entry info.  If none have been submitted,
//   redirect to edit_cc_g.php
...
$sql="SELECT class_dist FROM cc_g WHERE school='$school2'";
$result=mysql_query($sql);
$class_dist="";
...
$sql="SELECT duedate FROM form_duedates WHERE form='cc_g'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$duedate=$row[0];
$date=split("-",$duedate);
...
{
   if($director!=1)
      header("Location:edit_cc_g.php?session=$session&school_ch=$school_ch");
   else
...
$colors=$row2[5];
$mascot=$row2[6];
$sql2="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Girls Cross-Country'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
...
{
   echo "<br><br>";
   echo "<a class=small href=\"view_cc_g.php?session=$session&school_ch=$school_ch&print=1\" target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
   echo "<a class=small href=\"edit_cc_g.php?session=$session&school_ch=$school_ch\">Edit this Form</a>&nbsp;&nbsp;&nbsp;";
   if($level==1 || $level==2)
...
$string.="<th>GIRLS CROSS-COUNTRY DISTRICT ENTRY</th>";
$string.="</tr>";
$string.="<tr align=center>";
...
$string.="<td>".GetSchoolName($sid,'ccg',date("Y"))." $mascot</td></tr>";
$csv.="School/Mascot:, ".GetSchoolName($sid,'ccg',date("Y"))." $mascot\r\n";
...
if($print!=1)
{
   if(($count>6 && $class_dist!="A")||($count>7))
   {
      ...
      if($count==7 && ($class_dist=='' || !$class_dist))
         echo "<br>If you are a Class A school, make sure to select \"A\" as your class on the <a href=\"edit_cc_g.php?session=$session\">Edit Screen</a>";
      echo "</font>";
      echo "</th></tr>";
   }
?>
<tr align=center>
<td><br>
    <a class=small href="view_cc_g.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>&print=1" target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;
    <a class=small href="edit_cc_g.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>">Edit this Form</a>&nbsp;&nbsp;&nbsp;
