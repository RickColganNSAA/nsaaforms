...
//submit_cc_g.php: submits form info for Girls cross-country.
//  Form is submitted from edit_cc_g.php
...
$asst=ereg_replace("\"","\'",$asst);
$sql="UPDATE logins SET asst_coaches='$asst' WHERE school='$school2' AND sport='Girls Cross-Country' AND level=3";
...
for($i=0;$i<count($id);$i++)
{
     ...
     $sql="SELECT * FROM cc_g WHERE student_id='$id[$i]'";
     $result=mysql_query($sql);
     $update=0;
     ...
     if($update==1)
     {
	$sql="UPDATE cc_g SET checked='$check[$i]', class_dist='$class_dist' WHERE student_id='$id[$i]'";
     }

     //if student has not been submitted yet, use INSERT:
     else if($update==0)
     {
        $sql="INSERT INTO cc_g (student_id, school, checked, class_dist";
...
{
   $sql="UPDATE cc_g SET checked='$coop_check[$i]', class_dist='$class_dist' WHERE student_id='$coop_student[$i]'";
   $result=mysql_query($sql);
}
...
{
   header("Location:view_cc_g.php?session=$session&school_ch=$school_ch");
   exit();
}
?>
