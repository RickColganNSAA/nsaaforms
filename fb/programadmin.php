...
else
{
   ...
   echo "</p></div>";
   echo "</caption>";
   echo "<tr align=center><td><a class=small href=\"programadmin.php?session=$session&sport=$sport&sort=co_op&class=$class\">School</a>";
   if($sort=="co_op") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[co_op];
   $links[$ix]="<a href=\"programpdf.php?school_ch=$row[co_op]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>";
...
for($i=0;$i<count($orders);$i++)
{
   ...
   $sql2="SELECT approvedforprogram,programorder FROM ".$sport."school WHERE sid='$sid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
