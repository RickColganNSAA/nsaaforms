...
{	
	...
	if ( !empty($_FILES) )
	{
		...
		$uniqueFileName = sprintf("%s/fb_teamphoto_%s_1.%s",$pathinfo['dirname'],$sid,$pathinfo['extension']); //$fileName;
		for($k = 1; $k < 100; $k++)
		{
			...
			else
			{
				...
				$uniqueFileName = sprintf("%s/fb_teamphoto_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
                                $uniqueThumbName = sprintf("%s/fb_teamphoto_resized_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
			}
		...
		if ( citgf_moveuploadedfile( $file['tmp_name'], $uniqueFileName ) )
		{
			...
			/*
				...
				$thumbHeight=$thumbWidth/$thumbRatio;
             			if ( $thumbRatio < $imageRatio )        //WIDE PICTURE
             			{
                			$useImageWidth = $imageHeight*$thumbRatio;
                			$useImageHeight = $imageHeight;
                			$imageY=0;
                			$imageX=($imageWidth/2)-($useImageWidth/2);
             			}
             			else    //TALL PICTURE
             			{
                			$useImageWidth = $imageWidth;
                			$useImageHeight = $imageWidth/$thumbRatio;
                			$imageY=($imageHeight/2)-($useImageHeight/2);
                			$imageX=0;
             			}
             			$thumbimage = imagecreatetruecolor((int)$thumbWidth, (int)$thumbHeight);
             			if(@imagecopyresampled($thumbimage, $image, 0, 0, $imageX, $imageY, $thumbWidth, $thumbHeight, $useImageWidth, $useImageHeight))
             			{
			...
			*/
			...
			$sql="UPDATE fbschool SET filename='$useFileName' WHERE sid='$sid'";
