...
else
{
   ...
   echo "<td><b>Form (.csv for Excel)</b></td>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a href=\"view_sb.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>";
...
for($i=0;$i<count($orders);$i++)
{
   ...
   $sql2="SELECT approvedforprogram,programorder FROM sbschool WHERE sid='$sid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
   ...
   echo "<tr align=left><td>".GetSchoolName($sid,$sport)."</td>";
       $activ=$sportname;
	   $activ_lower=strtolower($activ);
	   $sch=ereg_replace(" ","",GetSchoolName($sid,$sport));
	   $sch=ereg_replace("-","",$sch);
	   $sch=ereg_replace("\.","",$sch);
	   $sch=ereg_replace("\'","",$sch);
	   $sch=strtolower($sch);
	   ...
	   $filename="$sch$activ_lower";
	   $filename.="state.csv";
	  echo "<td><a class=small href=\"../attachments.php?session=$session&filename=$filename\">$filename</a></td>";
