<?php
//sbstateadmin.php: NSAA Softball Admin for State Entry Forms
...
if ($classs)
{
header("Location:sbstateadmin.php?session=$session&viewdistrict=1&class=$classs");
exit();
}
if ($export)
{
    ...
    $sql="SELECT DISTINCT t1.school,t2.class,t2.mascot,t2.colors,t2.coach,t2.sid FROM sb AS t1, sbschool AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id "; 
    if($class && $class!='')
...
/*  if($row[mascot]!='') $mascot=$row[mascot];
	...
	$record=GetWinLoss($row[sid],'sb');
	
	$sql_21="SELECT class_dist FROM sb WHERE school='$row[school]'";
	...
	$csv.="Light Jersey #,Dark OR Libero Jersey #,Name,Grade,Position,BA,AB,H,R,RBI,HR,W,L,ERA\r\n";
	
	//$sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM sb_state AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL), t1.libero";
	$sql_11="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM sb AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL)";
    
	$result_11=mysql_query($sql_11); 
	$count=0;
		while($row_11=mysql_fetch_array($result_11))
		{
		  if($row_11[3]=="y")	//that student was checked to be on the roster
		  {
             $year=GetYear($row_11[semesters]);
			 $temp=split("-",$row_11[14]);
			 ...
			 $csv.="$row_11[5],$row_11[6],$row_11[first] $row_11[last],$year,$row_11[7],$row_11[8],$row_11[9],$row_11[10],$row_11[11],$row_11[12],$row_11[13],$temp[0],$temp[1],$row_11[15]\r\n";

			 $count++;
		  }
		}
		$csv.="Games\r\n\"Opponent\",\"W/L\",\"Score\",\"Opp. Score\"\r\n";
		$sched=GetSchedule($row[sid],'sb');
		$gamect=0;
	   ...
	   for($i=0;$i<count($sched[oppid]);$i++)
	   {
	    ...
	    $sql_12="SELECT * FROM sbschool WHERE sid='$row[sid]'";
		$result_12=mysql_query($sql_12);
		$row_12=mysql_fetch_array($result_12);
		
		$sql_13="SELECT name,asst_coaches FROM logins WHERE school='$row[school]' AND sport='Softball'";
...
if ($state_export)
{
    ...
    $sql="SELECT DISTINCT t1.school,t2.class,t2.mascot,t2.colors,t2.coach,t2.sid FROM sb_state AS t1, sbschool AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id AND t1.submitted!='' "; 
    //$sql="SELECT DISTINCT school,submitted FROM $table WHERE submitted!=''";
	if($class && $class!='')
    $sql.=" AND t2.class='$class'";
    //$sql.=" ORDER BY t1.school";
    $sql.=" ORDER BY t2.programorder ASC";
	$result = mysql_query($sql);
	...
	$sql_21="SELECT class_dist FROM sb_state WHERE school='$row[school]'";
	$result_21=mysql_query($sql_21);
	$row_21=mysql_fetch_array($result_21);
	$class_dist=$row_21[0];
	...
	//$sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM sb_state AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL), t1.libero";
	$sql_11="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM sb_state AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL)";
...
if($save)
{
   ...
   for($i=0;$i<count($sid);$i++)
   {
      ...
      {
	 $sql="UPDATE sbschool SET approvedforprogram='".time()."' WHERE sid='$sid[$i]' AND approvedforprogram=0";
         $result=mysql_query($sql);
         $sql="UPDATE sbschool SET programorder='$programorder[$i]' WHERE sid='$sid[$i]'";
         $result=mysql_query($sql);
      }
      else
      {
         $sql="UPDATE sbschool SET approvedforprogram='0' WHERE sid='$sid[$i]'";
         $result=mysql_query($sql);
   ...
   $sql="SELECT DISTINCT programorder FROM sbschool WHERE programorder>0";
   $result=mysql_query($sql);
   $ct1=mysql_num_rows($result);
   $sql="SELECT programorder FROM sbschool WHERE programorder>0";
   $result=mysql_query($sql);
   $ct2=mysql_num_rows($result);
   if($ct1!=$ct2)
   {
      $sql="SELECT programorder,COUNT(programorder) FROM sbschool WHERE programorder>0  AND class='$class' GROUP BY programorder";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
	 if($row[1]>1) 
...
echo "<form method=post action=\"sbstateadmin.php\">";
...
if(mysql_num_rows($result)==0 || $viewdistrict==1)
{
   ...
   if(mysql_num_rows($result)>0) echo "<br>[<a class=small href=\"sbstateadmin.php?session=$session\">View schools who have submitted a STATE form</a>]";
   echo "<br><br>";
   echo "</caption>";
   /////////
   ...
   $sql2="SELECT DISTINCT class FROM sbschool WHERE class!='' ORDER BY class";
   ...
   echo "</select>";
   echo "&nbsp;&nbsp;| &nbsp;<a href=\"sbstateadmin.php?session=$session&class=$class&export=yes\">Download for Excel</a></p></div></td></tr>";   
   /////////
   ...
   $sql="SELECT DISTINCT t1.school,t2.sid,t2.filename FROM $sport AS t1, ".$sport."school AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id ORDER BY t1.school"; 
   //$sql="SELECT DISTINCT t1.school,t2.sid,t2.filename FROM $sport AS t1, ".$sport."school AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id ORDER BY t2.programorder"; 
   $result=mysql_query($sql);
echo mysql_error();
   echo "<tr align=center><td><a class=small href=\"sbstateadmin.php?session=$session&sport=$sport&sort=t1.school\">School</a>";
   if($sort=="t1.school") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td><td><a class=small href=\"sbstateadmin.php?session=$session&sport=$sport&sort=t2.filename%20DESC\">Team Photo</a>";
...
else
{
   ...
   echo "</select>";
   echo "&nbsp;&nbsp;| &nbsp;<a href=\"sbstateadmin.php?session=$session&class=$class&state_export=yes\">Download for Excel</a></p></div></td></tr>";   
   echo "</p></div>";
   echo "<p><a href=\"sbstateadmin.php?session=$session&viewdistrict=1\">Click Here</a> to view the list of schools with District Entry Forms (with the Team Photo column included).</p>";
   echo "</caption>";
   echo "<tr align=center><td><a class=small href=\"sbstateadmin.php?session=$session&sport=$sport&sort=school&class=$class\">School</a>";
   if($sort=="school") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td><td><b>Submitted Roster</b></td><td><a class=small href=\"sbstateadmin.php?session=$session&sport=$sport&sort=submitted%20DESC&class=$class\">Date Submitted</a>";
   if($sort=="submitted DESC") echo "<img style='width:15px;float:right' src='../arrowup.png' border='0'>";
   echo "</td>";
   if($class && $class!='')
   {
      echo "<td><a class=small href=\"sbstateadmin.php?session=$session&sport=$sport&sort=programorder%20ASC&class=$class\">PROGRAM<br>ORDER</a>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a class=small href=\"view_sb.php?school_ch=$row[school]&session=$session\" target=\"_blank\">View Form</a>&nbsp;|&nbsp;<a class=small href=\"view_sb.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>&nbsp;|&nbsp;<a class=small href=\"../attachments.php?session=$session&filename=$filename\">Download for Excel</a>";
   if($statesubmitted==0)
      $links[$ix]="<a class=small href=\"view_sb.php?school_ch=$row[school]&session=$session&viewdistrict=1&makepdf=1\" target=\"_blank\">Preview PDF</a>";
...
for($i=0;$i<count($orders);$i++)
{
   ...
   $sql2="SELECT approvedforprogram,programorder,filename FROM sbschool WHERE sid='$sid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
