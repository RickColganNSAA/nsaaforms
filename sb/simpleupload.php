...
{	
	...
	if ( !empty($_FILES) )
	{
		...
		for($k = 1; $k < 100; $k++)
		{
			...
			else
			{
				...
				$uniqueFileName = sprintf("%s/sb_teamphoto_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
                                $uniqueThumbName = sprintf("%s/sb_teamphoto_resized_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
			}
	...
	*/
			...
			$sql="UPDATE sbschool SET filename='$useFileName' WHERE sid='$sid'";
