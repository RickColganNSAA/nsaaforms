...
if ($classs)
{
header("Location:sbstateadmin1.php?session=$session&viewdistrict=1&class=$classs");
exit();
}
if ($export)
{
	...
	{
	//$row[record]=GetWinLoss($row[sid],'sb');
	unset($row[sid]);
	fputcsv($output, $row);
	}
...
echo "<form method=post action=\"sbstateadmin1.php\">";
...
if(mysql_num_rows($result)==0 || $viewdistrict==1)
{
   ...
   if(mysql_num_rows($result)>0) echo "<br>[<a class=small href=\"sbstateadmin1.php?session=$session\">View schools who have submitted a STATE form</a>]";
   echo "<br><br>";
   echo "</caption>";
   /////////
   ...
   echo "</select>";
   echo "&nbsp;&nbsp;| &nbsp;<a href=\"sbstateadmin1.php?session=$session&class=$class&export=yes\">Download for Excel</a></p></div></td></tr>";   
   /////////
   ...
   $result=mysql_query($sql);
echo mysql_error();
   echo "<tr align=center><td><a class=small href=\"sbstateadmin1.php?session=$session&sport=$sport&sort=t1.school\">School</a>";
   if($sort=="t1.school") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td><td><a class=small href=\"sbstateadmin1.php?session=$session&sport=$sport&sort=t2.filename%20DESC\">Team Photo</a>";
...
else
{
   ...
   echo "</select>";
   echo "</p></div>";
   echo "<p><a href=\"sbstateadmin1.php?session=$session&viewdistrict=1\">Click Here</a> to view the list of schools with District Entry Forms (with the Team Photo column included).</p>";
   echo "</caption>";
   echo "<tr align=center><td><a class=small href=\"sbstateadmin1.php?session=$session&sport=$sport&sort=school&class=$class\">School</a>";
   if($sort=="school") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td><td><b>Submitted Roster</b></td><td><a class=small href=\"sbstateadmin1.php?session=$session&sport=$sport&sort=submitted%20DESC&class=$class\">Date Submitted</a>";
   if($sort=="submitted DESC") echo "<img style='width:15px;float:right' src='../arrowup.png' border='0'>";
   echo "</td>";
   if($class && $class!='')
   {
      echo "<td><a class=small href=\"sbstateadmin1.php?session=$session&sport=$sport&sort=programorder%20ASC&class=$class\">PROGRAM<br>ORDER</a>";
