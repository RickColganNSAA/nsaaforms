...
if($sid2 && $sid2!='')
    $sql0="SELECT * FROM sbschool WHERE (sid='$sid1' OR sid='$sid2') ORDER BY programorder";
else
    $sql0="SELECT * FROM sbschool WHERE sid='$sid1'";
...
while($row0=mysql_fetch_array($result0))
{
    ...
    $sql_coop="SELECT * FROM sbschool WHERE mainsch='$row1[id]' AND (othersch1!=0 OR othersch2!=0 OR othersch3!=0)";
    $result_coop=mysql_query($sql_coop);
    $coop=mysql_fetch_array($result_coop);
    if(!empty($coop))
    {
        if (!empty($coop['othersch1'])){
            $sql_coop1="SELECT girlsenrollment FROM headers WHERE id='$coop[othersch1]'";
            $result_coop1=mysql_query($sql_coop1);
            $row_coop1=mysql_fetch_array($result_coop1);
            $co_op+=(int)$row_coop1[girlsenrollment];
        }

        if (!empty($coop['othersch2'])){
            $sql_coop2="SELECT girlsenrollment FROM headers WHERE id='$coop[othersch2]'";
            $result_coop2=mysql_query($sql_coop2);
            $row_coop2=mysql_fetch_array($result_coop2);
            $co_op+=(int)$row_coop2[girlsenrollment];
        }
        if (!empty($coop['othersch3'])){
            $sql_coop3="SELECT girlsenrollment FROM headers WHERE id='$coop[othersch3]'";
            $result_coop3=mysql_query($sql_coop3);
            $row_coop3=mysql_fetch_array($result_coop3);
            $co_op+=(int)$row_coop3[girlsenrollment];
        }
    }
    //COACH
    ...
    */
    ...
    while($row=mysql_fetch_array($result))
    {
        ...
        /*
          if($row[average]==0 && $row[at_bats]==0) $row[average]="&nbsp;";
            if($row[at_bats]==0) $row[at_bats]="&nbsp;";
            if($row[hits]==0) $row[hits]="&nbsp;";
            if($row[runs_scored]==0) $row[runs_scored]="&nbsp;";
            if($row[runs_batted]==0) $row[runs_batted]="&nbsp;";
            if($row[home_runs]==0) $row[home_runs]="&nbsp;";
            if($row[pitching_record]==0) $row[pitching_record]="&nbsp;";
            if($row[pitching_era]==0) $row[pitching_era]="&nbsp;";
        */
