...
echo "<br><img src=\"/images/NSAAlogo.gif\">";
...
if($hostschool!='' || $site!='') echo "<p><b>Host School (Site):</b> $hostschool ($site)</p>";
...
while($row=mysql_fetch_array($result))
{
   ...
   {
      $sql2="SELECT * FROM $db_name.".$sport."sched WHERE distid='$row[distid]' AND gamenum='$row[gamenum]'";
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
      echo GetSchoolName($row2[sid],$sport)." vs. ".GetSchoolName($row2[oppid],$sport);
   }
   echo "</b></p>";
   $sql2="SELECT t1.first,t1.last,t1.middle FROM officials AS t1,$contracts AS t2 WHERE t1.id=t2.offid AND t2.disttimesid='$row[id]' ORDER BY t1.last,t1.first,t1.middle";
   $result2=mysql_query($sql2);
   echo "<p>";
   while($row2=mysql_fetch_array($result2))
   {
      echo "$row2[first] $row2[middle] $row2[last]<br>";
   }
   echo "</p>"; 
}
...
if($offinfo!='')
   echo "<h3>Officials' Contact Info:</h3>$offinfo";
...
if($level==1 && $edit==1)
{
   echo "<form method=post action='".$sport."showtoad.php'><input type=hidden name='session' value='$session'><textarea cols=80 rows=25 name='body'>$row[body]</textarea><br><input type=submit name='save' value='Save Changes'></form>";
}
else
{
   echo "<p>$row[body]</p>";
...
echo "</div><p><a href=\"javascript:window.close()\">Close</a></p>";
echo $end_html;
?>
