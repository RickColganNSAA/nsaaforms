...
$sql2="SELECT * FROM $db_name2.sptest ORDER BY place";
...
if($submit)
{
   ...
   for($i=0;$i<count($offid);$i++)
   {
      if($check[$i]=='x')
      {
         $sql="SELECT * FROM $db_name2.sp_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 $result=mysql_query($sql);
 	 if(mysql_num_rows($result)==0)
	 {
	    $sql2="INSERT INTO $db_name2.sp_votes (school,ad_coach,district,officialid) VALUES ('$school2','$ad_coach','$dist','$offid[$i]')";
	 }
	 else
	 {
	    $sql2="UPDATE $db_name2.sp_votes SET district='$dist' WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 }
	 $result2=mysql_query($sql2);
	 ...
	 $offlist.="$row[first] $row[last] ($row[city])<br>";
      ...
      {
	 $sql="DELETE FROM $db_name2.sp_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
         $result=mysql_query($sql);
      }
   }
      ...
      echo $init_html;
      echo $header;
      echo "<br><br><table><tr align=left><td><b>You have voted for the following Speech Judges:</td></tr>";
      echo "<tr align=left><td>$offlist</td></tr>";
      echo "<tr align=left><td><b>Thank you for voting!<br><br></b>";
      if($level==1)
         echo "<a href=\"jvote.php?sport=sp&session=$session\">Return to Speech Ballots Admin</a>";
...
$sql="SELECT * FROM $db_name2.sp_votes WHERE school='$school2' AND ad_coach='$ad_coach'";
$result=mysql_query($sql);
if(mysql_num_rows($result)>0 && $level!=1)
{
   echo "<br><br>You have already submitted your ballot for Speech Judges.<br><br>";
   echo "Thank You!<br><br><a href=\"../welcome.php?session=$session\">Home</a>";
   echo $end_html;
   exit();
}
//ELSE
echo "<br><form method=post action=\"vote_sp.php\">";
...
$sql="SELECT t1.last,t1.first,t1.city,t1.id FROM $db_name2.judges AS t1,$db_name2.sptest_results AS t2,$db_name2.spapply AS t3 WHERE t1.id=t2.offid AND t1.id=t3.offid AND t1.payment!='' AND t1.spmeeting='x' AND t2.correct>=$needed AND (t3.state1='x' OR t3.state2='x') ORDER BY t1.last,t1.first";
$result=mysql_query($sql);
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT * FROM $db_name2.sp_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$row[id]'";
   $result2=mysql_query($sql2);
   if(mysql_num_rows($result2)>0) echo " checked";
   echo ">$row[first] $row[last] ($row[city])<br><br>";
