...
while($row0=mysql_fetch_array($result0))
{
   ...
   for($e=0;$e<count($spevents2);$e++)
   {
      ...
      $csv="\"Contestants\",\"Class\",\"Event\",\"Judges\",\"Room\"\r\n";
      while($row=mysql_fetch_array($result))
      {
         ...
         for($i=0;$i<count($studs);$i++)
         {
               ...
               for($j=0;$j<count($dstuds);$j++)
	       {
            ...
            $csv.="\"$studlist\",\"$class\",\"$eventstr\",\"".trim($row[first])." ".trim($row[last])."\",\"$room\"\r\n";
         }
         if(count($studs)==0)	//contestants not known yet
         {
            $csv.="\"\",\"$class\",\"$eventstr\",\"".trim($row[first])." ".trim($row[last])."\",\"$room\"\r\n";
         }
      }
      $open=fopen(citgf_fopen("/home/nsaahome/reports/spstateballotexport".$class.$spevents[$e].".csv"),"w");
      fwrite($open,$csv);
      fclose($open); 
 citgf_makepublic("/home/nsaahome/reports/spstateballotexport".$class.$spevents[$e].".csv");
      echo "$spevents3[$e]:<br><a class=small href=\"reports.php?session=$session&filename=spstateballotexport".$class.$spevents[$e].".csv\">spstateballotexport".$class.$spevents[$e].".csv</a><br><br>";
   }//end for each event
   echo "</td>";
}//end for each class
echo "</tr></table>";
