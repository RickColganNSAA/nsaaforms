...
/***********************************
jconvictions.php
Report of Judges who've marked
...
echo "<a name=\"top\">&nbsp;</a>";
echo "<table class='nine' frame=all rules=all style=\"border:#808080 1px solid;\" cellspacing=0 cellpadding=5><caption><b>Judges with Misdemeanor or Felony Convictions:</b>";
$sql="SELECT * FROM judges WHERE conviction='yes'";
$result=mysql_query($sql);
if(mysql_num_rows($result)==0)	//IF NONE AT ALL
{
   echo "<p>(0 Judges)</p></caption></table>";
   echo "<br><br><a href=\"managejudge.php?session=$session\">Return to Manage Judges</a>&nbsp;&nbsp;&nbsp;";
...
if(mysql_num_rows($result)==1)
   echo "<p>(".mysql_num_rows($result)." Judge)</p>";
else
   echo "<p>(".mysql_num_rows($result)." Judges)</p>";
if(mysql_num_rows($result)>0 || ($lastname && $lastname!=''))
{
   echo "<form method=post action='jconvictions.php'>";
...
echo "<tr align=center><td><b>Judge's Name</b><br>(Click to View/Edit Profile)</td><td><b>Contact Info</b></td><td><b>Explanation of Conviction(s)</b></td></tr>";
$ix=1;
while($row=mysql_fetch_array($result))
{
   echo "<tr align=left valign=top><td><a href=\"edit_judge.php?session=$session&id=$row[id]\" target=\"_blank\">$row[first] $row[last]</a><br>$row[city], $row[state]</td>";
   echo "<td>";
...
echo "<a href=\"managejudge.php?session=$session\">Return to Manage Judges</a>&nbsp;&nbsp;&nbsp;";
echo "<a href=\"welcome.php?session=$session\">Home</a>";
echo $end_html;
