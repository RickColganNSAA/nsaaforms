<?php
/****CONTRACT TO HOST GIRLS TENNIS DISTRICTS****/
...
if($submit)
{
   if($level!=1)
   {
      if($submit=="Submit")
      {
		...
		/*
            $datech="";
            for($i=0;$i<count($te_ghostdates);$i++)
            {
               if($day[$i])
                  $datech.=$day[$i]."/";
            }
...
// agrees to host the Class $class-$district $sportname District Tournament on May 1, 2, 3, 4, and 5, 2007.  Please complete as soon as possible:</b><br><br>";
   ...
   for($i=0;$i<count($te_ghostdates2);$i++)
   {
      echo "<input type=checkbox name=\"day[$i]\" value=\"$te_ghostdates2[$i]\"> $te_ghostdates[$i]<br>";
   }
   echo "</td></tr>";
	*/
