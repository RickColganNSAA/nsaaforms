...
echo "<form name=\"test_form\" method=post action=\"swtest_update.php\">";
...
echo "<table width=90%>";
echo "<caption><b>Swimming & Diving Rules Examination</b><br>";
if(GetLevel($session)==1)
{
   echo "for <font style=\"color:red\"><b>".GetOffName($offid)."</b></font><br>";
}
$date=split("-",GetTestDueDate("sw"));
$duedate=date("F d, Y",mktime(0,0,0,$date[1],$date[2],$date[0]));
echo "Due $duedate</caption>";
echo "<tr align=left><td align=left colspan=2>Copyrighted and Published by the National Federation of State High School Associations<br><br><i><b>Instructions:</b> Indicate whether you think each statement is true or false by checking the correct circle.</i></td></tr>";
echo "<tr align=center><td align=center colspan=2><hr><table>";
$sql="SELECT category FROM swtest_categ WHERE id='$categid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$category=$row[0];
echo "<tr align=left><th align=left colspan=2>$category:</th></tr>";
//get answers already entered by this official
$sql="SELECT * FROM swtest_results WHERE offid='$offid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
for($i=1;$i<=100;$i++)
{
...
$sql="SELECT question,place FROM swtest WHERE category='$categid' ORDER BY place";
$result=mysql_query($sql);
$ix=0;
