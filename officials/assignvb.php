...
<body onload="OffAssign.initialize('<?php echo $session; ?>','vb');">
<?php
...
echo "<form name=assignform method=post action=\"assignvb.php#timeslots\">";
...
echo "<table";
if(!$type || $type=='~' || !$distid || $distid=='~' || $type=="District Final")
   echo " width=90%";
echo "><caption><b>";
echo "<a class=small target=new href=\"vbcontract.php?session=$session&sample=1\">Sample District Volleyball Contract</a>&nbsp;&nbsp;";
echo "<a class=small target=new href=\"vbstatecontract.php?session=$session&sample=1\">Sample State Volleyball Contract</a><br><br>";
if($sport && $sport!="~")
{
   echo "Choose Type:&nbsp;";
...
if($sport && $sport!='~' && $type && $type!="~" && ($type=="State" || ($distid && $distid!='~')))
{
   ...
   while($row2=mysql_fetch_array($result2))
   {
      ...
      $sql3="SELECT id,time,gamenum FROM $disttimes WHERE distid='$distid' AND day='$row2[day]' ORDER BY gamenum";
      $result3=mysql_query($sql3);
      $curct=0;
      while($row3=mysql_fetch_array($result3))
      {
	 ...
	 $sql="DELETE FROM $contracts WHERE offid='0'";	//delete empty contract slots
         $result=mysql_query($sql);
         $sql="SELECT * FROM $contracts WHERE disttimesid='$curdisttimesid' ORDER BY id";
         $result=mysql_query($sql);
         $curct=0;
         if($type=="District Final") $max=2;
	 else $max=3;
         echo "<table>";
         while($row=mysql_fetch_array($result))
         {
	    ...
	    echo "<tr align=left>";
            if($mode=="Switch to Quick Mode" || $hiddenmode=="quick")   //in quick mode
               ...
               echo "<td><input type=text name=\"$varname2\" value=\"[Click to Choose Official]\" onClick=\"window.open('offspick.php?zones=$zones&dates=$dates&andor=$andor&sport=$sport&disttimesid=$curdisttimesid&session=$session&ix=$ix','offspick','resizable=yes,scrollbars=yes');\" readOnly=true class=tiny size=20></td>";
            echo "</tr>";
            $ix++; $curct++;
         }
	 echo "</table></td></tr>";
   ...
   echo "</td></tr>";
   }//end if NOT STATE or DISTRICT FINAL
   else 		//STATE
   {
      ...
      { 
         ...
         $num=$ix+1;
		 if($mode=="Switch to Quick Mode" || $hiddenmode=="quick") //in quick mode
               echo "$num.<table cellspacing=0 cellpadding=0><tr align=left><td><input class=tiny type=text size=25 name=\"$varname2\" id=\"$varname2\" value=\"".GetOffName($row3[offid])."\" onkeyup=\"OffAssign.lookupOffs('$varname2',this.value);\"></td></tr><tr align=left><td><div style=\"display:none;\" class=\"searchresults\" id=\"".$varname2."List\" name=\"".$varname2."List\"></div></td></tr></table>";
            else
               echo "$num.<input type=text name=\"$varname2\" value=\"".GetOffName($row3[offid])."\" onClick=\"window.open('offspick.php?zones=$zones&dates=$dates&andor=$andor&sport=$sport&disttimesid=$curdisttimesid&session=$session&ix=$ix','offspick','resizable=yes,scrollbars=yes');\" readOnly=true class=tiny size=20>";
	     */
         //echo "$num. <input type=text name=\"$varname2\" value=\"".GetOffName($row3[offid])."\" onClick=\"window.open('offspick.php?zones=$zones&dates=$dates&andor=$andor&sport=$sport&disttimesid=$stateid&session=$session&ix=$ix','offspick','resizable=yes,scrollbars=yes');\" readOnly=true class=tiny size=25>";
         $num=$ix+1;
		 $varname2="offname".$ix; $varname1=$varname2."id";
	    echo "<input type=hidden name=\"timeid[$ix]\" value=\"$curdisttimesid\">";
	    ...
	    //echo "<tr align=left>";
		 ...
		 echo "<br>";
         if(($ix+1)%15==0) echo "</td>";
         $ix++;
      }
      $max=45; 
      if($ix==$max)	//always have at least 5 spots
         $max=$ix+5;
      while($ix<$max)
      {
         if($ix%15==0) echo "<td>";
	    ...
	    //echo "<tr align=left>";
            if($mode=="Switch to Quick Mode" || $hiddenmode=="quick")   //in quick mode
               ...
               echo "$num.<input type=text name=\"$varname2\" value=\"[Click to Choose Official]\" onClick=\"window.open('offspick.php?zones=$zones&dates=$dates&andor=$andor&sport=$sport&disttimesid=$curdisttimesid&session=$session&ix=$ix','offspick','resizable=yes,scrollbars=yes');\" readOnly=true class=tiny size=20>";
            
         //echo "$num. <input type=text name=\"$varname2\" value=\"[Click to Choose Official]\" onClick=\"window.open('offspick.php?zones=$zones&dates=$dates&andor=$andor&sport=$sport&disttimesid=$stateid&session=$session&ix=$ix','offspick','resizable=yes,scrollbars=yes');\" readOnly=true class=tiny size=25>";
         echo "<br>";
         if(($ix+1)%15==0) echo "</td>";
         $ix++;
      }
