   ...
   while($row2=mysql_fetch_array($result2))
   {
         ...
         else
	 {
	    ...
	    echo "<input type=hidden name=\"timeid[$ix]\" value=\"$curdisttimesid\">";
            ...
            echo "<input type=text name=\"$varname2\" value=\"".GetOffName($row[offid])."\" onClick=\"window.open('offspick.php?zones=$zones&dates=$dates&andor=$andor&sport=$sport&disttimesid=$curdisttimesid&session=$session&ix=$ix','offspick','resizable=yes,scrollbars=yes');\" readOnly=true class=tiny size=20>";
	    echo "<br>";
            $curct++; $ix++;
         }
         while($curct<$max)
