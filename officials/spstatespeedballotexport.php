...
while($row0=mysql_fetch_array($result0))
{
   ...
   for($e=0;$e<count($spevents2);$e++)
   {
      ...
      $csv="\"Event\",\"Room\",\"Judge\",\"Code 1\",\"Student 1\",\"Code 2\",\"Student 2\",\"Code 3\",\"Student 3\",\"Code 4\",\"Student 4\",\"Code 5\",\"Student 5\",\"Code 6\",\"Student 6\"\r\n";
	 ...
	 $studlist=array(); $codelist=array(); 
         for($i=0;$i<count($studs);$i++)
         {
            if(ereg(",",$studs[$i]))	//DRAMA or DUET
            {
               ...
               for($j=0;$j<count($dstuds);$j++)
	       {
	          ...
	          if($codelist[$i]=="") $codelist[$i]=GetStateSpeechCode($row4[sid],$spevents[$e],$studs[$i]);
	          if(ereg("[(]",$row3[first]))
	          {
	             $temp=split("[(]",$row3[first]);
	             $nickname=$temp[1];
	             $nickname=ereg_replace("[)]","",$nickname);
	             $studlist[$i].="$nickname $row3[last], ";
	          }
	          else 
	             $studlist[$i].="$row3[first] $row3[last], ";
	       }
               $studlist[$i]=substr($studlist[$i],0,strlen($studlist[$i])-2);
            }
            else if($studs[$i]!='')
            { 
	       ...
	       $codelist[$i]=$row4[sid];
            ...
            //$csv.="\"$studlist\",\"$class\",\"$eventstr\",\"$row[first] $row[last]\",\"$room\"\r\n";
         }
	 $csv.="\"$eventstr\",\"$room\",\"$row[first] $row[last]\",";
	 for($i=0;$i<count($studlist);$i++)
         {
	    if($studlist[$i]!='')
	    {
	       $csv.="\"".$codelist[$i]."\",\"".$studlist[$i]."\",";
	    }
	 }
	 $csv.="\r\n";
 ...
 citgf_makepublic("/home/nsaahome/reports/spstatespeedballotexport".$class.$spevents[$e].".csv");
      echo "$spevents3[$e]:<br><a class=small href=\"reports.php?session=$session&filename=spstatespeedballotexport".$class.$spevents[$e].".csv\">spstatespeedballotexport".$class.$spevents[$e].".csv</a><br><br>";
   }//end for each event
   echo "</td>";
}//end for each class
echo "</tr></table>";
