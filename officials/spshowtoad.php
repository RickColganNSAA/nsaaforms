...
$sql="SELECT t1.first,t1.middle,t1.last,t1.homeph,t1.workph,t1.cellph,t1.email,t2.* FROM judges AS t1,$contracts AS t2 WHERE t2.distid='$id' and t2.offid=t1.id AND t2.post='y' ORDER BY t1.last, t1.first, t1.middle";
$result=mysql_query($sql);
$totalposted=mysql_num_rows($result);
...
echo "<tr align=left><td>$accepted of $totalposted judges have accepted contracts to work the $type $class-$dist $sportname contest you are hosting.</td></tr>";
...
while($row=mysql_fetch_array($result))
{
   ...
   if($row[email]!="")
   {
      echo "E-mail:&nbsp;<a href=\"mailto:$row[email]\" class='small'>$row[email]</a><br>";
   }
   echo "<br>Needs Mileage? <b><u>$row[mileage]</u></b>";
   echo "</td>";
   if($sport=='sp')
   {
      echo "<td>";
