...
if($print!=1 && $level!=3)
{
   echo "<form method=post action=\"sobassignreport.php\">";
...
if($type=='state')
{
   echo "&nbsp;<a href=\"sogassignreport.php?database=$database&session=$session&type=state&gender=Girls\">Girls</a>";
}
...
else if($type=='state')
{
   ...
   $result=mysql_query($sql);
   echo "<tr align=center><td align=left><a class=small href=\"sobassignreport.php?sport=$sport&session=$session&type=state&confirmed=$confirmed\">Official</a></td>";
   if($level==1) echo "<td><b>SSN</b></td>";
   echo "<td><b>City</b></td>";
   if($level!=3)
   {
      echo "<td><a class=small href=\"sobassignreport.php?sport=$sport&session=$session&type=state&confirmed=$confirmed&sort=post\">Posted</a></td>";
      echo "<td><a class=small href=\"sobassignreport.php?sport=$sport&session=$session&type=state&confirmed=$confirmed&sort=accept\">Accept</a></td>";
      echo "<td><a class=small href=\"sobassignreport.php?sport=$sport&session=$session&type=state&confirmed=$confirmed&sort=confirm\">NSAA</a></td>";
   }
   echo "<td><a class=small href=\"sobassignreport.php?sport=$sport&session=$session&type=state&confirmed=$confirmed&sort=standby\">Stand-By</a></td></tr>";
   while($row=mysql_fetch_array($result))
   {
...
if($type=="abc" && $print!=1)
{
   echo "<a class=small href=\"sobassignreport.php?session=$session&sport=$sport\">View DISTRICT Assignments in Class/District Order</a>&nbsp;&nbsp;";
   echo "<a class=small href=\"sobassignreport.php?session=$session&sport=$sport&type=state\">View STATE Assignments</a>&nbsp;&nbsp;";
}
else if($type=='state' && $print!=1)
{
   echo "<a class=small href=\"sobassignreport.php?session=$session&sport=$sport\">View DISTRICT Assignments</a>&nbsp;&nbsp;";
}
elseif($print!=1)
{
   echo "<a class=small href=\"sobassignreport.php?session=$session&sport=$sport&type=abc\">View DISTRICT Assignments in ABC Order</a>&nbsp;&nbsp;";
   echo "<a class=small href=\"sobassignreport.php?session=$session&sport=$sport&type=state\">View STATE Assignments</a>&nbsp;&nbsp;";
}
if($print!=1)
