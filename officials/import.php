...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="INSERT INTO fboff (offid,class,suptestdate,mailing,years) VALUES ('$curid','$row[class]','$row[suptestdate]','$row[mailing]','$row[years]')";
   $result2=mysql_query($sql2);
...
while($row=mysql_fetch_array($result))
{
   ...
   for($i=1;$i<=7;$i++)
   {
      ...
      if(trim($regyr)!="")
      {
         $sql2="INSERT INTO fboff_hist (offid,regyr,appdate,contest,rm,obtest,suptest,class) VALUES ('$curid','$fall-$spring','$newappdate','$row[$ix3]','$row[$ix4]','$row[$ix5]','$row[$ix6]','$row[$ix7]')";
         $result2=mysql_query($sql2);
      }
   }
}
