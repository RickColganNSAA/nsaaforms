...
echo "<center><br>";
echo "<form name=\"test_form\" method=post action=\"trtest_update.php\">";
...
echo "<table width=90%>";
echo "<caption><b>$thisyear Track & Field Rules Examination - Part I</b><br>";
if(GetLevel($session)==1)
{
   echo "for <font style=\"color:red\"><b>".GetOffName($offid)."</b></font><br>";
}
$date=split("-",GetTestDueDate("tr"));
...
$sql="SELECT category FROM trtest_categ WHERE id='$categid'";
...
$sql="SELECT question,place FROM trtest WHERE category='$categid' ORDER BY place";
$result=mysql_query($sql);
$ix=0;
