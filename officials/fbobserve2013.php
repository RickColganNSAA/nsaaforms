...
if($dbname!=$db_name2 && substr(preg_replace("/[^0-9]/","",$dbname),0,4)<2012)
{
...
if($submit && $submit!="Go")	//put eval in db and show user what he/she entered
{
   ...
   {
      $sql2="INSERT INTO fbobserve (obsid,offid,gameid,home,visitor,homescore,visitorscore,site,refereename,umpirename,linesmanname,linejudgename,backjudgename, ";
      for($i=1;$i<=10;$i++)
	 $sql2.="weather".$i.", ";
      $sql2.="weather,";
      for($i=1;$i<=3;$i++)
	 ...
	 $sql2.="psreferee".$i.",psumpire".$i.",pslinesman".$i.",pslinejudge".$i.",psbackjudge".$i.", ";
      ...
      for($i=1;$i<=5;$i++)
	 $sql2.="bthalves".$i.", ";
      $sql2.="bthalves,sidelinebox1,sidelinebox2,sidelinebox,gametempo,recommendations,comments,postseason,postlevelA,postlevelB,postlevelC,postlevelD) VALUES ('$obsid','$offid','$gameid','$home','$visitor','$homescore','$visitorscore','$site','$refereename','$umpirename','$linesmanname','$linejudgename','$backjudgename',";
      for($i=1;$i<=10;$i++)
      {
   ...
   {
      $sql2="UPDATE fbobserve SET home='$home',visitor='$visitor',site='$site',homescore='$homescore',visitorscore='$visitorscore',refereename='$refereename',umpirename='$umpirename',linesmanname='$linesmanname',linejudgename='$linejudgename',backjudgename='$backjudgename',";
      for($i=1;$i<=10;$i++)
      {
...
if(!($print==1 && GetLevel($session)==2) && $sample==0)
{
...
if((GetLevel($session)==1 || !$submitted) && $gameid && $gameid!='new')
...
if($gameid=='new' || $submit=="Go")
{
   ...
   echo "</select>&nbsp;<input type=submit name=submit value=\"Go\"></div>";
}
...
if($print!=1)
{
   ...
   if($submitted==1) echo $refereename."</td></tr>";
   ...
   if($submitted==1) echo $umpirename."</td></tr>";
   ...
   if($submitted==1) echo $linesmanname."</td></tr>";
   ...
   if($submitted==1) echo $linejudgename."</td></tr>";
   ...
   if($submitted==1) echo $backjudgename."</td></tr>";
...
echo "</tr></table><br>";
if($submitted==1)
   echo "Comments: $field</div>";
else
   echo "Comments:<br><textarea style=\"width:700px;height:50px;\" name=field>$field</textarea></div>";
...
echo "> All black shoes (NO WHITE)</td>";
...
echo "> On field 30 minutes prior to kick-off</td>";
...
echo "> Inspect playing field & pylons</td>";
echo "</tr></table><br>";
if($submitted==1)
...
echo "> Captains' backs to sideline</td>";
echo "</tr><tr align=left>";
...
echo "> R's GL - L's side - Outside of hash</td><td width='25%'><input type=checkbox name=\"kreferee2\" value=\"x\"";
...
echo "> R's Free kick line opposite linesman</td><td width='25%'><input type=checkbox name=\"kumpire2\" value=\"x\"";
...
echo "> Move along sidelines - 15 yards</td></tr>";
echo "</table></td></tr>";
        //Linesman
...
echo "> R's 50 yd line - Chain side</td><td width='25%'><input type=checkbox name=\"klinesman2\" value=\"x\"";
...
echo "> Move along sideline - 15 yards</td></tr>";
echo "</table></td></tr>";
        //Line Judge
...
echo "> R's 10 yd line opposite linesman</td><td width='25%'><input type=checkbox name=\"klinejudge2\" value=\"x\"";
...
echo "> K's free kick line - chain side</td><td width='25%'><input type=checkbox name=\"kbackjudge2\" value=\"x\"";
...
echo "> Cover to opposite 45 yd line</td></tr>";
echo "</table></td></tr>";
        //Other Kick off:
echo "<tr align=left><td colspan=3><br>";
echo "<table class='nine' cellspacing=0 cellpadding=3>";
echo "<tr align=left>";
echo "<td width='42%'><input type=checkbox name=\"kickoff1\" value=\"x\"";
...
echo "> Umpire, Backjudge & Linesman - Move up own sideline to clear for play.</td>";
echo "</tr>";
echo "<tr align=left>";
echo "<td width='42%'><input type=checkbox name=\"kickoff3\" value=\"x\"";
...
echo "> Maintain position on sideline at all times (except Referee).</td>";
echo "</tr>";
echo "<tr align=left>";
echo "<td width='42%'><input type=checkbox name=\"kickoff5\" value=\"x\"";
...
echo "</tr>";
echo "<tr align=left>";
echo "<td width='42%'><input type=checkbox name=\"kickoff7\" value=\"x\"";
...
echo "> Umpire - Temporarily move to 9 yd marks/identify free-kick line.</td>";
echo "</tr>";
echo "</table>";
echo "<br>";
...
echo "> 5 yds/5 yds from deepest back</td><td><input type=checkbox name=\"psreferee3\" value=\"x\"";
...
echo "> Establish LOS with upfield foot</td></tr>";
	//Line Judge
...
echo "> No deeper than 20 yds off LOS</td><td><input type=checkbox name=\"psbackjudge2\" value=\"x\"";
...
echo "> Stay parallel to the runner</td><td><input type=checkbox name=\"slinesmanrun2\" value=\"x\"";
if($slinesmanrun2=='x') echo " checked";
echo "> Squeeze & clean up backside (To hash)</td><td><input type=checkbox name=\"slinesmanrun3\" value=\"x\"";
...
echo "> Move down the sideline as required</td><td><input type=checkbox name=\"slinesmanpass2\" value=\"x\"";
if($slinesmanpass2=='x') echo " checked";
echo "> Squeeze & clean up backside (To hash)</td><td><input type=checkbox name=\"slinesmanpass3\" value=\"x\"";
...
echo "> Stay parallel to the runner</td><td><input type=checkbox name=\"slinejudgerun2\" value=\"x\"";
if($slinejudgerun2=='x') echo " checked";
echo "> Squeeze & clean up backside (To hash)</td><td><input type=checkbox name=\"slinejudgerun3\" value=\"x\"";
...
echo "> Move down the sideline as required</td><td><input type=checkbox name=\"slinejudgepass2\" value=\"x\"";
if($slinejudgepass2=='x') echo " checked";
echo "> Squeeze & clean up backside (To hash)</td><td><input type=checkbox name=\"slinejudgepass3\" value=\"x\"";
...
echo "> 4-7 yards deep on LJ side&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=checkbox name=\"pumpire2\" value=\"x\"";
...
echo "> Pivot to view LJ side then cover</td></tr>";
echo "</table></td></tr>";
	//Linesman
echo "<tr align=left><td colspan=3>&nbsp;&nbsp;&nbsp;<b>Linesman:</b><br>";
echo "<table class='nine' cellspacing=0 cellpadding=3 style=\"margin-left:25px;width:700px;\">";
...
echo "> Communicate with Umpire and Linesman. (Record down, distance, yard line)</p>";
echo "<h5>Umpire</h5>";
echo "<p style='margin-left:30px;'><input type=checkbox name=\"btpumpire\" value=\"x\"";
if($btpumpire=='x') echo " checked";
echo "> Quickly take ball to corresponding point on other half of field and reverse directions.</p>";
echo "<h5>Linesman</h5>";
echo "<p style='margin-left:30px;'><input type=checkbox name=\"btplinesman\" value=\"x\"";
if($btplinesman=='x') echo " checked";
echo "> Reset chain crew, then observe team on your sideline.</p>";
echo "<h5>Line Judge</h5>";
...
if($submitted!=1 && $print!=1)
{
   ...
   echo "<input type=submit name=submit class='fancybutton' value=\"Save & Keep Editing\">";
   echo "<br><br><font style=\"color:blue\"><b>NOTE:</b> Once you click \"Submit Evaluation\", your submission of this evaluation is final.  YOU MUST CLICK \"Submit Evaluation\" FOR THE EVALUATION TO BE SENT!!!<br>You will be able to view your submitted evaluations,but you will NOT be able to edit them.<br><input type=submit name=submit class='fancybutton2' value=\"Submit Evaluation\">";
}
else if($edit==1 && $level==1)
{
   echo "<input type=submit name=submit class='fancybutton' value=\"Save Changes\">";
}
if($sample==0)
   echo "</form>";
echo "<br><br>";
