...
if($submit)
{
   ...
   for($i=0;$i<count($offid);$i++)
   {
      if($rank[$i]!='none')
      {
         $sql="SELECT * FROM $db_name2.bbg_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 $result=mysql_query($sql);
 	 if(mysql_num_rows($result)==0)
	 {
	    $sql2="INSERT INTO $db_name2.bbg_votes (school,ad_coach,district,officialid,rank) VALUES ('$school2','$ad_coach','$dist','$offid[$i]','$rank[$i]')";
	 }
	 else
	 {
	    $sql2="UPDATE $db_name2.bbg_votes SET district='$dist',rank='$rank[$i]' WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 }
	 $result2=mysql_query($sql2);
	if(mysql_error()) { echo "$sql2<br>"; exit(); }
	 ...
	 $offlist.="<br>";
      ...
      {
	 $sql="DELETE FROM $db_name2.bbg_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
         $result=mysql_query($sql);
      }
   }
   ...
   echo "<br><br><table><tr align=left><td><b>You have voted for the following Girls Basketball officials:</td></tr>";
   echo "<tr align=left><td>$offlist</td></tr>";
   echo "<tr align=left><td><b>Thank you for voting!<br><br></b>";
   if($level==1)
      echo "<a href=\"vote.php?sport=bbg&session=$session\">Return to Girls Basketball Ballots Admin</a>";
...
$sql="SELECT * FROM $db_name2.bbg_votes WHERE school='$school2' AND ad_coach='$ad_coach'";
$result=mysql_query($sql);
if(mysql_num_rows($result)>0 && $level!=1)
{
   echo "<br><br>You have already submitted your ballot for Girls Basketball Officials.<br><br>";
   echo "Thank You!<br><br><a href=\"../welcome.php?session=$session\">Home</a>";
   echo $end_html;
   exit();
}
//ELSE
echo "<form method=post action=\"vote_bbg.php\">";
...
echo "Girls Basketball Officials Ballot:</b>";
...
$database=$db_name2;
//$sql="SELECT t1.id,t1.first,t1.last,t1.city,t1.photofile,t1.photoapproved FROM $db_name2.officials AS t1, $db_name2.bboff AS t2, $db_name2.bbapply AS t3 WHERE t1.id=t2.offid AND t2.offid=t3.offid AND t2.class='C' AND t3.date11='x' AND t3.date12='x' AND t3.date13='x' ORDER BY t1.last,t1.first";
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT * FROM $db_name2.bbg_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$row[id]'";
   $result2=mysql_query($sql2);
...
echo "</table>";
echo "</form>";
if($level==100) echo "<a href=\"welcome.php?obssport=bbg&session=$session\">Return Home</a>";
echo $end_html;
