...
else if($submit=="Submit Test Answers")
{
   ...
   {
      $sql2="INSERT INTO ".$sport."off_hist (offid,regyr,obtest,obtestattempts) VALUES ('$offid','$regyr','$score','0')";
   }
   else         //UPDATE
   {
   ...
   if(GetLevel($session)!=1 && $score<80 && $obtestattempts<3)        //FAILED - CAN RETAKE --  && $offid!='3427')
   {
        ...
        <p>You may <a href=\"ditest.php?session=$session&retake=$resultsid\">Re-Take this Test</a> up to 3 times in order to pass it before the due date has passed.</p>".$end_html;
