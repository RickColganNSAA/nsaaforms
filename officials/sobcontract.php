...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE sobcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE district='1'";
   $result=mysql_query($sql);
}
...
$sql="SELECT * FROM sobcontracttext WHERE district='$district'";
$result=mysql_query($sql);
...
echo "<form method=post action=\"sobcontract.php\">";
...
if($edit==1)
   echo "<a class=small href=\"sobcontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1 && $level==1)
   echo "<a class=small href=\"sobcontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($accept!='y' && $accept!='n' && ($level!=1 || $sample==1))
{
   ...
   if($edit==1)
   {
      ...
      echo $text4; //"I, as an independent contractor, accept the above agreement for the $thisyear $type Boys Soccer Tournament.";
   echo "</td></tr>";
...
{
   echo "<tr align=left><td><br><br><input type=radio name=confirm value='y'>&nbsp;&nbsp;&nbsp;The NSAA confirms the above agreement for the $thisyear $type Boys Soccer Tournament";
   echo ".</td></tr>";
