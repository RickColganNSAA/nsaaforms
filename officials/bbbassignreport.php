...
if($print!=1)
{
   ...
   if($type=="abc")
      echo "&nbsp;&nbsp;|&nbsp;&nbsp;<a href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport\">View DISTRICT Assignments in Class/District Order</a>";
   else if($type!='state')
      echo "&nbsp;&nbsp;|&nbsp;&nbsp;<a href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport&type=abc\">View DISTRICT Assignments in ABC ORDER</a>&nbsp;&nbsp;";
   echo "</p>";
}
...
if($type=='state')
{
   echo "&nbsp;<a href=\"bbgassignreport.php?database=$database&session=$session&type=state&gender=Girls\">Girls</a>";
}
...
<?php
   if($type=='state')
      echo "<label class='option'><a target=\"_blank\" href=\"bbbassignexport.php?session=$session&gender=$gender\">State Officials MAILING EXPORT</a></label>";
   ...
   echo "<label class='option'><a href=\"assignpost.php?return=bbbassignreport&session=$session&sport=$sport&type=$type\">POST All ";
   if($type=="state") echo "STATE";
   else echo "NON-STATE";
   echo " Contracts</a></label>";
   echo "<label class='option'><a href=\"bbbcontracts.php?session=$session";   
   ...
   if($type=='state' && $confirmed!=1)
      echo "<p><a class=small href=\"bbbassignreport.php?session=$session&sport=$sport&type=$type&confirmed=1\">Only Show Accepted & Confirmed Contracts</a></p>";
   else if($type=='state' && $confirmed==1)
      echo "<p><a class=small href=\"bbbassignreport.php?session=$session&sport=$sport&type=$type\">Show ALL Contracts</a></p>";
}
echo "</caption>";
...
else if($type=='state')
{
   ...
   $result=mysql_query($sql);
   echo "<tr align=center><td rowspan=2 align=left><a class=small href=\"bbbassignreport.php?database=$database&gender=$gender&sport=$sport&session=$session&type=state&confirmed=$confirmed\">Official</a></td>";
   echo "<td rowspan=2><b>SSN</b></td><td rowspan=2><b>City</b></td>";
   echo "<td rowspan=2><a class=small href=\"bbbassignreport.php?gender=$gender&sport=$sport&database=$database&session=$session&type=state&confirmed=$confirmed&sort=post\">Posted</a></td>";
   echo "<td rowspan=2><a class=small href=\"bbbassignreport.php?gender=$gender&sport=$sport&database=$database&session=$session&type=state&confirmed=$confirmed&sort=accept\">Accept</a></td>";
   echo "<td rowspan=2><a class=small href=\"bbbassignreport.php?gender=$gender&sport=$sport&database=$database&session=$session&type=state&confirmed=$confirmed&sort=confirm\">NSAA</a></td>";
   echo "<td rowspan=2><a class=small href=\"bbbassignreport.php?gender=$gender&sport=$sport&database=$database&session=$session&type=state&confirmed=$confirmed&sort=standby\">Stand-By</a></td>";
   echo "<td colspan=4><b>Dates Needing Lodging</b></td>";
   echo "<td rowspan=2><a class=small href=\"bbbassignreport.php?gender=$gender&sport=$sport&database=$database&session=$session&type=state&confirmed=$confirmed&sort=arrive\">Arrival<br>Time</a></td>";
   echo "<td rowspan=2><a class=small href=\"bbbassignreport.php?gender=$gender&sport=$sport&database=$database&session=$session&type=state&confirmed=$confirmed&sort=special\">Special Requests</a></td>";
   echo "</tr><tr align=center>";
$sql2="SELECT * FROM bbtourndates WHERE boys='x' AND lodgingdate='x' ORDER BY tourndate";
$result2=mysql_query($sql2);
while($row2=mysql_fetch_array($result2))
{
...
if($type=="abc" && $print!=1)
{
   echo "<a class=small href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport\">View DISTRICT Assignments in Class/District Order</a>&nbsp;&nbsp;";
   echo "<a class=small href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport&type=state\">View STATE Assignments</a>&nbsp;&nbsp;";
}
else if($type=='state' && $print!=1)
{
   echo "<a class=small href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport\">View DISTRICT Assignments</a>&nbsp;&nbsp;";
}
elseif($print!=1)
{
   echo "<a class=small href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport&type=abc\">View DISTRICT Assignments in ABC Order</a>&nbsp;&nbsp;";
   echo "<a class=small href=\"bbbassignreport.php?database=$database&session=$session&sport=$sport&type=state\">View STATE Assignments</a>&nbsp;&nbsp;";
}
if($print!=1)
