...
if($sport && $sport!='' && is_uploaded_file($uploadedfile))
{
   ...
   else
   {
	 ...
	 for($i=0;$i<count($line1);$i++)
	 {
	       ...
	       if(substr($question,0,1)=='"' && substr($question,strlen($question)-1,1)=='"')
	       ...
	       if($answernum=="")	//answer was not a number
	       {
	          $correct=trim(strtolower($ques[7]));
	          if($correct=="t") $correct="a";
		  else if($correct=="f") $correct="b";
	       }
	       else
	       {
	          $answernum--;
...
echo "<form method=post action=\"importtest.php\" enctype=\"multipart/form-data\">";
...
if($import && $errors!='')
   ...
   echo "<div class=alert>The test has been successfully imported! <a href=\"edittest.php?session=$session&sport=$sport\">View and edit the test questions HERE</a></div><br><br>";
echo "<h1>Import PART 1 $sportname Test Questions/Answers:</h1>";
echo "<p><i>Please upload the $sportname Part 1 Test questions, answers and rule references by following the instructions below.</i></p>
	<ol>
