<?php
/* BALLOT FOR COACHES TO VOTE FOR SOFTBALL UMPIRES */
/* ADAPTED FROM vote_ba.php ON 9/25/13 (ANN GAFFIGAN) */
...
if($submit)
{
   ...
   for($i=0;$i<count($offid);$i++)
   {
      if($check[$i]=='x')
      {
         $sql="SELECT * FROM $db_name2.sb_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 $result=mysql_query($sql);
 	 if(mysql_num_rows($result)==0)
	 {
	    $sql2="INSERT INTO $db_name2.sb_votes (school,ad_coach,district,officialid) VALUES ('$school2','$ad_coach','$dist','$offid[$i]')";
	 }
	 else
	 {
	    $sql2="UPDATE $db_name2.sb_votes SET district='$dist' WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 }
	 ...
	 $offlist.="$row[first] $row[last] ($row[city])<br>";
      ...
      {
	 $sql="DELETE FROM $db_name2.sb_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
         $result=mysql_query($sql);
      }
   }
   if($count<=10)
   {
      echo $init_html;
      echo $header;
      echo "<br><br><table><tr align=left><td><b>You have voted for the following Softball Umpires:</td></tr>";
      echo "<tr align=left><td>$offlist</td></tr>";
      echo "<tr align=left><td><b>Thank you for voting!<br><br></b>";
      if($level==1)
         echo "<a href=\"vote.php?sport=sb&session=$session\">Return to Softball Ballots Admin</a>";
...
$sql="SELECT * FROM $db_name2.sb_votes WHERE school='$school2' AND ad_coach='$ad_coach'";
$result=mysql_query($sql);
if(mysql_num_rows($result)>0 && $level!=1 && !($count>10 && $submit))
{
   echo "<br><br>You have already submitted your ballot for Softball Umpires.<br><br>";
   echo "Thank You!<br><br><a href=\"../welcome.php?session=$session\">Home</a>";
   echo $end_html;
   exit();
}
//ELSE
echo "<form method=post action=\"vote_sb.php\">";
...
echo "Softball Officials Ballot:</b><br><i>Please vote for a maximum of <b>10</b> officials.";
if($count>10 && $submit)
{
...
$sql="SELECT t1.id,t1.first,t1.last,t1.city,t1.photofile,t1.photoapproved FROM $db_name2.officials AS t1, $db_name2.sboff AS t2, $db_name2.sbapply AS t3 WHERE t1.id=t2.offid AND t2.offid=t3.offid AND t3.date3='x' AND t3.date4='x' AND t3.date5='x' ORDER BY t1.last,t1.first";
$result=mysql_query($sql);
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT * FROM $db_name2.sb_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$row[id]'";
   $result2=mysql_query($sql2);
