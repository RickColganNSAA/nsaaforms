...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE vbcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE district='1'";
   $result=mysql_query($sql);
}
...
$sql="SELECT * FROM vbcontracttext WHERE district='$district'";
$result=mysql_query($sql);
...
echo "<form method=post action=\"vbcontract.php\">";
...
if($edit==1)
   echo "<a class=small href=\"vbcontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1 && $level==1)
   echo "<a class=small href=\"vbcontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($submit)
{
   ...
   for($i=0;$i<count($gameid);$i++)
   {
      ...
      $sql="SELECT offid FROM $contracts WHERE disttimesid='$gameid[$i]'";
      $result=mysql_query($sql);
      $partners="";
      while($row=mysql_fetch_array($result))
      {
...
{
   echo "<tr align=left><td><br><br><input type=radio name=confirm value='y'>&nbsp;&nbsp;&nbsp;The NSAA confirms the above agreement for the $thisyear $type Volleyball Tournament";
   echo ".</td></tr>";
