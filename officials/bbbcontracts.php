...
echo "<a class=small href=\"".$sport."assignreport.php?session=$session\">View All $sportname Assignments</a>&nbsp;&nbsp;";
echo "<a class=small href=\"assignpost.php?session=$session&sport=$sport&type=&return=".$sport."contracts\">POST All DISTRICT $sportname Contracts</a>&nbsp;&nbsp;";
echo "<a class=small href=\"assignpost.php?session=$session&sport=$sport&type=state&return=".$sport."contracts\">POST All STATE $sportname Contracts</a><br>";
echo "<br>";
echo "<table cellspacing=4 cellpadding=4>";
echo "<caption><b>$sportname Officials' Contracts:</b></caption>";
echo "<form method=post action=\"bbbcontracts.php\" name=\"bbbcontracts\">";
...
while($row=mysql_fetch_array($result))
{
   ...
   if($row[type]=='State') echo "state";
   echo "contract.php";
   echo "?session=$session&givenoffid=$row[offid]&distid=$row[id]\">$row[gender] $row[type]";
