...
if($save || $hiddensave)	
{
   ...
   for($i=0;$i<$total;$i++)
   {
      ...
      if($$var2!="[Click to Choose Official]" && $$var2!="[Type all or part of the name]" && $$var2!='')
      {
 	    ...
 	    if($crewchief[$curid]==$$var1) $chief='x';
            ...
            $sql2="INSERT INTO $contracts (offid,disttimesid,crewchief) VALUES ('".$$var1."','$curid','$chief')";
            $result2=mysql_query($sql2);
         }
	 else
	 {
	    $sql2="UPDATE $contracts SET crewchief='$chief' WHERE offid='".$$var1."' AND disttimesid='$curid'";
	    $result2=mysql_query($sql2);
         }
      }
   }
...
<body onload="OffAssign.initialize('<?php echo $session; ?>','ba');">
...
echo "<form name=assignform method=post action=\"assignba.php\">";
...
if(!$type || $type=='~' || !$distid || $distid=='~')
   echo " width=90%";
echo "><caption><b>";
echo "<a class=small target=new href=\"bacontract.php?session=$session&sample=1\">Sample District Baseball Contract</a>&nbsp;&nbsp;";
echo "<a class=small target=new href=\"bastatecontract.php?session=$session&sample=1\">Sample State Baseball Contract</a><br><br>";
if($sport && $sport!="~")
{
   echo "Choose Type:&nbsp;";
...
if($sport && $sport!='~' && $type && $type!="~" && ($type=="State" || ($distid && $distid!='~')))
{
   ...
   if($type!="State" && $type!='District Final')
   {
	 ...
	 echo "$times</td></tr>";
      ...
      echo "</table><a name=\"timeslots\"><hr>&nbsp;</a></td></tr>";
      ...
      else      //in normal mode
      {
         ...
         echo "<i>NOTE: To remove an official from an assignment in Normal Mode, click on the box<br>with the official's name and then in the window that pops up, click on the \"RESET\" link.</i>";
   ...
   while($row2=mysql_fetch_array($result2))
   {
	 ...
	 echo "<tr align=left valign=top><td><b>$row3[time]</b></td></tr><tr align=left><td><table>";
