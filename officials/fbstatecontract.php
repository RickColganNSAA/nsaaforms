...
if(!$givenoffid && $level!=1) $offid=GetOffID($session);
else if($level==1 && $givenoffid) $offid=$givenoffid;
...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE fbcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE finals='1'";
   $result=mysql_query($sql);
}

//get contract text from DB
$sql="SELECT * FROM fbcontracttext WHERE finals='1'";
$result=mysql_query($sql);
...
echo "<form method=post action=\"fbstatecontract.php\">";
...
if($edit==1)
   echo "<br><a class=small href=\"fbstatecontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1)
   echo "<br><a class=small href=\"fbstatecontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($edit==1)
   ...
   echo $text1;
echo "</td></tr>";
/*** UPDATED 11/17/15 to REFLECT NEW WAY OF STORING THESE POSTSEASON GAMES ****/
