...
if($deletephoto)
{
   ...
   $sql="UPDATE officials SET photofile='',photoapproved='' WHERE id='$offid'";
      $filename.=$offid;
   $result=mysql_query($sql);
}
if($upload)
...
if($save)
{
   ...
   $sql="UPDATE officials SET address='$address',city='$city',state='$state',zip='$zip',homeph='$homeph',workph='$workph',cellph='$cellph',email='$email' WHERE id='$offid'";
   $result=mysql_query($sql);

   if(trim($address)=="" || trim($city)=="" || trim($state)=="" || trim($zip)=="")
   {
      //don't let them save unless they enter address
      $errormsg="You must enter your Address, City, State and Zip.";
   }
   else $errormsg="";
}
...
echo "<br>";
echo "<form method=post action=\"uploadphotos.php\" target=\"uploadframe\" enctype=\"multipart/form-data\">";
echo "<input type=hidden name=session value=\"$session\">";
echo "<table><caption><b>Edit Contact Information:</b><hr></caption>";
...
if($photofile!='' && citgf_file_exists("photos/$photofile"))      //photo exists
{
   ...
   echo "<img src=\"photos/$photofile\" border=0 width=\"100px\"><br><a href=\"editinfo.php?session=$session&deletephoto=1\" onClick=\"return confirm('Are you sure you want to delete this photo?  You cannot undo this action.');\">Delete Photo</a><br>";
