...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE bacontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE district='1'";
   $result=mysql_query($sql);
}
...
$sql="SELECT * FROM bacontracttext WHERE district='$district'";
$result=mysql_query($sql);
...
echo "<form method=post action=\"bacontract.php\">";
...
if($edit==1)
   echo "<a class=small href=\"bacontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1 && $level==1)
   echo "<a class=small href=\"bacontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($submit)
{
   ...
   {
      $g=$i+1;
      echo "<tr align=\"left\"><th>Game $g:</th></tr><tr align=\"left\"><td>";
      $sql="SELECT * FROM $disttimes WHERE id='$gameid[$i]'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
	 //Get Opponents:
      $sql3="SELECT * FROM $db_name.".$sport."sched WHERE distid='$distid' AND gamenum='$row[gamenum]'";
      $result3=mysql_query($sql3);
      $row3=mysql_fetch_array($result3);
   
      echo "<ul><li><b>Date & Time:</b> ".date("F j, Y",strtotime($row['day']))." at $row[time]</li>";
      $teams=trim(GetSchoolName($row3[sid],$sport)." vs ".GetSchoolName($row3[oppid],$sport));
      if($teams=="vs") $teams="TBA";
      echo "<li><b>Teams:</b> $teams</li>";
      echo "<li><b>Site:</b> $row[site]</li>";
      echo "<li><b>District Director:</b> $row[director]</li>";
      echo "<li><b>Crew:</b>";
      $sql2="SELECT offid,crewchief FROM $contracts WHERE disttimesid='$gameid[$i]' ORDER BY crewchief DESC";
      $result2=mysql_query($sql2);
      $partners="";
      while($row2=mysql_fetch_array($result2))
      {
         $partners.=GetOffName($row2[offid]);
	 if($row2[crewchief]=='x') $partners.=" (Crew Chief)";	
	 $partners.=", ";
      }
      $partners=substr($partners,0,strlen($partners)-2);
      echo "&nbsp;$partners</li></ul></td></tr>";
   } //END FOR EACH $gameid
...
{
   echo "<tr align=left><td><br><br><input type=radio name=confirm value='y'>&nbsp;&nbsp;&nbsp;The NSAA confirms the above agreement for this $type Baseball Tournament";
   echo ".</td></tr>";
