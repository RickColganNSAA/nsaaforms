...
echo "<form name=assignform method=post action=\"assignbbb.php\">";
echo "<input type=hidden name=session value=$session>";
...
if($sport && $sport!='~' && $type && $type!="~" && ($type=="State" || ($distid && $distid!='~')))
{
   ...
   if($type!="State")
   {
	 ...
	 $sql3="SELECT time FROM $disttimes WHERE distid='$distid' AND day='$row2[day]' ORDER BY gamenum,time"; 
         $result3=mysql_query($sql3);
	 $day=split("-",$row2[day]);
   ...
   while($row2=mysql_fetch_array($result2))
   {
	 ...
	 $curday="(Date?)";
      ...
      echo ":</u></b></td></tr>";
      echo "<tr align=center><td><table>";
      $sql3="SELECT * FROM $disttimes WHERE distid='$distid' AND day='$row2[day]' ORDER BY time";
      $result3=mysql_query($sql3);
      $curct=0;
      while($row3=mysql_fetch_array($result3))
      {
	 $curdisttimesid=$row3[id];
	 echo "<tr valign=top align=left><td><b>$row3[time]</b></td><td>";
	 if($row3[sid]>0 && $row3[oppid]>0)	//CLASS A BASKETBALL
	 {
	    echo GetSchoolName($row3[sid],$sport)." vs ".GetSchoolName($row3[oppid],$sport)." at ";
	    if(trim($row3[site])!='') echo $row3[site];
	    else echo $row3[hostschool];
	 } 
         echo "<br><table><tr align=left valign=top>";
