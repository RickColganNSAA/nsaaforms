<?php
/****CONTRACT TO HOST unified bowling DISTRICTS****/
...
if($edit==1 && $savechanges)
{
   ...
   $text5=ereg_replace("\r\n","<br>",$text7);
   $text5=addslashes($text7);
   $text5=ereg_replace("\r\n","<br>",$text8);
   $text5=addslashes($text8);
   $text6=ereg_replace("\r\n","<br>",$text9);
   $text6=addslashes($text9);
   $sql="UPDATE ubocontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4',text7='$text7',text8='$text8',text9='$text9' WHERE district=
'2'";
   $result=mysql_query($sql);
   ...
   if($dates!="0000-00-00")
   {
      $sql="UPDATE ubobdistricts SET dates='$dates' WHERE type='District'";
      $result=mysql_query($sql);
   }
}
...
//get contract text
$dist=2;
$sql="SELECT * FROM ubocontracttext WHERE district='$dist'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$text1=$row[text1]; $text2=$row[text2];
$text3=$row[text3]; $text4=$row[text4];
$text7=$row[text7]; $text8=$row[text8];
$text9=$row[text9]; 
...
echo "<br>";
echo "<form method=post action=\"hostcontract_ubo.php\">";
...
if($edit==1)
   echo "<br><a class=small href=\"hostcontract_ubo.php?session=$session&sample=1\">Preview this Contract</a>";else
//code by robin
//   echo "<br><a class=small href=\"hostcontract_ubo.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
echo "</td></tr>";
/* echo "<tr align=left><td><b>Bowling Facility:</b></td><td>";
if($edit==1)
   echo "<input type=text class=tiny size=50 name=text7 value=\"$text7\">";
else
   echo $text7;
echo "<tr align=left><td><b>City:</b></td><td>";
if($edit==1)
   echo "<input type=text class=tiny size=50 name=text8 value=\"$text8\">";
else
   echo $text8;
echo "<tr align=left><td><b>Number of Lanes:</b></td><td>";
if($edit==1)
   echo "<input type=text class=tiny size=50 name=text9 value=\"$text9\">";
else
   echo $text9; */
echo "</table></td></tr>";
...
if($accept!='y' && $accept!='n' && ($sample==1 || $level!=1))	//Host has not accepted yet
{
   ...
   if($edit==1)
   {
	 ...
	 echo ">$i</option>";
      ...
      echo "</select><br>";
      echo "<font style=\"color:red\">[Only the NSAA can edit this date.  Editing this date will change the date on all unified bowling host contracts.  In addition, the year shown above will be the year in the \"SUBJECT:\" line at the top of this contract.]</font></td></tr>";
   } 
   else
   {
