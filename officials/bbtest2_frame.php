...
{
   $curcategid=0;
   for($i=0;$i<count($questionid);$i++)      //Update answers for each question in this section
   {
      //get current category ID
      if($curcategid==0)
      {
         $sql="SELECT category FROM $answerstable WHERE questionid='$questionid[$i]' AND offid='$offid'";
         $result=mysql_query($sql);
         $row=mysql_fetch_array($result);
         $curcategid=$row[0];
      }

      $sql="SELECT id FROM $answerstable WHERE offid='$offid' AND questionid='$questionid[$i]'";
      $result=mysql_query($sql);
      ...
      {
         $row=mysql_fetch_array($result);
         $sql2="UPDATE $answerstable SET answer='$answer[$i]' WHERE id='$row[id]'"; //offid='$offid' AND questionid='$questionid[$i]'";
         $result2=mysql_query($sql2);
      }
   }
   if($save)    //send to next section
   {
      ...
      $row=mysql_fetch_array($result);
      $next=$row[0]+1;
      $sql="SELECT id FROM $categtable WHERE place='$next'";
      $result=mysql_query($sql);
...
$sql="SELECT id FROM $categtable WHERE id='$categid'";
$result=mysql_query($sql);
if(mysql_num_rows($result)==0)
{
   $sql="SELECT id FROM $categtable ORDER BY place LIMIT 1";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   $categid=$row[id];
}
...
echo "<tr align=center><td align=center><table cellspacing=0 cellpadding=5 class=nine>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT choicevalue,choicelabel FROM ".$testtable."_mchoices WHERE questionid='$row[questionid]' ORDER BY orderby";
   $result2=mysql_query($sql2);
...
$sql="SELECT category,answer FROM $answerstable WHERE offid='$offid' ORDER BY category,place";
$result=mysql_query($sql);
...
$row=mysql_fetch_array($result);
$next=$row[0]+1;
$sql="SELECT DISTINCT t1.id FROM $categtable AS t1,$answerstable AS t2 WHERE t1.id=t2.category AND t1.place='$next'";
$result=mysql_query($sql);
if(mysql_num_rows($result)==0)
...
$sql="SELECT DISTINCT t1.id,t1.category,t1.place FROM $categtable AS t1,$answerstable AS t2 WHERE t1.id=t2.category ORDER BY t1.place";
$result=mysql_query($sql);
