...
$sql2="SELECT DISTINCT tourndate FROM sotourndates WHERE hostdate='x' AND girls='x' ORDER BY tourndate,label";
...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE sogcontracttext SET text1='$text1',text2='$text2',text3='$text3' WHERE district='2'";
   $result=mysql_query($sql);
}

//get contract text
$dist=2;
$sql="SELECT * FROM sogcontracttext WHERE district='$dist'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
...
echo "<br>";
echo "<form method=post action=\"hostcontract_sog.php\">";
...
if($edit==1)
   echo "<br><a class=small href=\"hostcontract_sog.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1)
   echo "<br><a class=small href=\"hostcontract_sog.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
echo "</table></td></tr>";
echo "<tr align=left><td>Before agreeing to accept this contract, please review the information about hosting this district by clicking on the <a class=small href=\"#\" onclick=\"window.open('sohostterms.php?session=$session','Soccer_Host_Terms','width=600,height=800');\">Terms & Conditions</a>.</td></tr>";
echo "<tr align=left><td>";
if($edit==1)
{
