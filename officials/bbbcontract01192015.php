...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE bbbcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE district='1'";
   $result=mysql_query($sql);
}
...
$sql="SELECT * FROM bbbcontracttext WHERE district='$district'";
$result=mysql_query($sql);
...
echo "<br>";
echo "<form method=post action=\"bbbcontract.php\">";
...
if($edit==1)
   echo "<a class=small href=\"bbbcontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1 && $level==1)
   echo "<a class=small href=\"bbbcontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($type!='State')
{
   ...
   echo "<table>";
   $sql2="SELECT DISTINCT gender,day FROM $disttimes WHERE (";
   for($i=0;$i<count($gameid);$i++)
   {
      $sql2.="id='$gameid[$i]' OR ";
   }
   $sql2=substr($sql2,0,strlen($sql2)-4);
   $sql2.=") ORDER BY gender,day";
   $result2=mysql_query($sql2);
   //echo $sql2;
   $curgender="";
   while($row2=mysql_fetch_array($result2))
   {
      $sql3="SELECT DISTINCT time FROM $disttimes WHERE gender='$row2[gender]' AND day='$row2[day]' AND (";
      for($i=0;$i<count($gameid);$i++)
      {
         $sql3.="id='$gameid[$i]' OR ";
      }
      ...
      while($row3=mysql_fetch_array($result3))
      {
         if(ereg('&',$row[gender]))
         {
            if($curgender!=$row2[gender])
            {
	       $curgender=$row2[gender];
	       echo "<tr align=left><td colspan=2>$curgender:</td></tr>";
            }
         }
         if($curday!=$row2[day]) 
         {
 	    ...
 	    $curday=$row2[day];
            $date=split("-",$curday); $day=date("F d",mktime(0,0,0,$date[1],$date[2],$date[0]));
...
{
   echo "<tr align=left><td><br><br><input type=radio name=confirm value='y'>&nbsp;&nbsp;&nbsp;The NSAA confirms the above agreement for the $thisyear $type Boys Basketball Tournament";
   echo ".</td></tr>";
