...
//get number of teams
$sql="SELECT * FROM baseeds WHERE distid='$distid'";
...
$homeid=GetSID2($row2[school],'ba');
echo "<font style=\"font-size:12pt;\"><b>$sportname $row[type] $row[class]-$row[district]</b></font><br><br><br>";
...
for($i=1;$i<=$teamct;$i++)
{
   $sql="SELECT * FROM $db_name2.badisttimes WHERE distid='$distid' AND gamenum='$i'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   $day=split("-",$row[day]);
   $field="match".$i;
   ...
   $sql="SELECT sid,ptavg FROM $db_name2.baseeds WHERE distid='$distid' AND seed='$i'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   $field="seed".$i;
   if($seeded!='y' || $bracketed!='y')
      ...
      $$field="<font style=\"font-size:9pt;\">#$i ".GetSchoolName($row[sid],'ba',$year)." $row[ptavg] (".GetWinLoss($row[sid],'ba',$year).")</font>";
   $field="sid".$i;
   if($seeded!='y' || $bracketed!='y')
      $$field='';
   else
...
{
   //MATCH 1:
   $sql="SELECT * FROM $db_name.basched WHERE distid='$distid' AND gamenum='1'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   if($row[oppscore]!="" && $row[sidscore]!="")	//get Game 3 info
   {
      //MATCH 3:
	 ...
	 $winner1sid=$row[oppid]; $score1="$row[oppscore]-$row[sidscore]";
      ...
      $winner1="<font style=\"font-size:9pt;\">".GetSchoolName($winner1sid,'ba',$year)." ($score1)</font>";

      $sql="SELECT * FROM $db_name.basched WHERE distid='$distid' AND gamenum='3'"; 
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);

      if($row[oppscore]!="" && $row[sidscore]!="")	//get Game 5 info
      {
         if($row[sidscore]>$row[oppscore]) 
	 {
	 ...
	 $winner3="<font style=\"font-size:9pt;\">".GetSchoolName($winner3sid,'ba',$year)." ($score3)</font>";
      }
      else $winner3="";
   }
   else
   ...
   //MATCH 2:
   $sql="SELECT * FROM $db_name.basched WHERE distid='$distid' AND gamenum='2'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   if($row[oppscore]!="" && $row[sidscore]!="") //get Match 4 info
   {
	 ...
	 $winner2sid=$row[oppid]; $score2="$row[oppscore]-$row[sidscore]";
      ...
      $winner2="<font style=\"font-size:9pt;\">".GetSchoolName($winner2sid,'ba',$year)." ($score2)</font>";

      $sql="SELECT * FROM $db_name.basched WHERE distid='$distid' AND gamenum='4'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
         ...
         else 
	 {
	    $winner4sid=$row[oppid]; $score4="$row[oppscore]-$row[sidscore]";
         ...
         $winner4="<font style=\"font-size:9pt;\">".GetSchoolName($winner4sid,'ba',$year)." ($score4)</font>";
      }
      else $winner4="";
   }
   else
   ...
   if($winner3!='' && $winner4!='')
   {
      $sql="SELECT * FROM $db_name.basched WHERE distid='$distid' AND gamenum='5'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
	 ...
	 $winner5=GetSchoolName($winner5sid,'ba',$year)." ($score5)";
      }
      else $winner5="WINNER";
   }
   else $winner5="WINNER";
...
else if($teamct==5)
{
   ...
   echo "<tr align=center><td width=$width height=$height class=border bgcolor=#E0E0E0><b>$match2</b></td></tr>";
   echo "<tr align=center valign=top><td width=$width height=60>$seed3</td></tr>";
   echo "</table>";
   echo "</td><td>";
   echo "<table cellspacing=0 cellpadding=0>";
   echo "<tr align=center valign=bottom><td width=$width height=60>$winner3</td></tr>";
