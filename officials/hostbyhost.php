...
if (mysql_num_rows($result2)==0){
    if(preg_match("/bb/",$sport) && ($class=="C1"||$class=="C2"||$class=="D1"||$class="D2")){
        $sql="INSERT INTO `bbbdisttimes` (`id`, `distid`, `day`, `time`, `gamenum`, `gender`, `notes`, `hostid`, `hostschool`, `site`, `director`, `email`, `post`, `accept`, `confirm`, `showgamenum`) VALUES (NULL, '$distid', '0000-00-00', '', '1', '', '', '0', '', '', '', '', '', '', '', '');";
        mysql_query($sql);
    }
}
...
else if($save || $hiddensave)	//SAVE ALL CHANGES
{
   ...
   if($type=="District Final" || $type=="Substate")
      $teamcount=2;
   if($sport=='sb') $gamecount=($teamcount*2)-1;          //# of games in tournament: SB=Double Elim, other=
   ...
   if(IsTimeslotSport($sport))	//sports with TIMES SLOTS
   { 	
...
//print_r($sport); exit;
   ...
   else if($distidfound && $distid && $distid!='' && $type && $type!='')
   {
	 ...
	 else if($type!='District Final' && $type!='Substate')	//not seeded yet
	 {
	    ...
	    else if($sport=='ba' || preg_match("/so/",$sport)) $max=7;
