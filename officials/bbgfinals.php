...
echo "<table>";
$sql="SELECT DISTINCT class FROM $db_name2.bbgdistricts WHERE type='District Final' ORDER BY class";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result))
{
   $class=$row['class'];
   $sql2="SELECT t1.*,t2.day,t2.time FROM $db_name2.bbgdistricts AS t1, $db_name2.bbgdisttimes AS t2 WHERE t1.id=t2.distid AND t1.type='District Final' AND t1.class='$class' ORDER BY t1.district";
   ...
   while($row2=mysql_fetch_array($result2))
   {
	 ...
	 $curcol=0;
      ...
      $sql3="SELECT * FROM $db_name.bbgsched WHERE distid='$row2[id]'";
