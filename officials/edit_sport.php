...
if($save)
{
   ...
   if(mysql_num_rows($result)>0)
   {
      ...
      if($sport=='bb') $sql2.=",gstateyears='$gstateyears',gnumstateyears='$gnumstateyears',bstateyears='$bstateyears',bnumstateyears='$bnumstateyears'";
   ...
   else
   {
      ...
      if($sport=='bb') $sql2.=",gstateyears,gnumstateyears,bstateyears,bnumstateyears";
   ...
   for($i=0;$i<count($subid);$i++)
   {
	...
	$newregyr.="-19";
	 ...
	 $rm[$i]=ereg_replace("\'","\'",$rm[$i]);
         ...
         $sql.="obtest,suptest,nhsoa,";
	 if(HasClinic($sport)) $sql.="clinic,";
	 if($sport=='tr') $sql.="position,";
	 $sql.="class,overrideclass) VALUES ('$offid','$newregyr','$newappdate','$contest[$i]','$rm[$i]',";
	 $sql.="'$obtest[$i]','$suptest[$i]','$nhsoa[$i]',";
	 if(HasClinic($sport)) $sql.="'$clinic[$i]',";
	 if($sport=='tr') $sql.="'$starter[$i]',";
	 $sql.="'$class2[$i]','$overrideclass[$i]')";
         $result=mysql_query($sql);

         //NOW UPDATE THEIR CLASSIFICATION (IF THEY QUALIFY) - ADDED NOV 6 2014
         UpdateRank($offid,$sport);

         if($nhsoa[$i]=='x')      //GO UPDATE THEIR OTHER SPORTS
         {
            $sql4="SHOW TABLES LIKE '%off_hist'";
            $result4=mysql_query($sql4);
            while($row4=mysql_fetch_array($result4))
            {
               $sql5="UPDATE $row4[0] SET nhsoa='x' WHERE offid='$offid' AND regyr='$newregyr'";
               $result5=mysql_query($sql5);
            }
            $sql4="UPDATE officials SET nhsoa='x' WHERE id='$offid'";
 	    $result4=mysql_query($sql4);
         }
         else      //IF OTHER SPORT HAS NHSOA CHECKED, CHECK IT FOR THIS SPORT
         {
            ...
            while($row4=mysql_fetch_array($result4))
            {
               $sql5="SELECT * FROM $row4[0] WHERE offid='$offid' AND regyr='$newregyr'";
               $result5=mysql_query($sql5);
               if($row5=mysql_fetch_array($result5))
               {
                  if($row5[nhsoa]=='x')    //THEN UPDATE THIS YEAR
                  {
                     $sql6="UPDATE $histtable SET nhsoa='x' WHERE offid='$offid' AND regyr='$newregyr'";
...
echo "<form method=post action=\"edit_sport.php\" name=\"sportform\">";
...
echo "<th class=smaller align=center>Tests</th><th class=smaller align=center>ST</th>";
if(HasClinic($sport)) echo "<th class=smaller align=center>Clinic</th>";
if($sport=='tr') echo "<th class=smaller align=center>Starter</th>";
echo "<th class=smaller align=center>Class</th><th class=smaller align=left>Over<br>-ride<br>Class</th><th class=smaller align=center>NHSOA</th></tr>";
...
while($row=mysql_fetch_array($result))
{
   ...
   */
   ...
   if(HasClinic($sport))
   {
      echo "<td align=center><input type=checkbox name=\"clinic[$ix]\" value='x'";
      if($row[clinic]=='x') echo " checked";
      echo "></td>";
   }
   if($sport=='tr')
   {
      echo "<td align=center><input type=checkbox name=\"starter[$ix]\" value='starter'";
      if($row[position]=='starter') echo " checked";
      echo "></td>";
   }
   echo "<td align=center><input type=text size=2 name=\"class2[$ix]\" value=\"$row[class]\"></td>";
   echo "<td align=left><input type=checkbox name=\"overrideclass[$ix]\" value=\"x\"";
   if($row[overrideclass]=="x") echo " checked";
   echo "></td>";
   echo "<td align=center><input type=checkbox name=\"nhsoa[$ix]\" value=\"x\"";
   if($row[nhsoa]=='x') echo " checked";
   echo "></td>";
   //get current year
...
while($ix<$max)
{
   ...
   if(HasClinic($sport))
      echo "<td align=center><input type=checkbox name=\"clinic[$ix]\" value='starter'></td>";
   if($sport=='tr') echo "<td align=center><input type=checkbox name=\"starter[$ix]\" value='x'></td>";
   echo "<td align=center><input type=text size=2 name=\"class2[$ix]\"></td>";
   echo "<td align=left><input type=checkbox name=\"overrideclass[$ix]\" value=\"x\"";
   if($row[overrideclass]=="x") echo " checked";
   echo "\"></td>";
   echo "<td align=center><input type=checkbox name=\"nhsoa[$ix]\" value=\"x\"></td>";
   echo "</tr>";
   $ix++;
}
echo "</table></td></tr>";
...
if($row[appid]!=0 && $row[appid]!='' && $row[payment]!="")
   echo "<td><a class=small target=new href=\"officialsapp.php?session=$session&curappid=$row[appid]\">App #$row[appid]</a></td></tr>";
else
   echo "<td>No</td></tr>";
echo "</table></td></tr>";
/*
