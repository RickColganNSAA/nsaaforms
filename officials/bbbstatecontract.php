...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE bbbcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4',text5='$text5',text6='$text6' WHERE district='0'";
   $result=mysql_query($sql);
}
...
if($submit || $submit2)
{
   if($level!=1 && (($submit && (($lodging && $accept) || $accept=='n')) || $submit2))
   {
         ...
         if($accept=='y' && $lodging=='y')	//show LODGING FORM
	 {
	    echo $init_html;
            echo "<br>";
	    echo "<form method=post action=\"bbbstatecontract.php\">";
	    ...
	    echo "<tr align=left><td><b>Please list any special room requests below:<br></b><br>";
	 ...
	 header("Location:bbbstatecontract.php?session=$session&distid=$distid");
	 exit();
      }
   }
...
echo "<img src=\"nsaacontract.png\">";
if($edit==1)
   echO "<br><a class=small href=\"bbbstatecontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1)
   echo "<br><a class=small href=\"bbbstatecontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
