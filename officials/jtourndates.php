...
echo $init_html;
echo GetHeaderJ($session,"duedates");

echo "<form method=post action=\"jtourndates.php\">
...
echo mysql_error();
   ...
   $sql="SELECT * FROM $table ORDER BY tourndate ASC";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0)
   {
   ...
   else
   {
      ...
      echo "<caption><p style=\"text-align:left;\"><a class='small' onClick=\"return confirm('Are you sure you want to reset the applications to host?');\" href=\"jtourndates.php?resethost=1&session=$session&sport=$sport\">RESET ".GetSportName($sport)." Applications to Host</a> (This will reset the <i>\"Interested? Yes or No\"</i> field, not the other fields on the form)</p><br>";
      ...
      if($usinggender==1)
      {
         echo "<a href=\"../hostapp_".$sport."_g.php?session=$session&nsaa=1\" target=\"_blank\" class=\"small\">Preview Girls</a><br>"; 
         echo "<a href=\"../hostapp_".$sport."_b.php?session=$session&nsaa=1\" target=\"_blank\" class=\"small\">Preview Boys</a>";
      }
      else
         echo "<a href=\"../hostapp_".$sport.".php?session=$session&nsaa=1\" target=\"_blank\" class=\"small\">Preview</a>";
      echo "</td><td><b>App to Off</b>";
      if($sport=='pp') $sport2="play";
      else if($sport=='sp') $sport2="speech";
      else $sport2=$sport;
      echo "<br><a href=\"".$sport2."app.php?header=no&session=$session\" target=\"_blank\" class=\"small\">Preview</a>";
