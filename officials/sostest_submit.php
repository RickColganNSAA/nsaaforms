...
else if($submit=="Submit Test Answers")
{
   ...
   //also update _hist table with this year's score
	 ...
	 $curyr1=$curyr+1;
	 $regyr=$curyr."-".$curyr1;
   $sql="SELECT * FROM sooff_hist WHERE offid='$offid' AND regyr='$regyr'";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0)       //INSERT
   {
      $sql2="INSERT INTO sooff_hist (offid,regyr,sobtest,sobtestattempts) VALUES ('$offid','$regyr','$score','1')";
   }
   else         //UPDATE
   {
      $sql2="UPDATE sooff_hist SET sobtest='$score',sobtestattempts=sobtestattempts+1 WHERE offid='$offid' AND regyr='$regyr'";
   }
   $result2=mysql_query($sql2);
   $sql="SELECT * FROM sooff_hist WHERE offid='$offid' AND regyr='$regyr'";
   $result=mysql_query($sql);
   ...
   $sql="SELECT DATE_ADD(duedate,INTERVAL 3 DAY) FROM test_duedates WHERE test='so'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   ...
   if(GetLevel($session)!=1 && $score<80 && $obtestattempts<3)        //FAILED - CAN RETAKE --  && $offid!='3427')
   {
        ...
        <p>You may <a href=\"sotest.php?session=$session&retake=$resultsid\">Re-Take this Test</a> up to 3 times in order to pass it before the due date has passed.</p>".$end_html;
$html="Official $offid has made $obtestattempts at the SO test and make attempt again. Please just check that this is working. Last attempt was ID $resultsid";
$text=$html;
SendMail("nsaa@nsaahome.criticalitgroup.com","NSAA","agaffigan@gazelleincorporated.com","Ann Gaffigan","An official may retake the Soccer test",$text,$html,array());
