...
if($offid==GetOffID($session))
{
   //CHECK THAT THIS OFFICIAL CAN VIEW THIS FORM
   $sql="SELECT * FROM $dbname.baobserve WHERE offid='$offid' AND gameid='$gameid' AND dateeval!=''";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0 || !$gameid)
   {
      echo "$sql<br>GAMEID $gameid";
      $gameid=0; $offid=0;
      $print=1;
   }
}
if($newgameid)
{
   $sql="UPDATE $dbname.baobserve SET gameid='$newgameid' WHERE gameid='$gameid' AND obsid='$obsid'";
   $result=mysql_query($sql);
   $gameid=$newgameid;
}
...
if($submit && $submit!="Go")	//put eval in db and show user what he/she entered
{
   ...
   {
      $sql2="UPDATE baobserve SET dateeval='$dateeval' WHERE obsid='$obsid' AND offid='$offid' AND gameid='$gameid'";
      $result2=mysql_query($sql2);
	 ...
	 $Text="A Nebraska School Activities Association Baseball Umpire's Evaluation has been filled out in your name.  Please login at https://secure.nsaahome.org/nsaaforms/officials/ to view your evaluation.\r\n\r\nThank You!";
         $Html="A Nebraska School Activities Association Baseball Umpire's Evaluation has been filled out in your name.  Please login at <a href=\"https://secure.nsaahome.org/nsaaforms/officials/\">https://secure.nsaahome.org/nsaaforms/officials/</a> to view your evaluation.<br><br>Thank You!";
  	 $Attm=array();
...
$sql="SELECT schools FROM $dbname.basched WHERE id='$gameid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$schools=$row[0];
...
if($print!=1) 
{
   echo "<form method=post action=\"baobserve.php\">";
...
echo "<table width=95%><caption><b>NSAA Baseball Umpire Observation Report:</b><br>";
...
if(GetLevel($session)==1 && $gameid && $gameid!='new')
   echo "<a href=\"deleteobserve.php?session=$session&dbname=$dbname&sport=ba&id=$id\" onClick=\"return confirm('Are you sure you want to delete this observation?  This action cannot be undone.');\">Delete this Observation</a>";
echo "</caption>";
...
if($gameid=='new' || $submit=="Go")
{
   ...
   $sql="SELECT * FROM $dbname.basched WHERE offid='$offid' ORDER BY offdate";
   $result=mysql_query($sql);
...
if($print!=1)
{
   ...
   //get date of game
   $sql="SELECT offdate,schools FROM $dbname.basched WHERE id='$gameid'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
      if(mysql_num_rows($result)==0)
      {
...
//General
$genques=array("Pregame Duties","Appearance","Professional Demeanor","Focus on the Game","Intensity/Alertness","Hustle");
$gennote=array("Arrive on field 10 minutes before game. Check field and conduct pre-game meeting.","Do they look like umpires?  Are shirts and slacks clean and pressed?  Shoes polished?","How do they carry themselves on the field?  Do they take the proper position between innings?","Do they talk to coaches/players too much?  Anticipate plays and upcoming situations.","Sharp and consistent from first pitch to final out.","Ready and willing to move to get into the proper position.");
...
$nrnote=array("Applies and enforces National Federation Rules correctly.","Explains, listens, stays calm and professional.  Willingness to confer with partner to get the call right.","Makes the tough calls and gets them correct. Appears sure and decisive when making calls.  Do they have control of the game?");
...
$hpnote=array("Takes proper position in the slot.  Places head in same position and is still on every pitch. Checks plate corners prior to pitch.","Calls the high and low strikes and ball over the plate.  Handles breaking pitch well.","Misses few pitches.  Stays the same inning-to-inning.  Handles breaking pitch well.","Makes calls clear with voice and uses proper signals and techniques.","Takes time to see the pitch completely before making call.","Shows lateral quickness and good foot speed to see plays and get out of the way.","Anticipates where to go on the field and removes mask when coming out from behind plate.","Uses hand or verbal signals, makes eye contact with partner(s) to cover possible situations.  Asks for help on check swing.","Rotates properly and takes the correct position to make calls and helps partner.");
...
$bnote=array("Makes the call with authority.","Stops, steadies body and head to see the play.","Takes the proper angle and gets within the proper distance to see the play and make the call.","Makes calls clear with voice and uses proper signals and techniques.","Sees the whole play before making the call.  Looks for possession before making the call.","Shows quickness and good foot speed to get in position to see plays.","Moves in anticipation of potential plays.","Uses hand or verbal signals, makes eye contact with partner(s) to cover possible situations.","Rotates properly and takes correct position to make calls and helps partner.");
