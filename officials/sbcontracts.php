...
echo "<form method=post action=\"sbcontracts.php\" name=sbcontracts>";
echo "<input type=hidden name=session value=$session>";
echo "<tr align=center><td colspan=2>";
echo "Choose Type:&nbsp;<select name=typech onchange=\"document.getElementById('distch').selectedIndex='0';submit();\"><option value=''>~</option>";
$types=array("District","State");
for($i=0;$i<count($types);$i++)
{
   echo "<option";
   if($typech==$types[$i]) echo " selected";
   echo ">$types[$i]</option>";
}
echo "</select>&nbsp;&nbsp;";
...
if($typech!='State')
{
   echo "Choose a District:&nbsp;<select name=\"distch\" id=\"distch\" onchange=\"submit();\"><option>All</option>";
   $sql="SELECT DISTINCT id,class,district FROM $districts WHERE type='$typech' ORDER BY class,district";
...
if($typech!='State')
{
   ...
   while($row=mysql_fetch_array($result))
   {
      ...
      echo "$row[class]-$row[district], hosted by $row[hostschool]";
      echo "</a><br>";
   }
   echo "</td><td>";
   $sql="SELECT * FROM $districts WHERE showoffs='y' ";
   ...
   while($row=mysql_fetch_array($result))
   {
         echo "<b>$row[type] $row[class]-$row[district]</b> assignments have been posted to the host school <b>$row[hostschool]</b>.<br>";
   }
   echo "</td></tr>";
...
$sql="SELECT DISTINCT t1.offid,t1.accept,t1.confirm,t2.class,t2.district,t2.id,t2.hostschool,t2.type FROM $contracts AS t1,$districts AS t2,$disttimes AS t3 WHERE t3.id=t1.disttimesid AND t3.distid=t2.id AND t1.post='y' AND t1.accept='y' AND t1.confirm='' ";
if($typech && $typech!="")
...
$sql="SELECT DISTINCT t1.offid,t1.accept,t1.confirm,t2.class,t2.district,t2.id,t2.hostschool,t2.type FROM $contracts AS t1,$districts AS t2,$disttimes AS t3 WHERE t3.id=t1.disttimesid AND t3.distid=t2.id AND t1.post='y' AND t1.accept='n' AND t1.confirm='' ";
if($typech && $typech!="")
   $sql.="AND t2.type='$typech' ";
if($distch && !ereg("All",$distch))
   $sql.="AND t2.id='$distch' ";
...
$sql="SELECT DISTINCT t1.offid,t1.accept,t1.confirm,t2.class,t2.district,t2.id,t2.hostschool,t2.type FROM $contracts AS t1,$districts AS t2,$disttimes AS t3 WHERE t3.id=t1.disttimesid AND t3.distid=t2.id AND t1.post='y' AND t1.accept='y' AND t1.confirm='y' ";
if($typech && $typech!="")
   $sql.="AND t2.type='$typech' ";
if($distch && !ereg("All",$distch))
   $sql.="AND t2.id='$distch' ";
...
$sql="SELECT DISTINCT t1.offid,t1.accept,t1.confirm,t2.class,t2.district,t2.id,t2.hostschool,t2.type FROM $contracts AS t1,$districts AS t2,$disttimes AS t3 WHERE t3.id=t1.disttimesid AND t3.distid=t2.id AND t1.post='y' AND t1.accept='y' AND t1.confirm='n' ";
if($typech && $typech!="")
   $sql.="AND t2.type='$typech' ";
if($distch && !ereg("All",$distch))
   $sql.="AND t2.id='$distch' ";
