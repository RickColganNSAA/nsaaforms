...
if($submit)
{
   ...
   for($i=0;$i<count($offid);$i++)
   {
      if($check[$i]=='x')
      {
         $sql="SELECT * FROM $db_name2.vb_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 $result=mysql_query($sql);
 	 if(mysql_num_rows($result)==0)
	 {
	    $sql2="INSERT INTO $db_name2.vb_votes (school,ad_coach,district,officialid) VALUES ('$school2','$ad_coach','$dist','$offid[$i]')";
	 }
	 else
	 {
	    $sql2="UPDATE $db_name2.vb_votes SET district='$dist' WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
	 }
	 ...
	 $offlist.="$row[first] $row[last] ($row[city])<br>";
      ...
      {
	 $sql="DELETE FROM $db_name2.vb_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$offid[$i]'";
         $result=mysql_query($sql);
      }
   }
   if($count<=10)
   {
      echo $init_html;
      echo $header;
      echo "<br><br><table><tr align=left><td><b>You have voted for the following Volleyball officials:</td></tr>";
      echo "<tr align=left><td>$offlist</td></tr>";
      echo "<tr align=left><td><b>Thank you for voting!<br><br></b>";
      if($level==1)
         echo "<a href=\"vote.php?sport=vb&session=$session\">Return to Volleyball Ballots Admin</a>";
...
$sql="SELECT * FROM $db_name2.vb_votes WHERE school='$school2' AND ad_coach='$ad_coach'";
$result=mysql_query($sql);
if(mysql_num_rows($result)>0 && $level!=1 && !($count>10 && $submit))
{
   echo "<br><br>You have already submitted your ballot for Volleyball Officials.<br><br>";
   echo "Thank You!<br><br><a href=\"../welcome.php?session=$session\">Home</a>";
   echo $end_html;
   exit();
}
//ELSE
echo "<form method=post action=\"vote_vb.php\">";
echo "<input type=hidden name=session value=\"$session\">";
echo "<input type=hidden name=nsaa value=\"$nsaa\">";
echo "<input type=hidden name=ad_coach value=\"$ad_coach\">";
echo "<input type=hidden name=school_ch value=\"$school_ch\">";
echo "<table width='800px' cellspacing=0 cellpadding=4><caption><b>";
if($sample==1) echo "Preview of ";
echo "Volleyball Officials Ballot:</b><br><i>Please vote for a maximum of <b>10</b> officials.";
if($count>10 && $submit)
{
...
//$sql="SELECT t1.id,t1.first,t1.last,t1.city,t1.photofile,t1.photoapproved FROM $db_name2.officials AS t1, $db_name2.vboff AS t2, $db_name2.vbapply AS t3 WHERE t1.id=t2.offid AND t2.offid=t3.offid AND t2.class='C' AND t3.date7='x' AND t3.date8='x' AND t3.date9='x' ORDER BY t1.last,t1.first";
$sql="SELECT t1.id,t1.first,t1.last,t1.city,t1.photofile,t1.photoapproved FROM $db_name2.officials AS t1, $db_name2.vboff AS t2, $db_name2.vbapply AS t3 WHERE t1.id=t2.offid AND t2.offid=t3.offid AND ( t3.date7='x' OR t3.date8='x' OR t3.date6='x') ORDER BY t1.last,t1.first";
$result=mysql_query($sql);
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT * FROM $db_name2.vb_votes WHERE school='$school2' AND ad_coach='$ad_coach' AND officialid='$row[id]'";
   $result2=mysql_query($sql2);
