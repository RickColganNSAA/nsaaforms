...
//get number of teams
$sql="SELECT * FROM bbgseeds WHERE distid='$distid'";
...
$homeid=GetSID2($row2[school],'bbg');
...
echo "</b></font><br><br>";
if(date("Y")==2011 && $distid==18) echo "Game 1 played at Perkins County High School<br><br>";
echo "<br>";
...
for($i=1;$i<=$teamct;$i++)
{
   $sql="SELECT * FROM $db_name2.bbgdisttimes WHERE distid='$distid' AND gamenum='$i'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   $day=split("-",$row[day]);
   $field="match".$i;
   if(trim(preg_replace("/[^0-9]/","",$row[time]))=="") $row[time]="&nbsp;";
   ...
   $sql="SELECT sid,ptavg FROM $db_name2.bbgseeds WHERE distid='$distid' AND seed='$i'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   $field="seed".$i;
   if($seeded!='y' || $bracketed!='y')
      ...
      $$field="<font style=\"font-size:9pt;\">#$i ".GetSchoolName($row[sid],'bbg',$year)." $row[ptavg] (".GetWinLoss($row[sid],'bbg',$year).")</font>";
   $field="sid".$i;
   if($seeded!='y' || $bracketed!='y')
      $$field='';
   else
...
{
   //MATCH 1:
   $sql="SELECT * FROM $db_name.bbgsched WHERE distid='$distid' AND gamenum='1'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   if($row[oppscore]!="" && $row[sidscore]!="")	//get Match 3 info
   {
	 ...
	 $winner1sid=$row[oppid]; $score1="$row[oppscore]-$row[sidscore]";
      ...
      $winner1="<font style=\"font-size:9pt;\">".GetSchoolName($winner1sid,'bbg',$year)." ($score1)</font>";

      $sql="SELECT * FROM $db_name.bbgsched WHERE distid='$distid' AND gamenum='3'"; 
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
	 ...
	 $winner3="<font style=\"font-size:9pt;\">".GetSchoolName($winner3sid,'bbg',$year)." ($score3)</font>";
      }
      else $winner3="";
   }
   else
   ...
   //MATCH 2:
   $sql="SELECT * FROM $db_name.bbgsched WHERE distid='$distid' AND gamenum='2'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   if($row[oppscore]!="" && $row[sidscore]!="") //get Match 4 info
   {
	 ...
	 $winner2sid=$row[oppid]; $score2="$row[oppscore]-$row[sidscore]";
      ...
      $winner2="<font style=\"font-size:9pt;\">".GetSchoolName($winner2sid,'bbg',$year)." ($score2)</font>";

      $sql="SELECT * FROM $db_name.bbgsched WHERE distid='$distid' AND gamenum='4'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
         ...
         else 
	 {
	    $winner4sid=$row[oppid]; $score4="$row[oppscore]-$row[sidscore]";
         ...
         $winner4="<font style=\"font-size:9pt;\">".GetSchoolName($winner4sid,'bbg',$year)." ($score4)</font>";
      }
      else $winner4="";
   }
   else
   ...
   if($winner3!='' && $winner4!='')
   {
      $sql="SELECT * FROM $db_name.bbgsched WHERE distid='$distid' AND gamenum='5'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);
	 ...
	 $winner5=GetSchoolName($winner5sid,'bbg',$year)." ($score5)";
      }
      else $winner5="WINNER";
   }
   else $winner5="WINNER";
