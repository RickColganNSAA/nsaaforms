...
if($submit=="Rescore $sportname Tests")
{
   ...
   while($row=mysql_fetch_array($result))
   {
	 ...
	 else	//wrong answer
	 {
      ...
      //update JUDGES table too
      $sql2="UPDATE judges SET ".$sport."test='$correct' WHERE id='$row[offid]'";
      $result2=mysql_query($sql2);
      //echo "$sql2<br>";
   }
}
...
echo $init_html;
echo GetHeaderJ($session,"jtestreport");
echo "<br>";
echo "<form method=post action=\"jtestreport.php\">";
echo "<input type=hidden name=session value=\"$session\">";
echo "<select name=sport><option value=''>Choose Sport</option>";
echo "<option value=\"pp\"";
if($sport=='pp') echo " selected";
echo ">Play Production</option><option value=\"sp\"";
...
echo "<table cellspacing=0 cellpadding=3 frame=all rules=all style=\"border:#808080 1px solid;\"><caption><b>$sportname Online Tests Report:</b><br><font style=\"font-size:8pt\">(Click column header to sort by that field)</font><br>";
if($showscore!='all')
   echo "<a class=small href=\"jtestreport.php?sport=$sport&session=$session&showscore=all\">View ALL $sportname Tests</a>";
else
   echo "<a class=small href=\"jtestreport.php?sport=$sport&session=$session\">View $sportname Tests that have NOT been removed from view ONLY</a>";
echo "<br></caption>";

//get all online test-submitters and show name, score, ones missed:
$sql="SELECT t1.last,t1.first,t1.middle,t2.offid,t2.datetaken,t2.correct,t2.missed FROM judges AS t1, $answers AS t2 WHERE t1.id=t2.offid AND t2.datetaken!='' ";
if($showscore!='all')
...
echo "<th class=smaller><a class=small href=\"jtestreport.php?session=$session&sport=$sport&sort=name\">Name</a></th>";
echo "<th class=smaller><a class=small href=\"jtestreport.php?session=$session&sport=$sport&sort=date\">Date Taken</a></th>";
$sql2="SELECT id FROM ".$sport."test";
$result2=mysql_query($sql2);
$total=mysql_num_rows($result2);
echo "<th class=smaller><a class=small href=\"jtestreport.php?session=$session&sport=$sport&sort=score\">Score</a><br>(out of $total)</th>";
...
while($row=mysql_fetch_array($result))
{
   ...
   echo "<td><a class=small target=new href=\"edit_judge.php?session=$session&id=$row[3]&header=no\">$row[first] $row[middle] $row[last]</a></td>"; 
