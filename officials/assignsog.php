...
echo "<form name=assignform method=post action=\"assignsog.php\">";
...
if($sport && $sport!='~' && $type && $type!="~" && ($type=="State" || ($distid && $distid!='~')))
{
   ...
   if($type=="State" || ($type!="State" && mysql_num_rows($result2)>0))
      ...
      echo "<tr align=center><td colspan=2>[No time slots have been entered for this district yet.  Please <a class=small href=\"hostbyhost.php?sport=sog&distid=$distid&session=$session\">Click Here</a> to add time slots for this district.]</td></tr>";
   echo "</table>";
}
echo "</form>";
