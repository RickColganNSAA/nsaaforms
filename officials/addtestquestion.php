...
/***************************************************
addtestquestion.php
Add a question to the pool for Part 1 online test
...
if($save && trim($question)!='')
{
   ...
   if($save=="Save Question")
      header("Location:addtestquestion.php?session=$session&quesid=$quesid&sport=$sport&edited=1");
   else
      header("Location:addtestquestion.php?session=$session&quesid=$quesid&sport=$sport&added=1");
}

echo $init_html;
echo GetHeader($session,"testreport");
echo "<br>";
echo "<a href=\"edittest.php?session=$session&sport=$sport\" class=small>Return to Edit $sportname Questions</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href=\"testreport.php?session=$session&sport=$sport\" class=small>Return to $sportname Online Test Admin</a><br><br>";
echo "<form method=post action=\"addtestquestion.php\">";
...
if($quesid)
{
   ...
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   echo "Edit $sportname Part 1 Question #$row[place]:";
}
else 
{
   $sql="SELECT * FROM $test ORDER BY place DESC LIMIT 1";
   $result=mysql_query($sql);
   $lastq=mysql_fetch_array($result);
   if(mysql_num_rows($result)==0) $place=1;
   else $place=$lastq[place]+1;
   echo "Add a New PART 1 $sportname Test Question (#$place):";
}
echo "</b><br>";
if($quesid)
{
   ...
   $nextid=$row2[id];
   if($previd)
      echo "<div style=\"float:left;\"><a href=\"addtestquestion.php?sport=$sport&session=$session&quesid=$previd\"><< Previous Question</a></div>";
   if($nextid)
      echo "<div style=\"float:right;\"><a href=\"addtestquestion.php?sport=$sport&session=$session&quesid=$nextid\">Next Question >></a></div>";
   echo "<div style='clear:both;'></div>";
}
if($added==1)
   echo "<div class=alert>Your question has been added! <a href=\"addtestquestion.php?sport=$sport&session=$session\">Add Another Question</a></div>";
else if($edited==1)
