...
echo "<center><br>";
echo "<form name=\"test_form\" method=post action=\"wrtest_update.php\">";
...
echo "<table width=90%>";
echo "<caption><b>$thisyear Wrestling Rules Examination - Part I</b><br>";
if(GetLevel($session)==1)
{
   echo "for <font style=\"color:red\"><b>".GetOffName($offid)."</b></font><br>";
}
$date=split("-",GetTestDueDate("wr"));
...
echo "<tr align=left><td align=left colspan=2><b>Every part of each question is to be answered. Indicate whether you believe a part is true or false by checking the appropriate circle.</td></tr>";
echo "<tr align=center><td align=center colspan=2><hr><table>";
$sql="SELECT category FROM wrtest_categ WHERE id='$categid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$category=$row[0];
echo "<tr align=left><th align=left colspan=2>$category:</th></tr>";
//get answers already entered by this official
$sql="SELECT * FROM wrtest_results WHERE offid='$offid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
for($i=1;$i<=100;$i++)
{
...
$sql="SELECT question,place FROM wrtest WHERE category='$categid' ORDER BY place";
$result=mysql_query($sql);
$ix=0;
