...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE bbgcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE district='1'";
   $result=mysql_query($sql);
}
...
echo "<br>";
echo "<form method=post action=\"bbgcontract.php\">";
...
if($edit==1)
   echo "<a class=small href=\"bbgcontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1 && $level==1)
   echo "<a class=small href=\"bbgcontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($type!='State')
{
   ...
   $sql="SELECT class,district,director,dates,hostschool,site,schools,type FROM $districts WHERE id='$distid'";
   $result=mysql_query($sql);
   //echo $sql;
   $row=mysql_fetch_array($result);
   if($edit==1)
   ...
   if($sample==1)
      ...
      echo "Girls $row[type] $row[class]-$row[district]";
   if($edit==1)
   ...
   {
      $sql3="SELECT DISTINCT time FROM $disttimes WHERE day='$row2[day]' AND (";
      for($i=0;$i<count($gameid);$i++)
      {
         $sql3.="id='$gameid[$i]' OR ";
      }
