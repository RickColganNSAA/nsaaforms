...
if($edit==1 && $savechanges)
{
   ...
   $sql="UPDATE sbcontracttext SET text1='$text1',text2='$text2',text3='$text3',text4='$text4' WHERE district='1'";
   $result=mysql_query($sql);
}
...
$sql="SELECT * FROM sbcontracttext WHERE district='$district'";
$result=mysql_query($sql);
...
echo "<form method=post action=\"sbcontract.php\">";
...
if($edit==1)
   echo "<a class=small href=\"sbcontract.php?session=$session&sample=1\">Preview this Contract</a>";
else if($sample==1 && $level==1)
   echo "<a class=small href=\"sbcontract.php?session=$session&edit=1\">Edit this Contract</a>";
echo "</td></tr>";
...
if($submit)
{
   ...
   while($row2=mysql_fetch_array($result2))
   {
      ...
      while($row3=mysql_fetch_array($result3))
      {
         ...
         $string.="$row3[time]";
         //check if crew chief
	 $sql4="SELECT crewchief FROM $contracts WHERE offid='$offid' AND disttimesid='$row3[id]'";
	 $result4=mysql_query($sql4);
 	 $row4=mysql_fetch_array($result4);
         if($row4[crewchief]=='x') $string.=" (You are the Crew Chief)";
	 $string.="/";
      }
      $string=substr($string,0,strlen($string)-1);
      $string.="</td></tr>";
   ...
   if($sample=='1')
   {
      $string="<tr align=left><td>October 5:</td><td>5:00 PM CST (You are the Crew Chief)/7:30 PM CST</td></tr><tr align=left><td>October 6:</td><td>11:00 AM CST</td></tr>";
      echo $string;
   }
   echo "</table></td></tr>";
   ...
   for($i=0;$i<count($gameid);$i++)
   {
      ...
      $sql="SELECT offid,crewchief FROM $contracts WHERE disttimesid='$gameid[$i]' ORDER BY crewchief DESC";
      $result=mysql_query($sql);
      $partners="";
      while($row=mysql_fetch_array($result))
      {
         $partners.=GetOffName($row[offid]);
	 if($row[crewchief]=='x') $partners.=" (Crew Chief)";	
	 $partners.=", ";
      }
   ...
   if($sample=='1')
   {
      ...
      echo "<tr align=left><td>October 5, 5:00 PM CST:</td><td>$partners1</td></tr>";
      echo "<tr align=left><td>October 5, 7:30 PM CST:</td><td>$partners</td></tr>";
      echo "<tr align=left><td>October 6, 11:00 AM CST:</td><td>$partners</td></tr>";
   }
   echo "</table></td></tr>";
...
{
   echo "<tr align=left><td><br><br><input type=radio name=confirm value='y'>&nbsp;&nbsp;&nbsp;The NSAA confirms the above agreement for the $thisyear $type Softball Tournament";
   echo ".</td></tr>";
