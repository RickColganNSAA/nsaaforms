<?php
//coop_vb.php: pop-up window to add coop-students to district entry
...
for($i=0;$i<8;$i++)
{
   ...
   if($check[$i]=='y' && $coop_school[$i]!="Choose School" && $coop_student[$i]!="Choose Student")
   {
      ...
      {
         $sql="INSERT INTO $table (student_id, school, checked, class_dist, co_op) VALUES ('$coop_student[$i]','$coop_school[$i]','y','$class_dist','$school2')";
         $result=mysql_query($sql);
      }
   }
}
...
<body>
<center>
<form name=form1 method="post" action="coop_vb.php">
...
for($i=0;$i<8;$i++)
{
   ...
   {
      $sql="SELECT id, last, first, middle, semesters FROM eligibility WHERE vb='x' AND school='$coop_school[$i]' ORDER BY last";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
