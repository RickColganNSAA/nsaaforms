	...
	if ( !empty($_FILES) )
	{
		...
		for($k = 1; $k < 100; $k++)
		{
			...
			else
			{
				...
				$uniqueFileName = sprintf("%s/vb_teamphoto_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
                                $uniqueThumbName = sprintf("%s/vb_teamphoto_resized_%s_%s.%s", $pathInfo['dirname'], $sid, $k, $pathInfo['extension']);
			}
...
/*
	...
	*/
			...
			$sql="UPDATE vbschool SET filename='$useFileName' WHERE sid='$sid'";
