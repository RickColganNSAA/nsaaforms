...
//IF $makepdf && NOT NSAA - CHECK THAT IT's BEEN APPROVED FOR PROGRAM
if($makepdf && $level!=1)
{  
   $sql="SELECT * FROM vbschool WHERE sid='$sid'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
   if($row[approvedforprogram]==0)
   {
      echo $init_html;
...
if(PastDue($duedate,8.5) && $director!=1)	//state form
{
   $state=1;
   $table="vb_state";
   $form_type="STATE";
...
if($state!=1)
{
   $string.="<tr align=center><td><b>Due $duedate2</b><br><br></td></tr>";
}
...
$string.="<td>".GetSchoolName($sid,'vb',date("Y"))." $mascot</td></tr>";
$csv.="School/Mascot:,".GetSchoolName($sid,'vb',date("Y"))." $mascot\r\n";
...
$string.="<tr align=left><th align=left>Class:</th><td>$class_dist</td></tr>";
...
if($makepdf)
{
   ...
   if(substr($school,0,1)=="W") $ix=0;
   else if(substr($school,0,1)=="T") $ix=1;
   else if(substr($school,0,3)=="Has") $ix=2;
   else if(substr($school,0,3)=="Har") $ix=3;
   else $ix=rand(0,3);
   $teamphoto=$photos[$ix];
...
$string.="<th class=smaller>Height</th><th class=smaller>Digs</th>";
$string.="<th class=smaller>Serve<br>Receptions</th>";
$string.="<th class=smaller>Ace<br>Serves</th>";
$string.="<th class=smaller>Ace<br>Blocks</th>";
$string.="<th class=smaller>Kills</th><th class=smaller>Assists</th></tr>";
if($makepdf)
{
   $smallw=35;
   $html="<table cellspacing=\"0\" cellpadding=\"1\"><tr align=\"center\">
        <td width=\"20\"><b>No.</b></td><td width=\"85\"><b>Name</b></td><td width=\"$smallw\"><b>Grd.</b></td><td width=\"$smallw\"><b>Ht.</b></td><td width=\"$smallw\"><b>Pos.</b></td><td width=\"$smallw\"><b>Digs</b></td><td width=\"$smallw\"><b>Rec.</b></td><td width=\"$smallw\"><b>Aces</b></td><td width=\"$smallw\"><b>Blocks</b></td><td width=\"$smallw\"><b>Kills</b></td><td width=\"$smallw\"><b>Assists</b></td></tr>";
}
...
while($row=mysql_fetch_array($result))
{
  if($row[7]=="y")	//that student was checked to be on the roster
  {
     ...
     $html.="<tr align=\"center\"><td width=\"20\">$row[3]&nbsp;</td><td width=\"85\" align=\"left\">$first $last</td><td width=\"$smallw\">$year</td><td width=\"$smallw\">$height</td><td width=\"$smallw\">$row[14]</td><td width=\"$smallw\">$row[8]</td><td width=\"$smallw\">$row[9]</td><td width=\"$smallw\">$row[10]</td><td width=\"$smallw\">$row[11]</td><td width=\"$smallw\">$row[12]</td><td width=\"$smallw\">$row[13]</td></tr>";
     $count++;
  }
}
$string.="</table></td></tr>";
...
$sched=GetSchedule($sid,'vb',$year);
   ...
   for($i=0;$i<count($sched[oppid]);$i++)
   {
      if($sched[oppid][$i]!='0')        //only individual games, not tournament names
      {
         $csv.="\"".GetSchoolName($sched[oppid][$i],'vb',$year)."\",\"";
...
$csv.="State Tournament Appearances:,\r\n";
$csv.="Most Recent State Tournament:,\r\n";
