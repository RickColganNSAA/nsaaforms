...
$csv.="\r\n\r\n";
   ...
   //Miscellaneous:
$sql="SELECT t1.id FROM muensembles AS t1, mucategories AS t2 WHERE t1.categid=t2.id AND t2.smlg='Small' AND t2.vocinst IS NULL";
$result=mysql_query($sql);
$smallMcount=0;
while($row=mysql_fetch_array($result))
{
   ...
   $curcount=CountEnsembles($schid,$row[id]);
   $smallMcount+=$curcount;
   $ix++;
}
echo "<tr align=left";
echo "><td>&nbsp;</td><td>You have entered <b><u>$smallMcount</u> Small Miscellaneous Ensemble";
$csv.="\"\",\"You have entered $smallMcount Small Miscellaneous Ensemble";
if($smallMcount!=1)
{   
   echo "s"; $csv.="s";
}
