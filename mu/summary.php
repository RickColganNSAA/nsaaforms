...
echo "<table><caption><b><u>$year1-$year2 OFFICIAL ENTRY FOR NSAA DISTRICT MUSIC CONTEST (Summary)</b></u><br>";
$csv="\"$year1-$year2 OFFICIAL ENTRY FOR NSAA DISTRICT MUSIC CONTEST (Summary)\"\r\n";
...
$csv.="\"District $row[distnum] Coordinators(s):\",\"$row2[coordinator] ($row2[school])\"\r\n";
...
$csv.="\"MUSIC DIRECTORS:\"\r\n";
	 ...
	 echo "<td>".$row2[$schoolf]."</td>";
         ...
         echo "</tr>";
         $csv.="\"".$mudirs[$i]."\",\"".$row2[$name]."\",\"".$row2[$email]."\",\"".$row2[$school3]."\",\"".$row2[$schoolf]."\",\"".$row[home]."\"\r\n";
      }
      echo "</table></td></tr>";
echo "</table></td></tr>";
echo "<tr valign=top align=center><td><table border=1 bordercolor=#000000 cellspacing=0 cellpadding=1><tr align=center valign=top><td>";
...
for($j=0;$j<count($categs[id]);$j++)
{
   ...
   if($vieworder==2 || $vieworder==3 || $vieworder==5 || $vieworder==7) 	//don't make new line
      echo "</td><td>";
   else if($vieworder==4 || $vieworder==6) //make new line
      echo "</td></tr></table></td></tr><tr align=center><td><table border=1 bordercolor=#000000 cellspacing=0 cellpadding=1><tr align=center valign=top><td>";
   
   echo "<table cellspacing=2 cellpadding=1><caption><font style=\"font-size:9pt\"><b>".strtoupper($categ).":</b></font></caption>";
   ...
   else if(!ereg("Solo",$categ))	//Everything but Solos 
   {
	 ...
	 $curct=CountEnsembles($schid,$row[id]);
         $ensembleid=$row[id];
	 if($curct>0)
	    $string.="[$curct]&nbsp;$row[ensemble]<br>";
         $csv.="\"$row[ensemble]\",\"$curct\"\r\n";
	    ...
	    {
		$string.=$row1[event];
         	if($row1[strings]=='x') $string.=" (All Strings Ensemble), ";
	        else $string.=", ";
	    }
	    $string.="No. in Group: $groupsize<br>";
	    $ct++;
	 }
	 if(mysql_num_rows($result1)>0) $string.="<br>";
	 ...
	 $curcol++;
      ...
      echo "<tr align=right><td colspan=2><b>Total Fee: $<u>".$curfee."</u></b></td></tr>";
      $csv.="\"\",\"Total Fee:\",\"$".$curfee."\"\r\n\r\n";
      echo "</table><br>";
   }//end if NOT Solos
   else		//SOLOS
   {
      echo "<table><tr align=left><td><b>Event:</b></td><td><b>Contestant:</b></td></tr>";
      $csv.="\"\",\"Event:\",\"Contestant:\"\r\n";
      $sql="SELECT * FROM muensembles WHERE categid='$categid'";
      $result=mysql_query($sql);
      $row=mysql_fetch_array($result);

      //show existing solos:
      $sql2="SELECT t1.*,t2.first,t2.last FROM muentries AS t1, eligibility AS t2 WHERE t1.studentid=t2.id AND t1.ensembleid='$row[id]' AND t1.schoolid='$schid' ORDER BY t2.last,t2.first";
      $result2=mysql_query($sql2);
	 ...
	 echo "</tr>";
         $csv.="\"$place\",\"$row2[event]\",\"$row2[first] $row2[last]\"\r\n";
         $ix++;
      }
      $curfee=number_format(CountEnsembles($schid,$row[id])*GetEnsembleFee($distid,$row[id]),2,'.','');
      echo "<tr align=right><td colspan=3><b>Total Fee: $<u>".$curfee."</u></b></td></tr>";
      $csv.="\"\",\"\",\"Total Fee:\",\"$".$curfee."\"\r\n";
      echo "</table>";
   }
   $csv.="\r\n";
...
$totalfee=0;
echo "<tr align=right><td><br><table bordercolor=#000000 cellspacing=0 cellpadding=2 frame=\"border\" rules=\"none\">";
echo "<tr align=left><td colspan=2><b><u>ENTRY FEES SUMMARY:</u></b></td></tr>";
$csv.="\"ENTRY FEES SUMMARY:\"\r\n";
...
//echo "<a class=small target=new2 href=\"/home/nsaahome/attachments/test.csv\">TESTING CSV</a>";
echo $end_html;
?>
