<?php
//hostapp_bb_g.php: site survey for Girls Basketball
...
echo $init_html;
if($nsaa!=1 && $level!=1 && $level!=5) echo $header;
else
   echo "<table width=100%><tr align=center><td><br><a href=\"javascript:window.close();\" class=small>Close</a><br><br>";

$curryear=date("Y",time());
if(date("m")<6) $curryear--;
$curryear1=$curryear+1;
//get due date of this site survey
$sql="SELECT duedate FROM app_duedates WHERE sport='bb_g'";
$result=mysql_query($sql);
...
{
   $sql2="INSERT INTO hostapp_bb_g (school) VALUES ('$school2')";
   $result2=mysql_query($sql2);
   $result=mysql_query($sql);
}
...
echo "<h3>Application to Host a $curryear-$curryear1 GIRLS BASKETBALL District/Sub-District Event</h3>";

if($print!=1) echo "<p>Due $duedate2</p>";

echo "<form method=post action=\"hostapp_bb_g.php\">";
...
if($submitapp && (!PastDue($duedate) || $level==1))
   echo "<font style=\"color:red\" size=2><b><i>Your application to host Girls Basketball District/Sub-District Events has been submitted.  Below is what you have submitted.  You may make changes to this application until the above due date.</i></b></font>";
...
if(PastDue($duedate) && $level!=1 && $school!="Test's School")
{
   ...
   {
      echo "<tr align=center><td><table><tr align=left><td><b>You did NOT submit an Application to Host Girls Basketball District/Subdistrict Events.  The due date for this application is past.</b></td></tr></table></td></tr>";
      echo "</table>";
      echo $end_html;
      exit();
   }  
...
echo "<tr align=left><th align=left>1) Are you interested in hosting any NSAA district contests for Girls Basketball?</th></tr>";
...
echo "<tr align=left><td colspan=2>(Before indicating availability of facilities, be certain your school can host on the date(s) indicated.)</td></tr>";

//GET DATES FROM tourndates table
$sql2="SELECT * FROM $db_name2.bbtourndates WHERE girls='x' AND hostdate='x' ORDER BY tourndate,label";
$result2=mysql_query($sql2);
$ix=0;
...
echo "<tr align=left><td colspan=2>(NOTE: Classes C1, C2, D1, D2 District Finals sites to be determined by sub-district winners.)</td></tr>";
echo "<tr align=left><td align=right><input type=checkbox name=classa value='y'";
if($row[classa]=='y') echo " checked";
if($print==1) echo " disabled";
echo "></td><td>Class A District (Tuesday-Thursday OR Tuesday-Friday)</td></tr>";
...
if($print==1)
   echo "<p>Teams: $row[teams], Spectators: $row[spectators], Parking: $row[parking], Locker Rooms: $row[lockers]</p></td></tr>";
else
{
...
echo ">No</td></tr>";
echo "<tr align=left><td colspan=2><br>";
echo "If your school is selected as a district host in girls basketball, who will serve as district director?";
if($print==1) echo "<p><i>$row[director]</i></p></td></tr>";
else echo "<br><input type=text name=director size=25 value=\"$row[director]\"></td></tr>";
echo "</table></td></tr>";
}
...
else echo "<br><textarea rows=5 cols=60 name=\"comments\">$row[comments]</textarea></td></tr>";
if($print!=1)
{
