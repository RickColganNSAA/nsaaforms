<?php
//view_ubo.php: Show submitted district
//   entry info.  If none have been submitted,
//   redirect to edit_ubo.php
...
else if($director==1)
{
   ...
   $hostid=$row[0];
   $sql="SELECT * FROM $db_name2.ccgdistricts WHERE hostid='$hostid'";
   $result=mysql_query($sql);
...
if(mysql_num_rows($result)>0)	//user is district host
{
   ...
   {
      header("Location:state_cc_g_view.php?session=$session&school_ch=$school_ch");
      exit();
   }
}
//get class_dist for this team, if submitted
$sql="SELECT class_dist FROM ubo WHERE school='$school2'";
$result=mysql_query($sql);
$class_dist="";
...
//get due date for Girls cross-country form:
$sql="SELECT duedate FROM form_duedates WHERE form='ubo'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$duedate=$row[0];
$date=split("-",$duedate);
...
// $sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM cc_g AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND ((t1.school='$school2' AND t2.school='$school2') OR (t1.co_op='$school2' AND t2.school=t1.school)) AND t1.checked='y' ORDER BY t2.last"; 
  $sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM ubo AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND ((t1.school='$school2' AND t2.school='$school2') OR (t1.co_op='$school2' AND t2.school=t1.school)) AND t1.checked='y' ORDER BY t2.last"; 
$result=mysql_query($sql); 
...
if($level==1)
   echo "<br><a class=small href=\"../welcome.php?session=$session&toggle=menu3&menu3sport=Cross-Country\">Return to Home-->Unified Bowling Entry Forms</a><br>";
}

//get in ation about school and coach:
$sql2="SELECT * FROM headers WHERE school='$school2'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
$colors=$row2[5];
$mascot=$row2[6];
$sql2="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Unified Bowling'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
...
{
   echo "<br><br>";
   echo "<a class=small href=\"view_ubo.php?session=$session&school_ch=$school_ch&print=1\" target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
   echo "<a class=small href=\"edit_ubo.php?session=$session&school_ch=$school_ch\">Edit this Form</a>&nbsp;&nbsp;&nbsp;";
   if($level==1 || $level==2)
      echo "<a class=small href=\"view_ubo.php?session=$session&school_ch=$school_ch\">Go to UNIFIED BOWLING District Entry Form</a>";
   echo "<br><br>";
}

$string.="<table>";
$string.="<tr align=center>";
$string.="<th>UNIFIED BOWLING ENTRY</th>";
$string.="</tr>";
$string.="<tr align=center>";
...
$sid=GetSID2($school,'ubo');
//$sql2="SELECT * FROM ccgschool WHERE sid='$sid'";
...
$string.="<td>".GetSchoolName($sid,'ubo',date("Y"))." $mascot</td></tr>";
$csv.="School/Mascot:, ".GetSchoolName($sid,'ubo',date("Y"))." $mascot\r\n";
...
if($print!=1)
{
   ...
   if($count>5 )
   {
      ...
      if($count==7 && ($class_dist=='' || !$class_dist))
         echo "<br>If you are a Class A school, make sure to select \"A\" as your class on the <a href=\"edit_ubo.php?session=$session\">Edit Screen</a>";
      echo "</font>";
      echo "</th></tr>";
   }
?>
<tr align=center>
<td><br>
    <a class=small href="view_ubo.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>&print=1" target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;
    <a class=small href="edit_ubo.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>">Edit this Form</a>&nbsp;&nbsp;&nbsp;
