<?php
//ubostateadmin.php: NSAA Volleyball Admin for State Entry Forms
...
if($save)
{
   ...
   for($i=0;$i<count($sid);$i++)
   {
      ...
      {
	 $sql="UPDATE uboschool SET approvedforprogram='".time()."' WHERE sid='$sid[$i]' AND approvedforprogram=0";
         $result=mysql_query($sql);
         $sql="UPDATE uboschool SET programorder='$programorder[$i]' WHERE sid='$sid[$i]'";
         $result=mysql_query($sql);
      }
      else
      {
         $sql="UPDATE uboschool SET approvedforprogram='0' WHERE sid='$sid[$i]'";
         $result=mysql_query($sql);
   ...
   $sql="SELECT DISTINCT programorder FROM uboschool WHERE programorder>0";
   $result=mysql_query($sql);
   $ct1=mysql_num_rows($result);
   $sql="SELECT programorder FROM uboschool WHERE programorder>0";
   $result=mysql_query($sql);
   $ct2=mysql_num_rows($result);
   if($ct1!=$ct2)
   {
      $sql="SELECT programorder,COUNT(programorder) FROM uboschool WHERE programorder>0  AND class='$class' GROUP BY programorder";
      $result=mysql_query($sql);
...
echo "<form method=post action=\"ubostateadmin.php\">";
...
if(mysql_num_rows($result)==0 || $viewdistrict==1)
{
   ...
   if(mysql_num_rows($result)>0) echo "<br>[<a class=small href=\"ubostateadmin.php?session=$session\">View schools who have submitted a STATE form</a>]";
   echo "<br><br>";
   echo "</caption>";
   $sort="t2.school";
   ...
   $result=mysql_query($sql);
echo mysql_error();
   echo "<tr align=center><td><a class=small href=\"ubostateadmin.php?session=$session&sport=$sport&sort=t1.school\">School</a>";
   if($sort=="t1.school") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td><td><a class=small href=\"ubostateadmin.php?session=$session&sport=$sport&sort=t2.filename%20DESC\">Team Photo</a>";
...
else
{
   ...
   $sql2="SELECT DISTINCT class FROM uboschool WHERE class!='' ORDER BY class";
   ...
   echo "</select>";
   echo "</p></div>";
   echo "<p><a href=\"ubostateadmin.php?session=$session&viewdistrict=1\">Click Here</a> to view the list of schools with District Entry Forms (with the Team Photo column included).</p>";
   echo "</caption>";
   echo "<tr align=center><td><a class=small href=\"ubostateadmin.php?session=$session&sport=$sport&sort=school&class=$class\">School</a>";
   if($sort=="school") echo "<img style='width:15px;float:right' src='../arrowdown.png' border='0'>";
   echo "</td><td><b>Submitted Roster</b></td><td><a class=small href=\"ubostateadmin.php?session=$session&sport=$sport&sort=submitted%20DESC&class=$class\">Date Submitted</a>";
   if($sort=="submitted DESC") echo "<img style='width:15px;float:right' src='../arrowup.png' border='0'>";
   echo "</td>";
   if($class && $class!='')
   {
      echo "<td><a class=small href=\"ubostateadmin.php?session=$session&sport=$sport&sort=programorder%20ASC&class=$class\">PROGRAM<br>ORDER</a>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT * FROM uboschool WHERE sid='$sid'";
   if($class && $class!='')
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a class=small href=\"view_ubo.php?school_ch=$row[school]&session=$session\" target=\"_blank\">View Form</a>&nbsp;|&nbsp;<a class=small href=\"view_ubo.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>&nbsp;|&nbsp;<a class=small href=\"../attachments.php?session=$session&filename=$filename\">Download for Excel</a>";
   if($statesubmitted==0)
      $links[$ix]="<a class=small href=\"view_ubo.php?school_ch=$row[school]&session=$session&viewdistrict=1&makepdf=1\" target=\"_blank\">Preview PDF</a>";
...
for($i=0;$i<count($orders);$i++)
{
   ...
   $sql2="SELECT approvedforprogram,programorder,filename FROM uboschool WHERE sid='$sid'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
