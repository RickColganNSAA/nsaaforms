<?php
//edit_ubo.php: Unified Bowling entry form

//check if user needs to be re-directed:
if(!empty($submit) && $submit=="Home")
{
...
$schoolid=$row[id]; $sport="ubo"; //$sport="go_g"; 
...
$sql="SELECT DISTINCT class FROM $db_name2.ubodistricts WHERE class!='' ORDER BY class";
$result=mysql_query($sql);
$class_array=array();
$i=0;
...
$sql="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Unified Bowling'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
...
if(PastDue($duedate,0) && $level!=1)
{
   ...
   $sql="SELECT * FROM ubo WHERE (school='$school2' OR co_op='$school2')";
   $result=mysql_query($sql);
   $ct=mysql_num_rows($result);
   if($ct>0)
   {
      echo "<a href=\"view_ubo.php?session=$session&school_ch=$school_ch\">";
...
if($level==1)
   echo "<br><a class=small href=\"../welcome.php?session=$session&toggle=menu3&menu3sport=Unified Bowling\">Return to Home-->Unified Bowling Entry Forms</a><br>";
?>
<form name=form1 method=post action="submit_ubo.php">
<input type=hidden name=session value=<?php echo $session; ?>>
<input type=hidden name=school_ch value="<?php echo $school_ch; ?>">
<table width=80%>
<tr align=center>
<th>UNIFIED BOWLING DISTRICT ENTRY</th>
</tr>
<tr align=center>
...
echo GetSchoolName($sid,'ubo')." $mascot";
   ...
   <tr align=left>
   <th>Class:</th><td><?php echo GetClass($sid,'ubo'); ?></td>
   </td>
   </tr>
   </table>
</td>
...
<?php
   //unified participants participants from eligibility table in db
	/*
   ...
   //echo '<pre>'; print_r($studs); print_r($sport); exit;
   //while($row=mysql_fetch_array($result))
