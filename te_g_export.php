...
$string="";
$sql="SELECT * FROM te_g ORDER BY school";
...
$sql="SELECT DISTINCT school FROM te_g ORDER BY class_dist";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result))
{
   ...
   $sql2="SELECT name FROM logins WHERE level=3 AND sport='Girls Tennis' AND school='$school2'";
   $result2=mysql_query($sql2);
   $row2=mysql_fetch_array($result2);
   $coach1=$row2[0];

   if($row[0]!="Test's School")
      $string.="$class,$row[0],$coach1,$s1name,$s1grade,$s1record,$s2name,$s2grade,$s2record,$d1name1,$d1grade1,$d1name2,$d1grade2,$d1record,$d2name1,$d2grade1,$d2name2,$d2grade2,$d2record\r\n";
}

$open=fopen(citgf_fopen("/home/nsaahome/reports/teplayers.txt"),"w");
fwrite($open,$string);
fclose($open); 
 citgf_makepublic("/home/nsaahome/reports/teplayers.txt");
header("Location:exports.php?session=$session&filename=teplayers.txt");
?>
