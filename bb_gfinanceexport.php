<?php
//bb_gfinanceexport.php: export file of all bb_g financial report data
...
$bb_gclass=array("A","B","C1","C2","D1","D2");
$bb_gfiles=array("C1Sub","C2Sub","D1Sub","D2Sub","C1","C2","D1","D2","B","A");

$bb_gfields=array("classdist","location","grossreceipts","offtotal","insdeduct","hostgiven","hostallow","nsaaallow","balance","milespaid","milespaidtot","prorate","milespaidpro","milespaidprotot","bonus","hostperc","hostbonus","nsaabonus","visperc","visitorbonus","grossreceipts","offtotal","hosttotal","hostexp","nsaapartial","insdeduct","nsaatotal");

$bb_gtitles=array("District","Site","Total Receipts","Officials","Insurance-10%","Host Allowance","Host School","NSAA","Balance","Expenses","Total Expenses","Prorate","Expenses","Expenses Paid","Balance Bonus","% Host","Host School","NSAA","% Teams","Teams","Total Receipts","Officials","Host","Teams","NSAA","Insurance","Check Amount");
...
for($file=0;$file<count($bb_gfiles);$file++)
{
    $csv="";
    ...
    if(ereg("Sub",$bb_gfiles[$file]))
    {
       $round=1;
       $class=substr($bb_gfiles[$file],0,2);
    }
    else
    {
       $round=2;
       $class=$bb_gfiles[$file];
    }

    // FOR EACH tournament in this (sub)district, CREATE column OF CSV FILE: 
    $sql="SELECT * FROM finance_bb_g WHERE class='$class' AND round='$round' ORDER BY district";
    $result=mysql_query($sql);
    $tid=0;	//tournament id
    $tourn=array();
    while($row=mysql_fetch_array($result))
    {
       // GET (SUB)DISTRICT INFO FROM finance_bb_g: 
       ...
       // FOR EACH GAME's REPORT in finance_bb_g_exp, STORE INFO: 
       $sql2="SELECT * FROM finance_bb_g_exp WHERE class='$class' AND district='".$tourn[district][$tid]."' AND round='$round' ORDER BY school";
   ...
   for($field=0;$field<count($bb_gfields);$field++)
   {
      // FOR EACH DATA FIELD
	 ...
	 $csv.="\r\n";
	 $csv.=$bb_gtitles[$field].",";
	 for($t=0;$t<$tid;$t++)
	 {
	    $csv.=$tourn[$bb_gfields[$field]][$t].",";
	 ...
	 for($t=0;$t<$tid;$t++)
	 {
	    if($bb_gfields[$field]=="grossreceipts")
	       $totalrec+=$tourn[$bb_gfields[$field]][$t];
	    else if($bb_gfields[$field]=="offtotal")
	       $totaloff+=$tourn[$bb_gfields[$field]][$t];
	    else if($bb_gfields[$field]=="hosttotal")
	       $totalhost+=$tourn[$bb_gfields[$field]][$t];
	    else if($bb_gfields[$field]=="hostexp")
	       $totalvis+=$tourn[$bb_gfields[$field]][$t];
	    else if($bb_gfields[$field]=="nsaapartial")
	       $totalnsaa+=$tourn[$bb_gfields[$field]][$t];
	    else if($bb_gfields[$field]=="insdeduct")
	       $totalins+=$tourn[$bb_gfields[$field]][$t];
	 }
      }
...
//SUMMARY CSV FILE:
$csv="";
$csv.="$curryear SUMMARY DISTRICT GIRLS BASKETBALL RECEIPTS\r\n\r\n\r\n\r\n";
...
$filename="bb_gsummary$curryear.csv";
$open=fopen(citgf_fopen("financialreports/$filename"),"w");
fwrite($open,$csv);
fclose($open); 
 citgf_makepublic("financialreports/$filename");
