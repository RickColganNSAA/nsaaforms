<?php
//submit_so_g.php: submits form info for girls soccer.
//  Form is submitted from edit_so_g.php
...
//HISTORICAL INFO
$sql="UPDATE sogschool SET tripstostate='$tripstostate',mostrecent='$mostrecent',championships='$championships',runnerup='$runnerup' WHERE sid='$sid'";
$result=mysql_query($sql);
if(mysql_error()) { echo mysql_error(); exit(); }
   ...
   if(!empty($_FILES["imageUpload"]["name"])){
	...
	$sql="UPDATE sogschool SET filename='$image'WHERE sid='$sid'";
    $result=mysql_query($sql);		
    }
//store assistant coaches listed
$asst_coaches=ereg_replace("\'","\'",$asst_coaches);
$sql="UPDATE logins SET asst_coaches='$asst_coaches' WHERE school='$school2' AND level=3 AND sport='Girls Soccer'";
$result=mysql_query($sql);
...
if($count>24)
{
   header("Location:edit_so_g.php?session=$session&school_ch=$school_ch&counterror=1");
   exit();
}
else if($send=='y')	//auto send to view_so_g.php and e-mail data to NSAA
{
   header("Location:view_so_g.php?session=$session&school_ch=$school_ch&send=y");
   exit();
}
...
{
   header("Location:view_so_g.php?session=$session&school_ch=$school_ch");
   exit();
}
?>
