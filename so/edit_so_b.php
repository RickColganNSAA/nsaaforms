...
{
   //$sql2="INSERT INTO $db_name.so_bstate SELECT * FROM $db_name.so_b WHERE (school='$school2' OR co_op='$school2')";
   $sql2="SELECT * FROM $db_name.so_b WHERE (school='$school2' OR co_op='$school2')";
   $result2=mysql_query($sql2);
   while($row2=mysql_fetch_array($result2))
   {
      $curcoop=addslashes($row2[co_op]);
      $curschool=addslashes($row2[school]);
      $sql3="INSERT INTO $db_name.so_bstate (nickname,student_id,checked,class_dist,record,jersey_lt,jersey_dk,position,goals,assists,gk_games,gk_goals_allowed,gk_saves,co_op,school) VALUES ('".addslashes($row2[nickname])."','$row2[student_id]','$row2[checked]','$row2[class_dist]','$row2[record]','$row2[jersey_lt]','$row2[jersey_dk]','$row2[position]','$row2[goals]','$row2[assists]','$row2[gk_games]','$row2[gk_goals_allowed]','$row2[gk_saves]','$curcoop','$curschool')";
...
?>
<center>
<form method=post action="submit_so_b.php" enctype="multipart/form-data">
<input type=hidden name=session value=<?php echo $session; ?>>
<input type=hidden name=school_ch value="<?php echo $school_ch; ?>">
<input type=hidden name=state value=<?php echo $state; ?>>
<table style="width:100%;">
<tr align=center>
<th>BOYS SOCCER <?php echo $form_type; ?> ENTRY</th>
</tr>
<?php
if($state!=1)
{
...
$class=$row['class'];
echo GetSchoolName($sid,'sob')." $mascot";
   ?>
   </td>
   </tr>
   <tr align=left>
...
if($level==1)
{
	  ...
	  $sql_coop="SELECT * FROM sobschool WHERE mainsch='$row_id[id]' AND (othersch1!='' OR othersch2!='' OR othersch3!='') ";
...
<?php
   ...
   //get boys soccer participants from eligibility table in db
   //$sql="SELECT id, first, last, middle, semesters, eligible FROM eligibility WHERE school='$school2' and so='x' AND (gender='M' OR (school='Omaha Northwest' AND gender='F')) ORDER BY last";
   $result=mysql_query($sql);
   $ix=0;
   //while($row=mysql_fetch_array($result))
   $studs=explode("<result>",GetPlayers('sob',$school));
   $ix=0;
