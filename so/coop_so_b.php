<?php
//coop_so_b.php: pop-up window to add coop-students to district entry
...
for($i=0;$i<8;$i++)
{
   ...
   if($check[$i]=='y' && $coop_school[$i]!="Choose School" && $coop_student[$i]!="Choose Student")
   {
      ...
      {
         $sql="INSERT INTO $table (student_id, checked, class_dist, co_op, school) VALUES ('$coop_student[$i]','y','$class_dist','$school2','$coop_school[$i]')";
         $result=mysql_query($sql);
      }
   }
}
...
<body>
<center>
<form name=form1 method="post" action="coop_so_b.php">
...
for($i=0;$i<8;$i++)
{
   ...
   {
      $sql="SELECT id, last, first, middle, semesters FROM eligibility WHERE so='x' AND gender='M' AND school='$coop_school[$i]' ORDER BY last";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
