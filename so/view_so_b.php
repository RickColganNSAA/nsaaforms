<?php
//edit_so_b.php: Boys Soccer Entry form
...
else if($director==1)
{
   ...
   $hostid=$row[0];
   $sql="SELECT * FROM $db_name2.sobdistricts WHERE hostid='$hostid'";
   $result=mysql_query($sql);
...
$sql="SELECT duedate FROM form_duedates WHERE form='so_b'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$duedate=$row[0];
$date=split("-",$duedate);
...
$sql="SELECT name, asst_coaches FROM logins WHERE school='$school2' AND sport='Boys Soccer'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$coach=$row[0];
$asst_coaches=$row[1];
...
if($print!=1)
{
   echo "<br><a href=\"view_so_b.php?session=$session&school_ch=$school_ch&print=1\" class=small target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"edit_so_b.php?session=$session&school_ch=$school_ch\" class=small>Edit this Form</a><br><br>";
   if($send=='y')
   {
...
$info.="<th>BOYS SOCCER $form_type ENTRY</th></tr>";
if($state!=1)
{
...
$sid=GetSID2($school,'sob');
$sql="SELECT * FROM sobschool WHERE sid='$sid'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
...
$info.="<td>".GetSchoolName($sid,'sob')." $mascot</td></tr>";
$csv.="School/Mascot:,".GetSchoolName($sid,'sob')." $mascot\r\n";
$info.="<tr align=left>";
...
if($filename!='')
   ...
   $info.="<td><a href=\"edit_so_b.php?session=$session&school_ch=$school_ch\">Click Here to Upload your Team Photo</a></td>";
if($level==1)
{
        //Superintendent
      ...
      $sql2="SELECT * FROM sobschool WHERE sid='$sid'";
      $result2=mysql_query($sql2);
      $row2=mysql_fetch_array($result2);
        //Trips to State: 4
      ...
      if($state==1)
      {
        ...
        <form method=\"post\" action=\"view_so_b.php\" target=\"_blank\">
...
else	//print!=1
{
   ...
   echo "<a href=\"edit_so_b.php?session=$session&school_ch=$school_ch\" class=small>Edit This Form</a>&nbsp;&nbsp;&nbsp;";
   echo "<a href=\"view_so_b.php?session=$session&school_ch=$school_ch&print=1\" class=small target=new>Printer-Friendly Version</a>&nbsp;&nbsp;&nbsp;";
...
if($send=='y')  //if box checekd at bottom of edit screen, send to NSAA
{
   ...
   $ToName="Jim Angele";
   $Subject="$school Boys Soccer State Tournament Roster";
   $Text="Attached is a CSV for Excel file of $school's Boys Soccer State Tournament 
Roster Information.  Thank you.";
   $Html="<font size=2 family=arial>Attached is a CSV-for-Excel file of $school'
s Boys Soccer State Tournament Roster information.<br><br>They have approved this as
