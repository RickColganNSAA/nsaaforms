<?php
//sobstateadmin.php: NSAA Boys Soccer Admin for State Entry Forms
...
echo "<br><table cellspacing=0 cellpadding=5 frame=all rules=all style=\"border:#a0a0a0 1px solid;\"><caption><b>State $sportname Entry Forms Admin</b><br><a href=\"sogstateadmin.php?session=$session\">Go to Girls Soccer Admin</a><br>";
if($errors && $errors!='')
{
...
if(mysql_num_rows($result)==0 || $viewdistrict==1)
{
   ...
   if(mysql_num_rows($result)>0) echo "<br>[<a class=small href=\"sobstateadmin.php?session=$session\">View schools who have submitted a STATE form</a>]";
   echo "<br><br>";
   echo "</caption>";
   if(!$sort || $sort=="") $sort="t1.school";
   $sql="SELECT DISTINCT t1.school,t2.sid,t2.filename FROM so_b AS t1, ".$sport."school AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id ORDER BY $sort"; 
   $result=mysql_query($sql);
echo mysql_error();
...
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a class=small href=\"view_so_b.php?school_ch=$row[school]&session=$session\" target=\"_blank\">View Form</a>&nbsp;|&nbsp;<a class=small href=\"view_so_b.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>&nbsp;|&nbsp;<a class=small href=\"../attachments.php?session=$session&filename=$filename\">Download for Excel</a>";
   if($statesubmitted==0)
      $links[$ix]="<a class=small href=\"view_so_b.php?school_ch=$row[school]&session=$session&viewdistrict=1&makepdf=1\" target=\"_blank\">Preview PDF</a>";
