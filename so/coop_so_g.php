<?php
//coop_so_g.php: pop-up window to add coop-students to district entry
...
<body>
<center>
<form name=form1 method="post" action="coop_so_g.php">
...
for($i=0;$i<8;$i++)
{
   ...
   {
      $sql="SELECT id, last, first, middle, semesters FROM eligibility WHERE so='x' AND gender='F' AND school='$coop_school[$i]' ORDER BY last";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
