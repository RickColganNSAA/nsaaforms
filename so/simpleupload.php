...
{	
	$sid=$_REQUEST['sid']; $sport=$_REQUEST['sport'];
	...
	if ( !empty($_FILES) )
	{
		...
		$uniqueFileName = sprintf("%s/%s_teamphoto_%s_1.%s",$pathinfo['dirname'],$sport,$sid,$pathinfo['extension']); //$fileName;
		for($k = 1; $k < 100; $k++)
		{
			...
			else
			{
				...
				$uniqueFileName = sprintf("%s/%s_teamphoto_%s_%s.%s", $pathInfo['dirname'], $sport, $sid, $k, $pathInfo['extension']);
                                $uniqueThumbName = sprintf("%s/%s_teamphoto_resized_%s_%s.%s", $pathInfo['dirname'], $sport, $sid, $k, $pathInfo['extension']);
			}
	...
	*/
			...
			$sql="UPDATE ".$sport."school SET filename='$useFileName' WHERE sid='$sid'";
...
<input type=hidden id='sport' name='sport' value="<?php echo $sport; ?>">
