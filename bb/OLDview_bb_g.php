...
$sql2="SELECT record,off_avg,def_avg,class_dist FROM $table WHERE school='$school2'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
$record=$row2[0];
$off_avg=$row2[1];
...
$info.="<td>".GetSchoolName($sid,'bbg',date("Y"))." $mascot</td></tr>";
$csv.="School/Mascot:,".GetSchoolName($sid,'bbg',date("Y"))." $mascot\r\n";
...
if($state==1)
{
   ...
   for($i=0;$i<count($sched[oppid]);$i++)
   {
      ...
      {
         $curopp=ereg_replace(", "," ",GetSchoolName($sched[oppid][$i],'bbg',$year));
