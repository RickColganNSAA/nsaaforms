<?php
//bbbstateadmin.php: NSAA Boys Basketball Admin for State Entry Forms
...
if ($classs)
{
header("Location:bbbstateadmin.php?session=$session&viewdistrict=1&class=$classs");
exit();
}
if ($export)
{
	...
	header('Content-Disposition: attachment; filename=bbbexportdata.csv');
	$output = fopen('php://output', 'w');
	
    $sql="SELECT DISTINCT t1.school,t2.class,t2.mascot,t2.colors,t2.coach,t2.sid FROM bb_b AS t1, bbbschool AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id "; 
    if($class && $class!='')
...
/*  if($row[mascot]!='') $mascot=$row[mascot];
	...
	$record=GetWinLoss($row[sid],'bbb');
	
	$sql_21="SELECT class_dist FROM bb_b WHERE school='$row[school]'";
	...
	$sql_11="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM bb_b AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL)";
	...
	$count=0;
	   ...
	   for($i=0;$i<count($sched[oppid]);$i++)
	   {
		  if($sched[oppid][$i]!='0')        //only individual games, not tournament names
		  {
			 $csv.="\"".GetSchoolName($sched[oppid][$i],'bbb')."\",\"";
	    ...
	    $sql_12="SELECT * FROM bbbschool WHERE sid='$row[sid]'";
		$result_12=mysql_query($sql_12);
		$row_12=mysql_fetch_array($result_12);
		
		$sql_13="SELECT name,asst_coaches FROM logins WHERE school='$row[school]' AND sport='Boys Basketball'";
...
if ($state_export)
{
    ...
    $sql="SELECT DISTINCT t1.school,t2.class,t2.mascot,t2.colors,t2.coach,t2.sid FROM bb_bstate AS t1, bbbschool AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id AND t1.submitted!='' "; 
	...
	$sql_21="SELECT class_dist FROM bb_bstate WHERE school='$row[school]'";
	$result_21=mysql_query($sql_21);
	$row_21=mysql_fetch_array($result_21);
	$class_dist=$row_21[0];
	...
	//$sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM sb_state AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL), t1.libero";
	$sql_11="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM bb_bstate AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL)";
...
echo "<br><table cellspacing=0 cellpadding=5 frame=all rules=all style=\"border:#a0a0a0 1px solid;\"><caption><b>State $sportname Entry Forms Admin</b><br><a href=\"bbgstateadmin.php?session=$session\">Go to Girls Basketball Admin</a><br>";
if($errors && $errors!='')
{
...
if(mysql_num_rows($result)==0 || $viewdistrict==1)
{
   ...
   if(mysql_num_rows($result)>0) echo "<br>[<a class=small href=\"bbbstateadmin.php?session=$session\">View schools who have submitted a STATE form</a>]";
   echo "<br><br>";
   echo "</caption>";
   /////////////////////
   ...
   $sql2="SELECT DISTINCT class FROM bbbschool WHERE class!='' ORDER BY class";
   ...
   echo "</select>";
   echo "&nbsp;&nbsp;| &nbsp;<a href=\"bbbstateadmin.php?session=$session&class=$class&export=yes\">Download for Excel</a></p></div></td></tr>";   
   
   /////////////////////
   //if(!$sort || $sort=="") 
   $sort="t1.school";
   $sql="SELECT DISTINCT t1.school,t2.sid,t2.filename FROM bb_b AS t1, ".$sport."school AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id ORDER BY $sort"; 
   $result=mysql_query($sql);
echo mysql_error();
...
else
{
   ...
   echo "</select>";
    echo "&nbsp;&nbsp;| &nbsp;<a href=\"bbbstateadmin.php?session=$session&class=$class&state_export=yes\">Download for Excel</a></p></div></td></tr>";   
   echo "</p></div>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a class=small href=\"view_bb_b.php?school_ch=$row[school]&session=$session\" target=\"_blank\">View Form</a>&nbsp;|&nbsp;<a class=small href=\"view_bb_b.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>&nbsp;|&nbsp;<a class=small href=\"../attachments.php?session=$session&filename=$filename\">Download for Excel</a>";
   if($statesubmitted==0)
      $links[$ix]="<a class=small href=\"view_bb_b.php?school_ch=$row[school]&session=$session&viewdistrict=1&makepdf=1\" target=\"_blank\">Preview PDF</a>";
