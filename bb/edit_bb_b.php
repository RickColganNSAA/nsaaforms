<?php
//edit_bb_b.php: Basketball entry form
...
$sql="SELECT choices FROM classes_districts WHERE sport='bb_b'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$class_array=split(",",$row[0]);

//get name of coach from logins table
$sql="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Boys Basketball'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$coach=$row[0];
$asst=$row[1];
...
$sql="SELECT duedate FROM form_duedates WHERE form='bb_b'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
$duedate=$row[0];
$date=split("-",$duedate);
...
/* DON'T LOCK JUST SWITCH DISTRICT TO STATE
else if(PastDue($duedate,0.5) && $level!=1)
{
//CHECK IF IT IS >2 DAYS PAST THE DUE DATE FOR THIS FORM
   ...
   $sql="SELECT * FROM bb_b WHERE school='$school2'";
   $result=mysql_query($sql);
   $ct=mysql_num_rows($result);
   if($ct>0)
   {
      echo "<a href=\"view_bb_b.php?session=$session&school_ch=$school_ch\">";
...
{
   $sql=ereg_replace("bb_bstate","bb_b",$sql);
   $result=mysql_query($sql);
}
$row=mysql_fetch_array($result);
$record=$row[0];
...
if($state==1 && mysql_num_rows($result)==0)
{
   ...
   $sql="SELECT * FROM bb_b WHERE school='$school2'"; //re-read state table
   $result=mysql_query($sql);
}
...
<form method=post action="submit_bb_b.php" enctype="multipart/form-data">
<input type=hidden name=session value=<?php echo $session; ?>>
<input type=hidden name=school_ch value="<?php echo $school_ch; ?>">
<input type=hidden name=state value=<?php echo $state; ?>>
<table width="800px">
<tr align=center>
<th>BOYS BASKETBALL <?php echo $form_type; ?> ENTRY</th>
</tr>
<?php
if($state!=1)
{
...
echo GetSchoolName($sid,'bbb')." $mascot";
   ?>
   </td>
   </tr>
   <tr align=left>
 ...
 your file has finished uploading, click "Save and Keep Editing" at the bottom of this page.</i></p>-->
...
if($level==1)
{
	  ...
	  $sql_coop="SELECT * FROM bbbschool WHERE mainsch='$row_id[id]' AND (othersch1!='' OR othersch2!='' OR othersch3!='') ";
...
?>
<tr align=left>
<td><p>Check the box next to the name of each student who will be participating in the <?php echo $form_type; ?> competition.  Required fields for the district tournament are marked with a *.  <b>All fields are required for the state entry form.</b>  When your team qualifies for the state tournament, you must update this form with complete statistics on each player by 10 a.m. the morning after your team has qualified.  No more than 14 may be designated as participants in any one district tournament game.  See NSAA Basketball Manual for rules about official tournament rosters.</p></td>
</tr>
...
?>
   <table cellspacing=0 cellpadding=3 frame=all rules=all style="border:#808080 1px solid;" class="nine">
<?php
$colheaders="
   <tr align=center>
...
echo $colheaders;
   ...
   $sql="SELECT id, first, last, middle, semesters, eligible FROM eligibility WHERE school='$school2' and bb='x' AND gender='M' ORDER BY last";
   $result=mysql_query($sql);
   $ix=0;
   while($row=mysql_fetch_array($result))
   {
