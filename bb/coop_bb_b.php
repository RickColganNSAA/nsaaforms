<?php
//coop_bb_b.php: pop-up window to add coop-students to district entry
...
<body>
<center>
<form name=form1 method="post" action="coop_bb_b.php">
...
for($i=0;$i<8;$i++)
{
   ...
   if($coop_school[$i] && $coop_school[$i]!="Choose School")
   {
      ...
      $sql="SELECT * FROM eligibility WHERE bb='x' AND gender='M' AND school='$coop_school[$i]' ORDER BY last";
      $result=mysql_query($sql);
      while($row=mysql_fetch_array($result))
      {
