<?php
//view_bb_b.php: Show submitted district
//   entry info.  If none have been submitted,
//   redirect to edit_bb_b.php
...
else if($director==1)
{
   ...
   $hostid=$row[0];
   $sql="SELECT * FROM $db_name2.bbbdistricts WHERE (hostid='$hostid' OR hostid2='$hostid')";
   $result=mysql_query($sql);
   if(mysql_num_rows($result)==0)
   {
      //CHECK bbbdisttimes (CLASS A BASKETBALL)
      $sql="SELECT * FROM $db_name2.bbbdisttimes WHERE hostid='$hostid'";
      $result=mysql_query($sql);
      if(mysql_num_rows($result)==0)
      {
...
{
   if($director!=1)
      header("Location:edit_bb_b.php?session=$session&school_ch=$school_ch");
   else
...
$sql2="SELECT name,asst_coaches FROM logins WHERE school='$school2' AND sport='Boys Basketball'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
$coach=$row2[0];
$asst=$row2[1];
...
if($print!=1)
{
echo "<a href=\"view_bb_b.php?session=$session&school_ch=$school_ch&print=1\" target=new class=small>Printer/E-mail Friendly Version</a>&nbsp;&nbsp;&nbsp;";
echo "<a href=\"edit_bb_b.php?session=$session&school_ch=$school_ch\" class=small>Edit this Form</a><br><br>";
...
$info.="<th>BOYS BASKETBALL $form_type ENTRY</th>";
$info.="</tr>";
if($state!=1)
{
   $info.="<tr align=center>";
...
$sid=GetSID2($school,'bbb');
$sql2="SELECT * FROM bbbschool WHERE sid='$sid'";
$result2=mysql_query($sql2);
$row2=mysql_fetch_array($result2);
...
$class=$row2['class'];
$info.="<td>".GetSchoolName($sid,'bbb')." $mascot</td></tr>";
$csv.="School/Mascot:,".GetSchoolName($sid,'bbb')." $mascot\r\n";
...
if($filename!='')
   ...
   $info.="<td><a href=\"edit_bb_b.php?session=$session&school_ch=$school_ch\">Click Here to Upload your Team Photo</a></td>";
if($level==1)
{
        //Superintendent
      ...
      if($state==1)
      {
        ...
        <form method=\"post\" action=\"view_bb_b.php\" target=\"_blank\">
...
if($entryct>14)
{
   echo "<font style=\"color:red; font-size:9pt\"><b>You have entered too many students!! </b>You may only enter <b>14</b> on this form. Please <a href=\"edit_bb_b.php?session=$session&school_ch=$school_ch\">correct this error</a>.</font>";
}
...
if($state==1)
{
   ...
   $sql="SELECT * FROM bbbschool WHERE sid='$sid'";
   $result=mysql_query($sql);
   $row=mysql_fetch_array($result);
...
<tr align=center>
<td><br>
    <a href="view_bb_b.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>&print=1" target=new class=small>Printer/E-mail Friendly Version</a>
    &nbsp;&nbsp;&nbsp;
    <a href="edit_bb_b.php?session=<?php echo $session; ?>&school_ch=<?php echo $school_ch; ?>" class=small>Edit this Form</a>
    &nbsp;&nbsp;&nbsp;
...
<?php
//echo "<input type=button name=addressbook value=\"Address Book\" onclick=\"window.open('../addressbook.php?session=$session&school_ch=$school2&form=bb_b','addressbook','menubar=no,location=no,resizable=no,scrollbars=yes,width=500,height=600')\">";
?>
</td>
</tr>
