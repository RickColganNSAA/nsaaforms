<?php
//bbgstateadmin.php: NSAA Girls Basketball Admin for State Entry Forms
//Created 2/28/09 because e-mailed forms were not being received, possibly due to spam filter
//2014 adapted for PDF generation for programs
//Author: Ann Gaffigan
...
if ($classs)
{
header("Location:bbgstateadmin.php?session=$session&viewdistrict=1&class=$classs");
exit();
}
if ($export)
{
	...
	header('Content-Disposition: attachment; filename=bbgexportdata.csv');
	$output = fopen('php://output', 'w');
	
    $sql="SELECT DISTINCT t1.school,t2.class,t2.mascot,t2.colors,t2.coach,t2.sid FROM bb_g AS t1, bbgschool AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id "; 
    if($class && $class!='')
...
/*  if($row[mascot]!='') $mascot=$row[mascot];
	...
	$record=GetWinLoss($row[sid],'bbg');
	
	$sql_21="SELECT class_dist FROM bb_g WHERE school='$row[school]'";
	...
	$csv.="No,Name,Grade,POS,HT,PTS,AVG,REB,AVG,AST,STL,BLK\r\n";
	...
	$sql_11="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM bb_g AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL)";
    
	$result_11=mysql_query($sql_11); 
	$count=0;
		while($row_11=mysql_fetch_array($result_11))
		{
		  if($row_11[checked]=="y")	//that student was checked to be on the roster
		  {
		     if($row[jersey_lt]=='') $row[jersey_lt]=$row[jersey_dk];
             $year=GetYear($row_11[semesters]);
			 $temp=split("-",$row_11[14]);
			 $test = explode("-",$row_11[height]);
			 $row_11[height]=$test[0].'--'.$test[1];
			 ...
			 $csv.="$row_11[jersey_lt],$row_11[first] $row_11[last],$year,$row_11[position],$row_11[height],$row_11[total_pts],$row_11[pt_avg],$row_11[total_rb],$row_11[reb_avg],$row_11[total_assists],$row_11[total_steals],$row_11[total_blocks]\r\n";
        
			 $count++;
		  }
		}
		$csv.="Games\r\n\"Opponent\",\"W/L\",\"Score\",\"Opp. Score\"\r\n";
		$sched=GetSchedule($row[sid],'bbg');
		$gamect=0;
	   ...
	   for($i=0;$i<count($sched[oppid]);$i++)
	   {
		  if($sched[oppid][$i]!='0')        //only individual games, not tournament names
		  {
			 $csv.="\"".GetSchoolName($sched[oppid][$i],'bbg')."\",\"";
	    ...
	    $sql_12="SELECT * FROM bbgschool WHERE sid='$row[sid]'";
		$result_12=mysql_query($sql_12);
		$row_12=mysql_fetch_array($result_12);
		
		$sql_13="SELECT name,asst_coaches FROM logins WHERE school='$row[school]' AND sport='Girls Basketball'";
...
if ($state_export)
{
    ...
    $sql="SELECT DISTINCT t1.school,t2.class,t2.mascot,t2.colors,t2.coach,t2.sid FROM bb_gstate AS t1, bbgschool AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id AND t1.submitted!='' "; 
	...
	$sql_21="SELECT class_dist FROM bb_gstate WHERE school='$row[school]'";
	$result_21=mysql_query($sql_21);
	$row_21=mysql_fetch_array($result_21);
	$class_dist=$row_21[0];
	...
	//$sql="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM sb_state AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL), t1.libero";
	$sql_11="SELECT t1.*, t2.last, t2.first, t2.middle, t2.semesters FROM bb_gstate AS t1, eligibility AS t2 WHERE t1.student_id=t2.id AND (t1.school='$row[school]' OR t1.co_op='$row[school]') AND t1.checked='y' ORDER BY CAST(t1.jersey_lt AS DECIMAL)";
...
echo "<br><table cellspacing=0 cellpadding=5 frame=all rules=all style=\"border:#a0a0a0 1px solid;\"><caption><b>State $sportname Entry Forms Admin</b><br><a href=\"bbbstateadmin.php?session=$session\">Go to Boys Basketball Admin</a><br>";
if($errors && $errors!='')
{
...
if(mysql_num_rows($result)==0 || $viewdistrict==1)
{
   ...
   if(mysql_num_rows($result)>0) echo "<br>[<a class=small href=\"bbgstateadmin.php?session=$session\">View schools who have submitted a STATE form</a>]";
   echo "<br><br>";
   echo "</caption>";
         /////////////////////
   ...
   $sql2="SELECT DISTINCT class FROM bbgschool WHERE class!='' ORDER BY class";
   ...
   echo "</select>";
   echo "&nbsp;&nbsp;| &nbsp;<a href=\"bbgstateadmin.php?session=$session&class=$class&export=yes\">Download for Excel</a></p></div></td></tr>";   
   
   /////////////////////
   //if(!$sort || $sort=="") 
   $sort="t1.school";
   $sql="SELECT DISTINCT t1.school,t2.sid,t2.filename FROM bb_g AS t1, ".$sport."school AS t2, headers AS t3 WHERE t1.school=t3.school AND t2.mainsch=t3.id ORDER BY $sort"; 
   $result=mysql_query($sql);
echo mysql_error();
...
else
{
   ...
   echo "</select>";
   echo "&nbsp;&nbsp;| &nbsp;<a href=\"bbgstateadmin.php?session=$session&class=$class&state_export=yes\">Download for Excel</a></p></div></td></tr>"; 
   echo "</p></div>";
...
while($row=mysql_fetch_array($result))
{
   ...
   $schs[$ix]=$row[school];
   $links[$ix]="<a class=small href=\"view_bb_g.php?school_ch=$row[school]&session=$session\" target=\"_blank\">View Form</a>&nbsp;|&nbsp;<a class=small href=\"view_bb_g.php?school_ch=$row[school]&session=$session&makepdf=1\" target=\"_blank\">Preview PDF</a>&nbsp;|&nbsp;<a class=small href=\"../attachments.php?session=$session&filename=$filename\">Download for Excel</a>";
   if($statesubmitted==0)
      $links[$ix]="<a class=small href=\"view_bb_g.php?school_ch=$row[school]&session=$session&viewdistrict=1&makepdf=1\" target=\"_blank\">Preview PDF</a>";
